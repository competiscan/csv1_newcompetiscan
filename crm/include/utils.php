...
function get_user_array($add_blank=true, $status="Active", $assigned_user="", $use_real_name=false, $user_name_begins = null, $is_group=' AND portal_only=0 ', $from_cache=true) {
	...
	//if($from_cache)
         if($from_cache && !empty($assigned_user))    
...
function return_module_language($language, $module, $refresh=false) {
    ...
    if(!file_exists('cache/modules/'. $module . '/language/'.$language.'.lang.php') 
...
/**
 * Check to display the studio link in Administration.
 * 
 * @return boolean true if the current user is an admin for a module other than Forecasts
 * as that is not a Studio enabled module and false in all other cases.
 */
...
/**
 * Returns modules for which the user is an admin.
 * The first time the function is called, the values are saved in a SESSION variable.
 * There is a special case for ForecastSchedule because it doesn't exist in the array "moduleList"
 * There are two kinds of admin (developer or admin+developer)
 * 
 * @param Object user,
 * @return an array with all modules for which the user is an admin.
 */
...
/**
 * Check if the user is an admin for at least one module.
 * The first time this function is called is when the user is logging in and the result has 
 * to be false not to display the admin link that's why we can't keep the value in a SESSION variable.
 * We use a SESSION variable when this step is done ( $_REQUEST['module']!= 'Users') or when the result is true.
 * There is a special case for ForecastSchedule because it doesn't exist in the array "moduleList"
 * There are two kinds of admin (developer or admin+developer)
 * 
 * @param Object user,
 * @return boolean true if the user is an admin for at least one module.
 *
 */
...
/**
 * Check if a user is an admin for a specific module.
 * There are two kinds of admin (developer or admin+developer)
 *
 * @param Object user 
 * @param String module name
 * @return boolean true if the user is admin for the module specified.
 *
 */
...
function clean_incoming_data() {
	...
	foreach ($_GET as $key => $val) {
		if (str_end($key, "_offset")) {
			clean_string($_GET[$key], "ALPHANUM"); // keep this ALPHANUM for disable_count_query
		}
...
function get_image($image,$other_attributes,$width="",$height=""){
        ...
        global $png_support;
        $rel_path = $image;
        if(defined('TEMPLATE_URL'))$rel_path = str_replace(TEMPLATE_URL.'/', '', $image);
        if ($png_support == false)
        $ext = "gif";
        else
        $ext = "png";
        $out = '';
		if (file_exists('custom/'. $rel_path.'.'.$ext)){
			$rel_path = 'custom/'. $rel_path;	
		}
        if (file_exists($rel_path.'.'.$ext)){
                $size=getimagesize($rel_path.'.'.$ext);
                if ($width == "") { $width = $size[0];}
                if ($height == "") { $height = $size[1];}
                $path = $image. '.'.$ext ;
                $out= "<img src='$path' width='".$width."' height='".$height."' $other_attributes>";
        }else if(substr_count($rel_path,'themes') > 0){
                $path = explode('/',$rel_path);
                $path[1] = 'default';
                $new_path = implode('/',$path);
				if(file_exists('custom/' . $new_path .'.'.$ext)){
					$new_path = 'custom/' . $new_path;
				}
                if (file_exists($new_path.'.'.$ext)){
                        $size = getimagesize($new_path.'.'.$ext);
                        if ($width == "") { $width = $size[0];}
                        if ($height == "") { $height = $size[1];}
                        $path = $new_path.'.'.$ext;
                        if(defined('TEMPLATE_URL'))$path = getWebPath($path);
                        $out= "<img src='$path' width='".$width."' height='".$height."' $other_attributes>";
                }

        }
        // Cache everything but the other attributes....
        $cached_results[$image] = "<img src='$path' width='".$width."' height='".$height."' ";
...
function getImagePath($image_name){
        ...
        static $default_path  = '';
        static $rel_path = '';
        static $default_rel_path = '';
        if(empty($rel_path)){
                $rel_path = $GLOBALS['image_path'];
                if(defined('TEMPLATE_URL'))$rel_path = str_replace(TEMPLATE_URL. '/', '', $GLOBALS['image_path']);
        }

         if(empty($default_path)){
                $default_path = str_replace('themes/'.$GLOBALS['theme'], 'themes/default', $GLOBALS['image_path']);
                $default_rel_path = str_replace('themes/'.$GLOBALS['theme'], 'themes/default', $rel_path);

         }
		 if(file_exists('custom/' . $rel_path .'/'. $image_name)){
		 	$image_path = 'custom/' . $rel_path .'/'. $image_name;
		 }else if (file_exists('custom/'. $default_path . '/' . $image_name)){
		 		$image_path =   'custom/'. $default_path . '/' . $image_name;
		 }
         else if(!file_exists($rel_path .'/'. $image_name) && file_exists($default_rel_path . '/' . $image_name)){
                $image_path =   $default_path . '/' . $image_name;
         }
         return $image_path;
...
function getJSPath($relative_path, $additional_attrs=''){
	...
	$path = $relative_path . '?s=' . $GLOBALS['js_version_key'] . '&c=' . $GLOBALS['sugar_config']['js_custom_version'] ;
...
function check_php_version($sys_php_version = '') {
	...
	// PHP version not supported on Oracle DB
	if ($dbType != 'oci8' || $dbType != 'oracle') {
		array_push($supported_php_versions, '5.1.4', '5.1.6');
	}
	// invalid versions above the $min_considered_php_version,
	// should be mutually exclusive with $supported_php_versions

	// SugarCRM prohibits install on PHP 5.0.0 on all platforms and
	// 5.1.4 and 5.1.6 on Oracle DB.
	$invalid_php_versions = array('5.0.0','5.2.0', '5.2.7');
	if($dbType == 'oci8' || $dbType == 'oracle') {
		array_push($invalid_php_versions, '5.1.4', '5.1.6');
	}
	// default unsupported
	$retval = 0;
