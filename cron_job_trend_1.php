...
foreach ($trendnotificationlist as $row_data) {
    ...
    $querySql="SELECT SQL_NO_CACHE DISTINCT ctr.trend_id,trend_name,trend_link,trend_date,ctr.rndtrend_id,file_path,file_name,audience_id,country_id,ctc.sector_id,ctc.category_id,ctc.subcategory_id,ctc.subtosubcategory_id FROM cscan_trend_report ctr JOIN cscan_trends_category ctc ON ctc.trend_id = ctr.trend_id WHERE 1=1  $andCond GROUP BY ctr.trend_id ORDER BY ctr.trend_date DESC";
...
function getTrendNotificationListData() {
    ...
    $query = "SELECT SQL_NO_CACHE DISTINCT ID, searchKey,searchType, mPanelID,sectorID,categoryID,subCategoryID,subSubCategoryID,s.country, s.userID,
        ...
        WHERE emailAlert='1' AND u.userID =45037
             AND active='y' AND
