...
if ($_SERVER['argc'] < 3) {
    print "exportExcel_back2.php sessionid serialized_array\n";
    exit;
}
...
$sectorIDArray = array();
if ($ssid > 0) {
    $sql = "SELECT mPanelID,mChannelID,sectorID,userID FROM cscan_search WHERE ID=$ssid";
...
if ($consumer) {
    ...
    $heading["mailspend"] = 'Estimated Spend';
...
$heading['estimated_spend'] = 'Spend ($)';
$heading['estimated_impression'] = 'Impression';
...
$appendsArray = array();
if ($more >= 1) {
    if ($sess_plevel == 1 || $sess_plevel == 2 || !in_array('me_conp', $sess_search_exclude) || !in_array('me_prizm', $sess_search_exclude)) {
    ...
    if ($is_excel > 0) {
        ...
        if ($sess_plevel == 1 || $sess_plevel == 2) {
            if ($more > 2 || $more == 1) {
                if (!empty($showheading['invitationID'])) {
                    $header .= csvExcape('Invitation ID') . ",";
                }
                if (!empty($showheading['trackingID'])) {
...
if ($ssid > 0 || $bid >= 0) {
    ...
    while ($row = $DRW->fetch_assoc($rs)) {
        ...
        if ($more > 0 || (($search_panelist_date || $consumer_only || $do_bid) && ($showheading['state'] || $showheading['gender'] || $showheading['age'] || $showheading['income'] || $showheading['DMA_ID']))) {
            $is_mv = false;
            $sql_P = "SELECT DISTINCT DATE_FORMAT(ppdate,'%Y-%m'),competi_id,invitationID,pp.panelist_id,pp.ppstateID,pp.pgender,pp.ppageID,pp.pincomeID,pp.pppostalcode,pp.ppmv,trackingID,pp.ppmv_m,dmap.code,pproductFICO
                ...
                if (!empty($showheading[$k])) {
                    switch ($k) {
                        ...
                        case 'sem_search_key':
                            ...
                            if (!empty($productID)) {
                                ...
                                if ($DRW->num_rows($res_creative_tbl) > 0) {
                                    while ($res_creative_row = $DRW->fetch_assoc($res_creative_tbl)) {
                                        ...
                                        $sql_md5_key = "SELECT ad_md5 FROM $creative_table WHERE productID = '" . $productID . "'";
                                        $query_md5_key = $DRW->query($sql_md5_key, $DRW_digital);
                                        if ($DRW->num_rows($query_md5_key) > 0) {
                                            $res_md5_key = $DRW->fetch_assoc($query_md5_key);
                                            $ad_md5_key = $res_md5_key['ad_md5'];
                                            break;
                                        }
                                    }
                                }
                                if (!empty($ad_md5_key)) {
                                    ...
                                    while ($res_semdet_row = $DRW->fetch_assoc($res_semdet_tbl)) {
                                        ...
                                        $sql_semdet_key = "SELECT sem_search_key FROM " . $semdet_table . " WHERE ad_md5 = '" . $ad_md5_key . "' AND sem_headline IS NOT NULL LIMIT 1";
                                        $res_sem_det_key = $DRW->query($sql_semdet_key, $DRW_digital);
                                        if ($DRW->num_rows($res_sem_det_key) > 0) {
                                            $data_sem_det_key = $DRW->fetch_assoc($res_sem_det_key);
                                            $flag_sem = true;
                                            break;
                                        }
                                    }
                            ...
                            if (isset($data_sem_det_key['sem_search_key']) && $data_sem_det_key['sem_search_key'] != '') {
                                $sem_search_key = $data_sem_det_key['sem_search_key'];
                            } else {
                                $sem_search_key = '';
                            }
                            @$DRW->free_result($res_sem_det_key);
                            $data_sem_det_key = array();
                            if ($is_excel > 0) {
                                if ($is_excel == 2) {
                        ...
                        case 'sem_url':
                            ...
                            if (!empty($productID)) {
                                ...
                                if ($DRW->num_rows($res_creative_tbl) > 0) {
                                    while ($res_creative_row = $DRW->fetch_assoc($res_creative_tbl)) {
                                        $creative_table = $res_creative_row['table_name'];
                                        $sql_md5_url = "SELECT ad_md5 FROM $creative_table WHERE productID = '" . $productID . "'";
                                        $query_md5_url = $DRW->query($sql_md5_url, $DRW_digital);
                                        if ($DRW->num_rows($query_md5_url) > 0) {
                                            $res_md5_url = $DRW->fetch_assoc($query_md5_url);
                                            $ad_md5_url = $res_md5_url['ad_md5'];
                                            break;
                                        }
                                    }
                                }
                                if (!empty($ad_md5_url)) {
                                    ...
                                    while ($res_semdet_row = $DRW->fetch_assoc($res_semdet_tbl)) {
                                        $semdet_table = $res_semdet_row['table_name'];
                                        $sql_semdet_url = "SELECT sem_url FROM " . $semdet_table . " WHERE ad_md5 = '" . $ad_md5_url . "' AND sem_headline IS NOT NULL LIMIT 1";
                                        $res_sem_det_url = $DRW->query($sql_semdet_url, $DRW_digital);
                                        if ($DRW->num_rows($res_sem_det_url) > 0) {
                                            $data_sem_det_url = $DRW->fetch_assoc($res_sem_det_url);
                                            $flag_sem = true;
                                            break;
                                        }
                                    }
                            ...
                            if (isset($data_sem_det_url['sem_url']) && $data_sem_det_url['sem_url'] != '') {
                            ...
                            $sem_url = '';
                            @$DRW->free_result($res_sem_det_url);
                            $data_sem_det_url = array();
                            break;
                        case 'sem_headline':
                            $productID = $row['theproductID'];
                            $sem_headline = '';
                            if (!empty($productID)) {
                                ...
                                if ($DRW->num_rows($res_creative_tbl) > 0) {
                                    while ($res_creative_row = $DRW->fetch_assoc($res_creative_tbl)) {
                                        $creative_table = $res_creative_row['table_name'];
                                        $sql_md5_head = "SELECT ad_md5 FROM $creative_table WHERE productID = '" . $productID . "'";
                                        $query_md5_head = $DRW->query($sql_md5_head, $DRW_digital);
                                        if ($DRW->num_rows($query_md5_head) > 0) {
                                            $res_md5_head = $DRW->fetch_assoc($query_md5_head);
                                            $ad_md5_head = $res_md5_head['ad_md5'];
                                            break;
                                        }
                                    }
                                }
                                if (!empty($ad_md5_head)) {
                                    ...
                                    while ($res_semdet_row = $DRW->fetch_assoc($res_semdet_tbl)) {
                                        $semdet_table = $res_semdet_row['table_name'];
                                        $sql_semdet_head = "SELECT sem_headline FROM " . $semdet_table . " WHERE ad_md5 = '" . $ad_md5_head . "' AND sem_headline IS NOT NULL LIMIT 1";
                                        $res_sem_det_head = $DRW->query($sql_semdet_head, $DRW_digital);
                                        if ($DRW->num_rows($res_sem_det_head) > 0) {
                                            $data_sem_det_head = $DRW->fetch_assoc($res_sem_det_head);
                                            $flag_sem = true;
                                            break;
                                        }
                                    }
                            ...
                            if (isset($data_sem_det_head['sem_headline']) && $data_sem_det_head['sem_headline'] != '') {
                            ...
                            $sem_headline = '';
                            @$DRW->free_result($res_sem_det_head);
                            $data_sem_det_head = array();
                            break;
                        case 'sem_description':
                            ...
                            if (!empty($productID)) {
                                ...
                                if ($DRW->num_rows($res_creative_tbl) > 0) {
                                    while ($res_creative_row = $DRW->fetch_assoc($res_creative_tbl)) {
                                        $creative_table = $res_creative_row['table_name'];
                                        $sql_md5_desc = "SELECT ad_md5 FROM $creative_table WHERE productID = '" . $productID . "'";
                                        $query_md5_desc = $DRW->query($sql_md5_desc, $DRW_digital);
                                        if ($DRW->num_rows($query_md5_desc) > 0) {
                                            $res_md5_desc = $DRW->fetch_assoc($query_md5_desc);
                                            $ad_md5_desc = $res_md5_desc['ad_md5'];
                                            break;
                                        }
                                    }
                                }
                                if (!empty($ad_md5_desc)) {
                                    ...
                                    while ($res_semdet_row = $DRW->fetch_assoc($res_semdet_tbl)) {
                                        $semdet_table = $res_semdet_row['table_name'];
                                        $sql_semdet_desc = "SELECT sem_description FROM " . $semdet_table . " WHERE ad_md5 = '" . $ad_md5_desc . "' AND sem_headline IS NOT NULL LIMIT 1";
                                        $res_sem_det_desc = $DRW->query($sql_semdet_desc, $DRW_digital);
                                        if ($DRW->num_rows($res_sem_det_desc) > 0) {
                                            $data_sem_det_desc = $DRW->fetch_assoc($res_sem_det_desc);
                                            $flag_sem = true;
                                            break;
                                        }
                                    }
                            ...
                            if (isset($data_sem_det_desc['sem_description']) && $data_sem_det_desc['sem_description'] != '') {
                            ...
                            $sem_description = '';
                            @$DRW->free_result($res_sem_det_desc);
                            $data_sem_det_desc = array();
                            break;
...
//							}
                        ...
                        case 'doclink':
                            if (in_array($row['mChannelID'], array(5, 7))) {
                                $url = 'http://www.competiscan.com/productDocuments.php?did=2&id=' . $row['theproductID'];
                            } else {
                                $url = 'http://www.competiscan.com/productDocuments.php?did=1&id=' . $row['theproductID'];
                            }
                            ...
                            if (!empty($productID)) {
                                ...
                                if (!empty($ad_md5)) {
                                    if (in_array($more, array(1, 87, 90))) {
                                        $imp_sql = "SELECT cp.panelist_id FROM cscan_panelists_product cpp INNER JOIN cscan_panelists cp ON(cpp.panelist_id = cp.panelist_id) WHERE cpp.productID = '" . $productID . "' AND cp.panelist_id='" . $pid . "'";
                                        ...
                                        if ($DRW->num_rows($imp_query) > 0) {
                                            ...
                                            while ($res_obse_row = $DRW->fetch_assoc($res_obse_tbl)) {
                                                ...
                                                $sql_impression = "SELECT panelist_id,estimated_spend, estimated_impressions FROM " . $obse_table . " WHERE ad_md5 = '" . $ad_md5 . "' AND panelist_id = '" . $pid . "' AND estimated_spend IS NOT NULL AND estimated_impressions IS NOT NULL";
                                            ...
                                            if (!empty($arr_sql)) {
                                                ...
                                                if ($DRW->num_rows($res_union_query) > 0) {
                                                    $res_union_row = $DRW->fetch_assoc($res_union_query);
                                                    $esti_spend = round($res_union_row['estimated_spend']);
                                                    $esti_impressions = round($res_union_row['estimated_impressions']);
                                                }
                                            }
                                        }
                                    } else {
                                        $arr_panelists = array();
                                        $imp_sql = "SELECT cp.panelist_id, cp.competi_id FROM cscan_panelists_product cpp INNER JOIN cscan_panelists cp ON(cpp.panelist_id = cp.panelist_id) WHERE cpp.productID = '" . $productID . "'";
                                        $imp_query = $DRW->query($imp_sql, $DRW_read);
                                        if ($DRW->num_rows($imp_query) > 0) {
                                            while ($imp_res = $DRW->fetch_assoc($imp_query)) {
                                        ...
                                        if (!empty($arr_panelists)) {
                                            ...
                                            if (!empty($arr_sql)) {
                                                ...
                                                $res_union_query = $DRW->query($sql_union, $DRW_digital);
                                                if ($DRW->num_rows($res_union_query) > 0) {
                                                    $total_spend = $total_impressions = 0;
                        ...
                        case 'refinance':
                            $productID = $row['theproductID'];
                            $refinance = $jumbo_ncnfg = $va = $fha = $conventional = $usda = 0;
                            if (!empty($productID)) {
                                $sql = "SELECT refinance,jumbo_ncnfg,va,fha,conventional,usda FROM cscan_product_detail WHERE productID='" . $productID . "'";
