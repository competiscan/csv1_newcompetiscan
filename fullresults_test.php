...
if($search_num_of_rows > 0) {
	...
	*/
	...
	$buttons .= '<div style="float:left;margin-left:0px;"><form action="myExcel1_latest.php" method="post" name="myexceller"><input type="hidden" name="ssid" value="'.$ssid.'" /><input type="hidden" name="bid" value="'.$bid.'" /><input type="hidden" name="page" value="'.$page.'" /><input type="hidden" name="num_rec" value="'.$search_num_of_rows.'" /><input type="hidden" name="sort" value="'.$sort.'" /><input class="submitbutton" type="submit" name="myExcel" value="My Excel" /></form></div><!--search--><!--paging-->';
}
else {
...
?>
               ...
               //if(($mid==3) AND ($rs['mPanelID']==1|| $rs['mPanelID']==2) AND ($rs['sectorID']==266) AND ($rs['categoryID']==269) ) {     
                   ...
                   $query_result_prod_email = $DRW->query($queryProdEmail,$DRW_read);
                    ...
                    if($numrows2>0){
                       ...
                       $url='https://vat-api.competiscan.com/v3/reviewed';
	...
	if ($_SESSION['sess_plevel'] > 0) {
            ...
            elseif ($rs['mChannelID']==3 || $rs['mChannelID']==1 || $rs['mChannelID']==2 || $rs['mChannelID']==6 || $rs['mChannelID']==14) {
