...
function insertData($type,$curdate){
    ...
    if ($sqlqueryres) {
        while ($row = $DRW->fetch_assoc($sqlqueryres)) {
            ...
            //Start of change check parent panelist not exists then parent insert into table
            if($parent_panelist_id!=0){
             $selsql_parent     =   "select id from cscan_consumer_scoring_report 
                            where panelist_id='".$parent_panelist_id."'                            
                         ";
            $ressql_parent     = $DRW->query($selsql_parent,$DRW_read2);  
                if ($DRW->num_rows($ressql_parent) < 1) { 
                    $sqlPanelist="Select competi_id from cscan_panelists where panelist_id='".$parent_panelist_id."'";
                    $rssqlPanelist = $DRW->query($sqlPanelist,$DRW_read2);
                    if($DRW->num_rows($rssqlPanelist)>0){
                        $digitalupdatedate=''; 
                        if($type=='3'){
                          $digitalupdatedate=",digital_update_date= '".$curdate."'";  
                         }
                         $rowDataCompeti_id = $DRW->fetch_assoc($rssqlPanelist);
                         $par_competi_id = $rowDataCompeti_id['competi_id'];
                         $sql_parent  = "insert into cscan_consumer_scoring_report set 
                                panelist_id='".$parent_panelist_id."',
                                competi_id ='".$par_competi_id."',
                                parent_panelist_id=0,entry_date='".$curdate."'$digitalupdatedate";
                        $DRW->query($sql_parent,$DRW_main); 
                    }
                }   
            }
            //End of change check parent panelist not exists then parent insert into table
