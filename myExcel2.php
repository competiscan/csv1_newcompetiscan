...
<table width="100%" border="0" cellpadding="5" cellspacing="0" class="bodytext">
	...
	<input type="hidden" name="ssid" value="<?php echo $_SESSION['sess_api_searchID']; ?>" />
	<input type="hidden" name="bid" value="<?php echo $basket_id; ?>" />
	<input type="hidden" name="page" value="<?php echo $page; ?>" />
	<input type="hidden" name="sort" value="<?php echo $sort; ?>" />
    ...
    //                    $showArray[] = array("usda","USDA");
		...
		if(in_array('sender_domain_permission',$_SESSION['sess_sender_domain'])){
			//if(ENV == 'localhost' || ENV == 'demo.competiscan.com'){
				$showArray[] = array('sender_domain_name','Sender Domain Name');
			//}
		}
		################add New field at myexcel#########
		$cols = 3;
		...
		<label><input type="radio" name="file_choice" value="2" checked='checked'<?php //if($templateFileType==2) echo ' checked="checked"'; ?> />Comma Separated Value (.csv)</label></td>
...
$(document).ready(function() {
    $('.myExcel_download').click(function() {
        ...
        function checkProgress() {
            ...
            var progressUrl = '<?php echo PROGRESS_MYEXCEL_UAT.$_SESSION['sess_userID']."/".$_SESSION['sess_api_searchID']; ?>/'+status_unique_id;
        ...
        $.ajax({
            url: 'load_ajax_myexceldata_uat.php',
...
$('.generate_chart').click(function() {
	...
	$.ajax({
		url: 'generatechart_data_uat.php',
