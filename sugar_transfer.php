...
while($data = $DRW->fetch_row($result)){
	...
	$email2 = '';
	$more_email_c = '';
	$q2 = "SELECT SQL_NO_CACHE email_address,primary_address FROM email_addresses ea, email_addr_bean_rel eb WHERE bean_id='".$DRW->real_escape_string($id)."' AND ea.id=eb.email_address_id AND bean_module='Contacts' order by primary_address desc,eb.date_created asc";
	...
	$resultp = $DRW->query($qp,$DRW_read2);
...
$result = $DRW->query($q,$DRW_read2);
while($data = $DRW->fetch_row($result)){
	
	$result_c_p = $DRW->query("SELECT SQL_NO_CACHE panelist_id,stateID FROM cscan_panelists WHERE active=1 AND (email='".$DRW->real_escape_string($data[0])."' OR alt_email='".$DRW->real_escape_string($data[0])."' OR more_email LIKE '%".mysqlLike($data[0])."%') LIMIT 1",$DRW_read2);
...
while($data = $DRW->fetch_row($result)){
	...
	if($ppstateID!=0){
		...
		$resultps = $DRW->query( $sqlps,$DRW_read2 );
