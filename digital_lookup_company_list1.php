...
if($companytext!='' || $showall) {
	$DIGITALAPILINK=DIGITAL_DASHBOARD_UAT.'company';
	$postdata = ['company_name' => $companytext];
	if($showall || $companytext!=''){
		$posteddata = json_encode($postdata);
		$get_data = callAPI('POST', $DIGITALAPILINK, $posteddata);
	}
	...
	// foreach($response['data'] as $allCompanyData){

	// 	echo $allCompanyData['competiscan_company']."\n";
	// }
    // echo "</pre>";
    // die;
}
...
if($rows>0 || count($companyArray)>0){
	...
	if($companytext!='' || $showall) {
		foreach($response['data'] as $allCompanyData){
			if(!in_array(strtolower($allCompanyData['competiscan_company']),$usedArray)){
				$key = array_search($allCompanyData['competiscan_company'],$companyArray);
				if($key!==false || ($companychecked && strtolower($allCompanyData['competiscan_company'])==$companytext_low)) {
					if(isset($companyArray[$key])) {
						unset($companyArray[$key]);
					}
					echo "<tr><td valign=\"top\"><input type=\"checkbox\" name=\"companysel[]\" value=\"".htmlspecialchars($allCompanyData['competiscan_company'], ENT_QUOTES)."\" onclick=\"doCompany();\" id=\"co$i\" checked=\"checked\" /></td><td><label for=\"co$i\">".$allCompanyData['competiscan_company']."</label></td></tr>";
				}
				else {
					echo "<tr><td valign=\"top\"><input type=\"checkbox\" name=\"companysel[]\" value=\"".htmlspecialchars($allCompanyData['competiscan_company'], ENT_QUOTES)."\" onclick=\"doCompany();\" id=\"co$i\" /></td><td><label for=\"co$i\">".$allCompanyData['competiscan_company']."</label></td></tr>";
				}
				$i++;
			}
		}
