            ...
            if filename in file:
                ...
                if int(dops)==2:
                    ...
                    if height >500 and height< 800:
                        new_img=img.resize((300,600),Image.ANTIALIAS)
                        new_img.save(path+file, 'JPEG', optimize=True,quality=90)
                    if height > 800 and height< 1000:
                        new_img=img.resize((300,800),Image.ANTIALIAS)
                        new_img.save(path+file, 'JPEG', optimize=True,quality=90)
                    if height > 1000 and height< 2500:
                        new_img=img.resize((300,1000),Image.ANTIALIAS)
                        new_img.save(path+file, 'JPEG', optimize=True,quality=90)
                    if height > 2500 and height< 6000:
                        new_img=img.resize((300,1500),Image.ANTIALIAS)
                        new_img.save(path+file, 'JPEG', optimize=True,quality=90)
                    '''if (width > 400):
