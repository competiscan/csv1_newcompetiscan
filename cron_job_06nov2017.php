...
$search = new \HS\Search($DRW, $DRW_read);
$user = new \HS\User($DRW, $DRW_read);
...
foreach ($notificationList as $row) {
    ...
    if (!isset($email_list[$row['userID']]))
        continue;

    $emailAddress = $email_list[$row['userID']]['emailAddress'];
    $is_public_user = $email_list[$row['userID']]['is_public_user'];
   ...
   // list($query) = doQuery($row['ID'],false,$usedate,false,-1,false,false,false,true,0,array(),$row['userID']);
    ...
    $result_product = $DRW->query($query.' ORDER BY company ASC, productHeadline ASC LIMIT 0,5000',$DRW_read);
...
$message='';
        ...
        $message .= '<table width="70%" align="center" cellspacing="1" cellpadding="4" border="1" rules="1" style="border-collapse:collapse" style="FONT-FAMILY: verdana, Arial, Helvetica, sans-serif;FONT-SIZE: 10pt;COLOR: #000000;TEXT-DECORATION: none;" bordercolor="4892F7">
        ...
        <th width="4%">Alert</th>
        <th width="36%" align="left">Company</th>
        <th width="60%" align="left">Headline</th>
        </tr>';
        ...
        while ($row_product = $DRW->fetch_array($result_product)) {
            ...
            if ($row['is_public'] || $is_public_user) {
                ...
                $link = 'https://www.competiscan.com/productDocuments.php?id='.$row_product['theproductID'].$did;
            } else {
                $link = 'https://www.competiscan.com/index.php?product='.$row_product['theproductID'];
            }
            ...
            $text_message .= "\nAlert #  :\t\t\t".$sn++."\nCompany  :\t\t\t".$company."\nHeadline :\t\t\t".$headline."\nURL      :\t\t\thttp://www.competiscan.com/index.php?product=".$row_product['theproductID']."\n";
            $text_message .= "--------------------------------------------------------------------------------------";
        }
       }
...
if ($result) {
    while ($row = $DRW->fetch_row($result)) {
        ...
        $query2 = $DRW->query($sqltwo,$DRW_read);
        ...
        $text_message = 'A new '.$sectorName.' trend report has been posted to the Trend Reports page at www.competiscan.com. For your convenience, a link to the report appears below. Please allow 2-3 minutes for the report to open.'."\n\n".'http://www.competiscan.com/index.php?trend_id='.$trend_id;
        $message = '<html><body>
        <p style="FONT-FAMILY: verdana, Arial, Helvetica, sans-serif;FONT-SIZE: 10pt;">A new <strong>'.htmlspecialchars($sectorName).'</strong> trend report has been posted to the Trend Reports page at <a href="http://www.competiscan.com/" target="_new" title="Click here to go to Competiscan">www.competiscan.com</a>. For your convenience, a link to the report appears below. Please allow 2-3 minutes for the report to open.</p>
        <p style="FONT-FAMILY: verdana, Arial, Helvetica, sans-serif;FONT-SIZE: 10pt;"><a href="http://www.competiscan.com/index.php?trend_id='.$trend_id.'" target="_new" title="Click here to view trend report on Competiscan">'.htmlspecialchars($sectorName.' '.$trend_name).'</a></p>
        </body></html>>';

        $sqlthree = "SELECT SQL_NO_CACHE userID FROM cscan_sector_users WHERE sectorID=$sectorID AND LEFT(trendSent,10)<'$trend_date' AND trend_id<>$trend_id";
        $query3 = $DRW->query($sqlthree,$DRW_read);

        while ($row3 = $DRW->fetch_row($query3)) {
            $userID = $row3[0];

            if (!isset($email_list[$userID]))
                continue;
