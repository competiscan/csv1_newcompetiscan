...
if(isset($_POST['basket_action'])){
	...
	if($basket_action>0){
		if($basket_action==7 || $basket_action==8 || $basket_action==9 || $basket_action==10){//Copy To/Add page/all || Move To page/all
			if($bid<0){
				list($selectQuery,$saved) = doQuery_latest2($ssid,false,'',false,-1,$dorelev,$doexpans);
				list($countQuery) = doQuery_latest2($ssid,true);
			}
			else{
				list($selectQuery,$saved) = doQuery_latest2(0, false, '', false, $bid);
				list($countQuery) = doQuery_latest2(0, true, '', false, $bid);
			}
			$selectQuery .= $orderby;
...
if($bid<0){
	...
	if(false && $_SESSION['sess_plevel']==2){
        ...
        list($countQuery) = doQuery_latest2($ssid,true,'',false,-1,false,false,$unapproved);   
        ...
        list($selectQuery,$saved) = doQuery_latest2($ssid,false,'',false,-1,$dorelev,$doexpans,$unapproved,false,-1,array(),0,((!empty($orderby)?$orderby:false)) ,((!empty($limit1)?$limit1:0)));
        
        //list($selectQuery,$saved) = doQuery($ssid,false,'',false,-1,$dorelev,$doexpans,$unapproved,false,-1,array(),0);
	...
	$displayKeywords = '';
        ...
        // NEW Comented query it will on if need
        //list($selectQuery,$saved) = doQueryNew(0, false, '', false, $bid,false,false,false,false,-1,array(),0,((!empty($orderby)?$orderby:false)),((!empty($limit1)?$limit1:0)));
        
                
}

/* below commented code move above in if-else*/
// OLD Query 
$selectQuery .= "$orderby LIMIT $limit1,$limit2"; 
//$selectQuery .= " LIMIT 0,$limit2"; 
...
<script src="includes/preview_latest.js?v=20131201" type="text/JavaScript"></script>
...
<?php
	...
	while($rs = $DRW->fetch_array($query)){
		...
		$isFICO = $rs['isFICO'];
                $isdigital=$rs['is_digital'];
                $isdigital_pan=$rs['panelist_sort'];
                $second_company=$rs['secondCompany'];
                $is_panelist_score=$rs['is_panelist_score'];
		if($productHeadline=='') {
			if($mid==5 || $mid==7){
...
?>
	...
	if($sample_javascript!=''){
        ...
        //$query3 = "SELECT is_digital FROM cscan_product_detail WHERE productID=$productID";
	...
	if($mid!=5 && $mid!=7){
		...
		if($document_size_byte>=0 && $mid!=10 && $mid!=9 && $mid!=5){
			?>
			<div style="float:left;padding:2px 0px 0px 14px;"><a class="bluelink" href="<?php echo 'productDocuments_latest.php?id='.$productID.$pdfsearchKeyword; ?>" onclick="pdfWin(<?php echo $productID; ?>); return false;" title="PDF Content"><img src="images/pdf.jpg" border="0" style="vertical-align:top;" /> <?php echo $DisplaySize; ?></a><br />
			<?php 
			if($_SESSION['sess_plevel']>0){
				?>
				<a class="bluelink" href="<?php echo 'productImages_latest.php?pp=1&amp;id='.$productID; ?>" target="_blank" title="PowerPoint Content"><img src="images/ppt.jpg" border="0" style="vertical-align:top;" /></a>
				<?php
			}
			if(!in_array('jpeg',$_SESSION['sess_search_exclude'])){
				?>
				<a class="bluelink" href="<?php echo 'productImages_latest.php?id='.$productID; ?>" target="_blank" title="JPEG Content"><img src="images/jpg.jpg" border="0" style="vertical-align:top;" /></a>
				<?php 
			}
                        $mpannelid_arr=@explode(',',$mpannelid);
			if($mid==3 AND (in_array(1,$mpannelid_arr) OR in_array(2,$mpannelid_arr))){
                            $query3 = "select productID,electronicID from cscan_product_detail where approved_date>='2020-12-01 00:00:00' AND  productID=$productID";   
                            $query_result3 = $DRW->query($query3,$DRW_read);
	...
	?>
        ...
        //if(ENV == 'localhost' || ENV == 'demo.competiscan.com'){
            if(isset($_SESSION['sess_anotation_tool_link']) AND in_array('anotation_tool_link',$_SESSION['sess_anotation_tool_link'])){ 
               if(($mid==3) AND ($rs['mPanelID']==1|| $rs['mPanelID']==2)) { 
               //if(($mid==3) AND ($rs['mPanelID']==1|| $rs['mPanelID']==2) AND ($rs['sectorID']==266) AND ($rs['categoryID']==269) ) {     
                   ...
                   $query_result_prod_email = $DRW->query($queryProdEmail,$DRW_read);
                    ...
                    if($numrows2>0){
                       ...
                       $url='https://vat.competiscan.com/v3/reviewed';
                       ...
                       $posteddata = json_encode($postdata);
                        ...
                        $responseData = json_decode($result, JSON_UNESCAPED_UNICODE);
                        /*echo "<pre>";
                        print_r($responseData);*/
                        $styleType='vertical-align:top;';
                        $imgSRC='settings-anotation.png';
                        if($curl_status_code==200){
                            if($responseData[0]['product_id']!='' AND $responseData[0]['reviewed']==1){
  ...
  /* ######  For digital media channel panelist ###### */
   ...
   /* ###### End For digital media channel panelist ###### */
    ...
    if($numrows_chk>0){
        if($isdigital_pan!='1' AND $is_panelist_score!='1'){
            echo '<br />';
        }
        ...
        <td class="bodytext" valign="top" style="border-bottom:none;">
            <?php            
                if(!empty($second_company)){?>
                 <strong>Additional Companies:</strong><br /><?php echo htmlspecialchars(str_replace(';',',',$second_company));
                }else{?>
                 <strong>Sector:</strong><br /><?php echo htmlspecialchars($sectorName);
                }
             ?>           
