    ...
    if ($searchbody || $searchsubj) {
        ...
        if ($numrow==0 && !empty($SPHINX_name)) {
            ...
            if (trim($ps)!='') {
                ...
                //$result['total_found'];
                //echo $result['total_found'].'total result';
...
if (count($muids)>0) {
    print "<div style=\"margin-top:4px;\" class=\"bodytext\">Assign Copies ID<input type=\"text\" name=\"copies_id\" id=\"copies_id\" size=\"10\" /> &nbsp; <input class=\"button\" type=\"submit\" name=\"update\" value=\"Update\" /></div>";
}
    ...
    if ($rowcnt>0) {
        ...
        if ($rowcnt>10) {
            ...
            for ($k=10;$k<=50;$k+=10) {
                if ($rowcnt>($k-10)) {
                    if ($limiter!=$k) {
                        $paging .= "<a href=\"{$link}page=0&amp;limshow=$k\" class=\"bluelink\">";
                    }
                    $paging .= $k;
                    if ($limiter!=$k) {
                        $paging .= "</a>";
