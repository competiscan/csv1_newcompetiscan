...
if ( ! empty( $slides ) ) {
	...
	$compass = home_url( '/competiscan-html/assets/compass/' );
	$deliverables = array(
		array(
			'img' => $compass . 'report1.png', 'h' => 'DME Report', 'sub' => 'Your creative execution, scored and explained.',
		...
		array(
			'img' => $compass . 'slide1.png', 'h' => 'DME Presentation', 'sub' => 'Your results, ready to present.',
		...
		array(
			'img' => $compass . 'report2.png', 'h' => 'CRE Report', 'sub' => 'Your offer vs. the real market.',
...
<section class="cs-x33" id="deliverables">
  <div class="cs-x41">
    <h2 class="cs-x42"><?php echo esc_html( $title ); ?></h2>
    <p class="cs-x43"><?php echo esc_html( $intro ); ?></p>
  </div>
  <div class="cs-x44">
    <button class="cs-x45" data-cs-crsl-prev aria-label="Previous deliverable"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M19 12H5"></path><path d="m12 19-7-7 7-7"></path></svg></button>
    <div class="cs-x46" data-cs-crsl>
      <?php foreach ( $deliverables as $di => $d ) : ?>
      <article class="<?php echo 0 === $di ? 'cs-x47' : 'cs-x59'; ?>" data-cs-crsl-slide aria-hidden="<?php echo 0 === $di ? 'false' : 'true'; ?>">
        <div class="cs-x48">
          <div class="cs-x49">
            <?php if ( ! empty( $d['img'] ) ) : ?><img class="cs-x50" src="<?php echo esc_url( $d['img'] ); ?>" alt="<?php echo esc_attr( $d['h'] ); ?>"><?php endif; ?>
          </div>
          <div class="cs-x51">
            <h3 class="cs-x52"><?php echo esc_html( $d['h'] ); ?></h3>
            <p class="cs-x53"><?php echo esc_html( $d['sub'] ); ?></p>
            <p class="cs-x54"><?php echo esc_html( $d['lead'] ); ?></p>
            <ul class="cs-x55">
              <?php foreach ( $d['b'] as $bullet ) : ?>
              <li class="cs-x56"><?php echo $check; // phpcs:ignore WordPress.Security.EscapeOutput ?><span><?php echo wp_kses_post( $bullet ); ?></span></li>
              <?php endforeach; ?>
            </ul>
          </div>
        </div>
    ...
    <button class="cs-x45" data-cs-crsl-next aria-label="Next deliverable"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14"></path><path d="m12 5 7 7-7 7"></path></svg></button>
  </div>
  <div class="cs-x60">
    <?php foreach ( $deliverables as $di => $d ) : ?>
    <button class="<?php echo 0 === $di ? 'cs-x61' : 'cs-x62'; ?>" data-cs-crsl-dot aria-label="Go to slide <?php echo (int) $di + 1; ?>"></button>
    <?php endforeach; ?>
  </div>
  <p class="cs-x63"><?php echo esc_html( $caption ); ?></p>
</section>
