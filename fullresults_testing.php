    ...
    if(isset($_POST['sendsave']) && trim($_POST['searchName']) != '') {
            ...
            if($numrow < 100 || $_SESSION['sess_plevel']==1 || $_SESSION['sess_plevel']==2) {
                    ...
                    if($searchName!=''){
                        ...
                        curl_setopt($chcsv2_save, CURLOPT_HTTPHEADER,array('Content-Type: application/json','User-Agent:'.$_SERVER['HTTP_USER_AGENT'], 'X-Forwarded-For:'.$_SERVER['REMOTE_ADDR']));
...
if($DRW->num_rows($resultCheck) > 0){
   ...
   if($dataSearchCheck['search_rules']!='' && $dataSearchCheck['search_rules']!='NULL'){
    ...
    if($dataSearchCheck['sender_domain_name']!='' && $dataSearchCheck['sender_domain_name']!='NULL'){
			$postdata['sender_domain'] = explode(' or ', str_replace('"', '', $dataSearchCheck['sender_domain_name']));
           // print_r($domains);
...
?>
        ...
        ?>
         <div style="padding:2px 0px 0px 14px;">&nbsp;<a class="bluelink" target="_blank" href="https://ai-analysis.competiscan.com/?productid=<?php echo $productID;?>&uid=<?php echo $_SESSION['sess_userID']; ?>"  title="Product Analysis">
  ...
  /* ######  For digital media channel panelist ###### */
   ...
   /* ######  F or FICO Vantage and Credit Score availability identification ###### */
