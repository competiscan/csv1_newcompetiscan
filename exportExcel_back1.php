...
if ($_SERVER['argc'] < 3) {
    print "exportExcel_back1.php sessionid serialized_array\n";
    exit;
}
...
if ($ssid > 0) {
    $sql = "SELECT mPanelID,mChannelID,sectorID,userID,delmethid_mult FROM cscan_search WHERE ID=$ssid";
    ...
    $delmethid = $data[4];
    @$DRW->free_result($rs);
    $mPanelIDArray = array_filter(explode(',', $mPanelID));
    $mChannelIDArray = array_filter(explode(',', $mChannelID));
    $sectorIDArray = array_filter(explode(',', $sectorID));
    $delmethidArray =array_filter(explode(' ,',$delmethid));
    ...
    while ($data = $DRW->fetch_row($rs)) {
        $as = array_filter(explode(',', $data[0]));
        foreach ($as as $a) {
            $a = (string) $a;
...
if ($ssid > 0 || $bid >= 0) {
    ...
    if ($search_panelist_date || $consumer_only || $do_bid) {
        ...
        if (!empty($state)) {
            $tmpArray = array_filter(explode(',', $state));
            $ppdatetext .= " (";
        ...
        foreach ($mult as $field => $val) {
            if ($val != '') {
                $tmpwhere = '';
                $tmpArray = array_filter(explode(',', $val));
                foreach ($tmpArray as $v) {
                    if ($v != '') {
                        if ($field == 'dmap.code') {
                            $tmpwhere .= " $field='" . $v . "' OR ";
        ...
        if ($search_competi_id != '') {
            $vs = array_filter(explode(',', $search_competi_id));
            $competi_ids = array();
            foreach ($vs as $v) {
    ...
    while ($row = $DRW->fetch_assoc($rs)) {
        ...
        if ($more > 0 || (($search_panelist_date || $consumer_only || $do_bid) && ($showheading['state'] || $showheading['gender'] || $showheading['age'] || $showheading['income'] || $showheading['DMA_ID']))) {
            ...
            /*$sql_P = "SELECT DISTINCT DATE_FORMAT(ppdate,'%Y-%m'),competi_id,invitationID,pp.panelist_id,pp.ppstateID,pp.pgender,pp.ppageID,pp.pincomeID,pp.pppostalcode,pp.ppmv,trackingID,pp.ppmv_m,dmap.code,pproductFICO
				FROM cscan_panelists_product pp LEFT JOIN cscan_dma_code_postalcode dmap ON (pp.pppostalcode=dmap.pppostalcode)$edcjoin, cscan_panelists cp WHERE pp.panelist_id=cp.panelist_id AND {$ppdatetext}productID=" . $row['theproductID'];*/
            //same panelist added then sum(ppmv)
...
//							}
                            if ($more == '1') {
                                if (($more > 2 || $more == 1) && !empty($ppstateID)) {
                                    ...
                                    if ($search_panelist_date || $consumer_only || $do_bid) {
                                        if ($state != '') {
                                            $tmp1 = array_filter(explode(',', $state));
                                            $tmp2 = array_filter(explode(',', $row['state']));
                                            foreach ($tmp2 as $k => $t) {
                                                if (!in_array($t, $tmp1)) {
                                                    unset($tmp2[$k]);
                                                }
...
//							}
                        ...
                        case 'agentCommunicationID':
                            if (in_array('citi', $sess_search_exclude)) {
                                $temp = array_filter(explode(',', $row['agentCommunicationID']));                                
                            ...
                            $row['agentCommunicationID']=ltrim($row['agentCommunicationID'],',') ;
                            $row['agentCommunicationID']=rtrim($row['agentCommunicationID'],',') ;
                        ...
                        case 'age':
                            if (($more > 2 || $more == 1) && !empty($ppageID)) {
                                ...
                                if ($search_panelist_date || $consumer_only || $do_bid) {
                                    if ($age != '') {
                                        $tmp1 = array_filter(explode(',', $age));
                                        $tmp2 = array_filter(explode(',', $row['age']));
                                        foreach ($tmp2 as $k => $t) {
                                            if (!in_array($t, $tmp1)) {
                                                unset($tmp2[$k]);
                                            }
                        ...
                        case 'income':
                            if (($more > 2 || $more == 1) && !empty($pincomeID)) {
                                ...
                                if ($search_panelist_date || $consumer_only || $do_bid) {
                                    if ($income_mult != '') {
                                        $tmp1 = array_filter(explode(',', $income_mult));
                                        $tmp2 = array_filter(explode(',', $row['incomeID']));
                                        foreach ($tmp2 as $k => $t) {
                                            if (!in_array($t, $tmp1)) {
                                                unset($tmp2[$k]);
                                            }
                        ...
                        case 'DMA_ID':
                            ...
                            if ($search_panelist_date || $consumer_only || $do_bid) {
                                if ($DMA_ID_mult != '') {
                                    $tmp1 = array_filter(explode(',', $DMA_ID_mult));
                                    $tmp2 = $dmaids;
                                    foreach ($tmp2 as $k => $t) {
                                        if (!in_array($t, $tmp1)) {
                                            unset($tmp2[$k]);
                        ...
                        case 'edc_id':
                            ...
                            if ($search_panelist_date || $consumer_only || $do_bid) {
                                if ($edc_id_mult != '') {
                                    $tmp1 = array_filter(explode(',', $edc_id_mult));
                                    $tmp2 = $edcids;
                                    foreach ($tmp2 as $k => $t) {
                                        if (!in_array($t, $tmp1)) {
                                            unset($tmp2[$k]);
                     ...
                     ############################## End Envelope/Postage Data Fields################
                        ...
                        case 'FeeProductType':
                            $tmp = array();
                            $FeeProductTypes = array_filter(explode(',', $row['FeeProductType']));
                        ...
                        case 'groupSize':
                            ...
                            $groupArray['0'] = 'N/A';
                            $gsizeArray = array_filter(explode(',', $row['groupSize']));
