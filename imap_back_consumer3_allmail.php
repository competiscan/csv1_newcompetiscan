...
/*
$result_cron_status = $DRW->query("SELECT status FROM cscan_consumer_cron_log limit 1", $DRW_read);
if ($DRW->num_rows($result_cron_status) > 0) {
    $data_cron_status = $DRW->fetch_row($result_cron_status);
    $cron_status = (int) $data_cron_status[0];
    if($cron_status==1){
        die('already running');
    }
}else{
    $query_ins = "insert into cscan_consumer_cron_log set status=0";
    $DRW->query($query_ins, $DRW_main);
}

$query = "update cscan_consumer_cron_log set start_date=now(),status=1";
...
$username1 = 'consumers3@sbkcenter.com';
$password1 = 'WX27d>y56';
...
foreach ($accountArray as $account) {
    ...
    if ($imap_stream === false) {
        ...
        //print_r($folders);
       echo $num_msg = imap_num_msg($imap_stream);
...
//$query_updt = "update cscan_consumer_cron_log set end_date=now(),status=0";
//$DRW->query($query_updt, $DRW_main);
