...
$sever1 = '{mail.sbkcenter.com:143/service=imap/novalidate-cert}';
...
$sever2 = '{mail.sbkcenter.com:143/service=imap/novalidate-cert}';
$mailbox2 = $sever2.'INBOX';
$username2 = 'username2';//producers.sbkcenter.com
$password2 = 'password2';
...
foreach($accountArray as $account){
	...
	if($imap_stream===false){
		...
		for($i=1;$i<=$num_msg;$i++){
			...
			//print_r($structure);
			if(isset($header->from)) $from = $header->from;
			else $from = '';
			if(isset($header->to)) $to = $header->to;
			else $to = '';
			if(isset($header->cc)) {
			...
			if(isset($header->udate)) $email_date = date('Y-m-d H:i:s',$header->udate);
			else $email_date = date('Y-m-d H:i:s');
			...
			if(isset($fromdbA[0])) $checkemail = strtolower(trim($fromdbA[0]));
			else $checkemail = '';
			...
			if($defaultcontact!='') $query .= " AND `contact_type_m_c`='".$DRW->real_escape_string($defaultcontact)."'";
			...
			if($count[0]==0) {
				...
				if($keep){
					...
					$query = "INSERT INTO `cscan_email` (e_assigned_admin_userID,`mailbox_uid`,`email_date`,`email_to`,`email_from`,`email_from_one`,`email_subject`,`contact_type_m_c`,`email_cc`,`email_bcc`,`deleted`,`panelist_id`,`email_stateID`) 
					...
					foreach($part_array as $parti=>$party){
				    	if(isset($party['ifid']) && $party['ifid']) $id = trim($party['id']);
				    	else $id = '';
				    	if(isset($party['type'])) $type_no = $party['type'];
						else $type_no = 7;
				    	if(isset($types[$type_no])) $type = $types[$type_no];
						else $type = '';
						if(isset($party['subtype'])) $subtype = strtolower($party['subtype']);
						else $subtype = '';
						if(isset($party['disposition'])) $disposition = strtolower($party['disposition']);
						else $disposition = '';
				    	if(isset($party['encoding'])) $encoding = $party['encoding'];
						else $encoding = 5;
				    	if(isset($party['bytes'])) $bytes = (int)$party['bytes'];
						else $bytes = 0;
						//description,lines
						...
						$charset = '';
						$paramArray = array();
						if(isset($party['dparameters'])) $paramArray[] = $party['dparameters'];
						if(isset($party['parameters'])) $paramArray[] = $party['parameters'];
							...
							foreach($param_o as $parameter){
								if($parameter->value!=''){
									...
									if(strpos($pname,'name')!==false){
										if($filename=='') $filename = trim($parameter->value);
										break;
									}
									...
									elseif(strpos($pname,'boundary')!==false){
										if($in_RFC822 && $RFC822_part2=='') $RFC822_part2 = $parti;
										continue 3;
									}
									else{
						...
						if($filename=='' && $tmpfilename!='') $filename = $tmpfilename;
						...
						if($parti!='') {
							...
							if($bodydata!=''){
								if($type_no==0 && ($subtype=='plain' || $subtype=='html')){
									if($encoding==3) $bodydata = base64_decode($bodydata);
									elseif($encoding==4) $bodydata = quoted_printable_decode($bodydata);
									
									if($charset=='utf-8') {
										$bodydata = utf8_decode($bodydata); //imap_utf8($bodydata)?
										//$is_UTF8 = true;
									}
								...
								elseif(($type_no!=1 && $type_no!=2) || $subtype=='rfc822'){
										...
										if(file_put_contents($tmpdatname,emailDecode($bodydata,$encoding))!==false){
											...
											foreach($tnefs as $k=>$tnef){
												if(is_file($tmpdirname.'/'.$tnef)){
													...
													if(isset($filetypeArray[0])) $type2 = $filetypeArray[0];
													else $type2 = '';
													if(isset($filetypeArray[1])) $subtype2 = $filetypeArray[1];
													else $subtype2 = '';
									...
									foreach($efilesArray as $efile){
										...
										$query = "INSERT INTO `cscan_email_file` (`cefpart`,`cefname`,`ceftype`,`muid`,`cefidentification`,`cefdisposition`,`cefencoding`) 
										...
										if($fp){
											...
											$query = "UPDATE `cscan_email_file` SET cefpath='".$DRW->real_escape_string('/contentFiles/'.$yearpath.$monthpath.$daypath.$cefid)."' WHERE cefid=$cefid";
											$DRW->query($query,$DRW_main);
										}
									}
								}
	...
	if(is_string($obj)){
		...
		foreach ($obj as $id => $object) {
			if(isset($object->mailbox) && isset($object->host)){
				$out = '';
				if(isset($object->personal)) $out .= '"'.str_replace( '"', '',$object->personal).'" <';
				$email = trim($object->mailbox . "@" . $object->host);
				$outArray[] = $email;
				$out .= $email;
				if(isset($object->personal)) $out .= '>';
				$outfullArray[] = $out;
			}
		}
	}
...
function create_part_array(&$part_array=array(),$structure, $parttext='') {
	...
	if(count($tmpArray)>0){
		if($next_obj===false && count($part_array)==0) $parttext = '1';
		$part_array[$parttext] = $tmpArray;
	}
	if($next_obj!==false){
		if($parttext!='') $structurepast = $parttext.'.';
		else $structurepast = '';
		$partno = 1;
