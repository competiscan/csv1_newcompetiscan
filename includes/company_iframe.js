...
function showCoSel(showall){
	...
	if(showall==1 || wholeval.length>0 || wholeval2.length>0){
		...
		if(parent_object_field=='productName'){
			var parent_text_objc = getParentObj('company');
			if(parent_text_objc){
		...
		processajax('company_list.php', true, 'POST', 'companytext='+encodeURIComponent(wholeval)+'&companysel='+encodeURIComponent(wholeval2)+'&parent_field='+encodeURIComponent(parent_object_field)+satext, document.getElementById('seltext'), 'doResponseCoSel');	
	}
	else{
		hideCoSel();
	}
