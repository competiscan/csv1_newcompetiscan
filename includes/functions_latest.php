...
function doQuery_latest($search_id, $docount = false, $addedtodatabaseover = '', $dograph = false, $bid = -1, $relev = false, $expans = false, $unapproved = false, $clear_ps = false, $search_panelist_date_over = -1, $search_values = array(), $sess_userID = 0) {
    ...
    if ($bid < 0) {
        ...
        if ($productName != '') {
            ...
            if (count($keyArray) > 1 || preg_match('/^"([^"]+)"$/', $productName)) {
                ...
                $searchstr =   array('"',"'");
                $repstr =   array('\"',"\'");
                $implodedata    =   "'".implode("','",str_replace($searchstr,$repstr,$keyArray))."'";
                 $multExactArray['productName'] = $implodedata;
...
//                            }
       ...
       // print_r($partsArray);
        if (count($partsArray) > 0) {
            ...
            $wherecondition .= 'in(' . implode(' , ', $partsArray) . ')  ';
        }
        $wherecondition=$fldnames.$wherecondition;
       ...
       // echo $sectorID.'cat='. $categoryID.'subcat='. $subCategoryID.'subsubcat='.$subSubCategoryID;
      ...
      //  print_r($partsArray2);exit;
        if (count($partsArray2) > 0) {
            ################## end for the new condition ###########################
            /*$newwhere='';
            $fldarray    =array();
            foreach($partsArray2 as $keypart=>$keyval){
                $replacestring=array("(",")");
                $replaceval     =   array("","");
                $alldataarray[] =   $keyval;
                
                if(strstr($keyval," AND ")!=false){
                    $newandarray[]    =   $keyval;
                }
           ...
           $newtempwhere    =   '';
           $fvalorcon       =   '';
            $newarray=  array_diff($alldataarray, $newandarray);
            print_r($newarray);
            for($n=0;$n<count($newandarray);$n++){
                 $keyArray=  explode("=",str_replace($replacestring,$replaceval,$newarray[$n]));
                
                $fldarray[$keyArray[0]][] =   $keyArray[1];
            }
            foreach($fldarray as $fkey=>$fval){
               $newtempwhere .= $fkey." in (".implode(",",$fval).")  ";
               
              
           }
           
           if(!empty($newandarray)){
               
              
               $newwhere=   implode(" OR ",$newandarray);
               //$where .= "  ( ".$newtempwhere. " OR ".$newwhere." ) ) )  ";
           }else{
               //$where .=$newtempwhere;
           }
            */
    ...
    $selectQuery .= " FROM cscan_product_detail pd $ignoreindex$ocrtext$pjoin$cjoin$ccjoin$afjoin$affjoin$dmajoin$edcjoin$sect_j$pcjoin$bjoin$ejoin$mljoin$rjoin$tljoin$ajoin$sjoin$ojoin $where$owhere $sortby";
    //exit;
    if ($dograph != 0) {
