...
function showCoSel(showall){
	...
	if(parent_text_obj){
		...
		processajax('digital_lookup_company_list.php', true, 'POST', 'companytext='+encodeURIComponent(wholeval)+'&companysel='+encodeURIComponent(wholeval2)+'&parent_field='+encodeURIComponent(parent_object_field)+satext, document.getElementById('seltext'), 'doResponseCoSel');	
	}
	else{
              hideCoSel();
	}
