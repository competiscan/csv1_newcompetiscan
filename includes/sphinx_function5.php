...
function doQuerytestsphinx2($search_id, $docount = false, $addedtodatabaseover = '', $dograph = false, $bid = -1, $relev = false, $expans = false, $unapproved = false, $clear_ps = false, $search_panelist_date_over = -1, $search_values = array(), $sess_userID = 0,$sql_P2='',$more='',$sql_P3='',$consumer) {
...
/* ##########  End for digital Module  ##########  */
        ...
        $panelistArray = array('ppageID' => $age, 'pincomeID' => $income_mult, 'isBiz' => $isBiz, 'dmaps.code' => $DMA_ID_mult, 'ppdate' => $ppdate, 'ppdate_month' => $ppdate_month, 'ppstateID' => $ppstateID, 'pgender' => $pgender, 'Income_Producing_Assets_Segment_Code' => $Income_Producing_Assets_Segment_Code, 'edc_id' => $edc_id_mult,'ValueScore'=>$value_score);
        if ($productName != '') {
        ...
        foreach ($panelistArray as $field => $val) {
            if ($val != '') {
                ...
                if ($field == 'Income_Producing_Assets_Segment_Code' || $field == 'ValueScore') {
                   ...
                   $dmajoin = ' JOIN cscan_dma_code_postalcode dmaps ON (pp.pppostalcode=dmaps.pppostalcode)';
                }
...
function doQuerytestsphinx2_20170620($search_id, $docount = false, $addedtodatabaseover = '', $dograph = false, $bid = -1, $relev = false, $expans = false, $unapproved = false, $clear_ps = false, $search_panelist_date_over = -1, $search_values = array(), $sess_userID = 0,$sql_P2='',$more='',$sql_P3='',$consumer) {
    global $DRW, $DRW_read, $DRW_main, $DRW_crm, $SPHINX_name;
    $ocrtext = '';
    $where = '';
    $sortby = '';
...
function doQuerytestsphinx2_20170608($search_id, $docount = false, $addedtodatabaseover = '', $dograph = false, $bid = -1, $relev = false, $expans = false, $unapproved = false, $clear_ps = false, $search_panelist_date_over = -1, $search_values = array(), $sess_userID = 0,$sql_P2='',$more='',$sql_P3='',$consumer) {
    global $DRW, $DRW_read, $DRW_main, $DRW_crm, $SPHINX_name;
    $ocrtext = '';
    $where = '';
    $sortby = '';
