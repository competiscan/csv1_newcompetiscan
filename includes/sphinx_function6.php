...
function doQuerytestsphinx3($search_id, $docount = false, $addedtodatabaseover = '', $dograph = false, $bid = -1, $relev = false, $expans = false, $unapproved = false, $clear_ps = false, $search_panelist_date_over = -1, $search_values = array(), $sess_userID = 0,$sql_P2='',$more='',$sql_P3='',$consumer) {
    ...
    if ($bid < 0) {
        if (count($search_values) > 0) {
            ...
            $savedQ = "SELECT searchKey,searchType,searchOption,mChannelID,sectorID,mPanelID,
                        ...
                        spanelist_filter,edc_id_mult,AffinitySubCategoryID_mult,ERateType_mult,EOfferPrice_mult,ETermLength_mult,is_ECancelFee,IssueTypeID_mult,pcountry,is_Reloadable,creditUnion,is_mobile,value_score
...
//        }
        ...
        if ($searchKey != '' || $searchKey2 != '') {
            if ($searchType == 'ocr') {
                ...
                $matchagainst = 'MATCH(productHeadline,entryID) AGAINST'; 
                $matchtext = "$matchagainst ('" . parseBool($searchKey) . "' IN BOOLEAN MODE)";
                if ($relev) {
    ...
    pd.is_mobile='1' THEN 'Desktop' WHEN pd.is_mobile='2' THEN 'Mobile' ELSE '' END ) AS 'Digital Source',IF(pd.simple_domain IS NULL or pd.simple_domain = '', '',pd.simple_domain) as 'Simple Domain'";  
   ...
   $selectQuery .= " ,IF(cscount.country IS NULL or cscount.country = '', cscount3.country, cscount.country) as 'Country'";
     ...
     if ($consumer) { 
		...
		(SELECT round(trim(SUM(pp2.ppmv)),2) FROM cscan_panelists_product pp2 WHERE pp2.productID=pd.productID)
			...
			// $sqlP2 = "SELECT round(sum(pp.ppmv)) FROM cscan_panelists_product pp LEFT JOIN cscan_dma_code_postalcode dmap ON (pp.pppostalcode=dmap.pppostalcode)$edcjoin, cscan_panelists cp WHERE pp.panelist_id=cp.panelist_id AND productID=pd.productID";

			// $sqlP3="SELECT round(SUM(ppmv)) as 'Estimated Mail Volume' FROM cscan_panelists_product pp WHERE pp.productID=pd.productID";


			// $query2 = "SELECT document_size_byte FROM cscan_document WHERE productID=pd.productID AND document_id=1"; 
		}  
  ...
  ################## For 'Insurance' & 'Investments/Annuities' Sector ##################
   ...
   ################## End For 'Insurance' & 'Investments/Annuities' Sector ##################		   
    ...
    pd.groupSize='0' THEN 'N/A' WHEN pd.groupSize='1' THEN '2-50' WHEN pd.groupSize='2' THEN '51-99' WHEN pd.groupSize='3' THEN '100-499' WHEN pd.groupSize='4' THEN '500 +' ELSE 'N/A' END ) AS 'Group Size',";
      ...
      //$selectQuery .= "CONCAT('','PDF Content') as 'PDF Content',";
      $selectQuery .= "CONCAT('http://www.competiscan.com/productDocuments.php?did=1&id=',pd.productID) as 'PDF Content',";
      

      // pd.agentCommunicationID
       //pd.addedToDatabase,
      ...
      //$creditunionjoin .= " LEFT JOIN cscan_company pa2,cscan_company_product pp2 on (pa2.companyID=pp2.companyID AND pp2.productID=pd.productID AND isCreditUnion=1)";
     ...
     ################## For 'Online Display Advertising' & 'Mobile' Media ##################
    ...
    ################## End For 'Mobile' & 'Social Media' Media ##################  
   ...
   if(in_array('87',$sectrarr) || in_array('90',$sectrarr)){
	/*	$mintel2 = new \HS\Mintel();
        $mintel_sets = $mintel2->getFields();
        $mintel_sets_2 = $mintel2->getFieldSet('incentive_set_2');
        $mintel_sets_3 = $mintel2->getFieldSet('incentive_set_3');

        $incentive_set = implode(',pd.', array_merge(array_keys($mintel_sets), array_keys($mintel_sets_2), array_keys($mintel_sets_3)));
        //$selectQuery .=",pd.$incentive_set";      
       */
...
//$sortby
    if ($dograph != 0) {
        ...
        //$selectQuery .=" GROUP BY pd.productID";
...
function doQuerytestsphinx3_20170620($search_id, $docount = false, $addedtodatabaseover = '', $dograph = false, $bid = -1, $relev = false, $expans = false, $unapproved = false, $clear_ps = false, $search_panelist_date_over = -1, $search_values = array(), $sess_userID = 0,$sql_P2='',$more='',$sql_P3='',$consumer) {
    global $DRW, $DRW_read, $DRW_main, $DRW_crm, $SPHINX_name;
    $ocrtext = '';
    $where = '';
    $sortby = '';
    ...
    if ($bid < 0) {
        if (count($search_values) > 0) {
            ...
            $savedQ = "SELECT searchKey,searchType,searchOption,mChannelID,sectorID,mPanelID,
                        ...
                        search_panelist_date,is_affinion,is_military,search_competi_id,ApplicationType_mult,is_multicultural,search_rules,IntroPricing_mult,is_rewards,RewardsProgramEmphasis_mult,is_incentive,responseMechID_mult,
                        multiculturalmarkets_mult,CardNetwork_mult,FeeProduct,external_link,FeeProductType,approved_date_to,ca_related,is_mover,scsc_primary,OptOutFirmOffer,searchKey2,search_type_and,riders_mult,is_hphsa,subSubCategoryID,Income_Producing_Assets_Segment_Code_mult,cg_id,is_citi,is_CreditCardMentioned,
                        spanelist_filter,edc_id_mult,AffinitySubCategoryID_mult,ERateType_mult,EOfferPrice_mult,ETermLength_mult,is_ECancelFee,IssueTypeID_mult,pcountry,is_Reloadable,creditUnion,is_mobile
                        FROM cscan_search WHERE ID='" . $search_id . "'";
            $rs = $DRW->query($savedQ, $DRW_read);
            $data = $DRW->fetch_row($rs);
            $searchKey = trim($data[0]);
...
function doQuerytestsphinx3_20170615($search_id, $docount = false, $addedtodatabaseover = '', $dograph = false, $bid = -1, $relev = false, $expans = false, $unapproved = false, $clear_ps = false, $search_panelist_date_over = -1, $search_values = array(), $sess_userID = 0,$sql_P2='',$more='',$sql_P3='',$consumer) {
    global $DRW, $DRW_read, $DRW_main, $DRW_crm, $SPHINX_name;
    $ocrtext = '';
    $where = '';
    $sortby = '';
    ...
    if ($docount) {
     ...
     $primarysubsubcatjoin= " LEFT JOIN cscan_sector cspssc on (cspssc.sectorID=(SELECT scsc_subSubCategoryID FROM cscan_scsc_product WHERE productID=pd.productID AND scsc_sort=1)) ";
      ...
      $affcatjoin=" LEFT JOIN cscan_affinity_category csafc on csafc.AffinityCategoryID=(
		...
		IF(csahorc.description IS NULL or csahorc.description = '', '', csahorc.description) as 'Advantage Home  Owner / Renter Code',IF(cscwc.Advantage_Home_Owner_Renter_Level IS NULL or cscwc.Advantage_Home_Owner_Renter_Level = '', '', cscwc.Advantage_Home_Owner_Renter_Level) as 'Advantage Home Owner / Renter Level',IF(cscwc.DMA_CODE IS NULL or cscwc.DMA_CODE = '', '', cscwc.DMA_CODE) as 'DMA CODE',
		IF(csdmac.description IS NULL or csdmac.description = '', '', csdmac.description) as 'DMA Name',IF(cscwc.Gender_code IS NULL or cscwc.Gender_code = '', '', cscwc.Gender_code) as 'Gender code',IF(cscwc.Occupation_code IS NULL or cscwc.Occupation_code = '', '', cscwc.Occupation_code) as 'Occupation code',IF(cscwc.MSA_CODE IS NULL or cscwc.MSA_CODE = '', '', cscwc.MSA_CODE) as 'MSA CODE',IF(cscwc.inq_win_past_6_mnths_except_promo_and_eval IS NULL or cscwc.inq_win_past_6_mnths_except_promo_and_eval = '', '', cscwc.inq_win_past_6_mnths_except_promo_and_eval) AS '# inq w/in past 6 mnths except promo and eval',IF(cscwc.Age_of_oldest_account_months IS NULL or cscwc.Age_of_oldest_account_months = '', '', cscwc.Age_of_oldest_account_months) AS 'Age of oldest account (months)',IF(cscwc.Age_of_newest_account_months IS NULL or cscwc.Age_of_newest_account_months = '', '', cscwc.Age_of_newest_account_months) AS 'Age of newest account (months)',
...
function doQuerytestsphinx3_20170608($search_id, $docount = false, $addedtodatabaseover = '', $dograph = false, $bid = -1, $relev = false, $expans = false, $unapproved = false, $clear_ps = false, $search_panelist_date_over = -1, $search_values = array(), $sess_userID = 0,$sql_P2='',$more='',$sql_P3='',$consumer) {
    global $DRW, $DRW_read, $DRW_main, $DRW_crm, $SPHINX_name;
    $ocrtext = '';
    $where = '';
    $sortby = '';
...
//        $where .= ") AND ";
    ...
    if ($docount) {
        ...
        $primarysectorjoin= " LEFT JOIN cscan_sector csps on (csps.sectorID=(SELECT scsc_sectorID FROM cscan_scsc_product WHERE productID=pd.productID AND scsc_sort=1)) "; 
    //    $primarysectorjoin= " LEFT JOIN cscan_sector csps find_in_set(csps.sectorID=pd.sectorID) ";   
      //((CONCAT(',',sectorID,',') REGEXP ',372,'))
      

      // $catjoin= " LEFT JOIN cscan_sector cc on (cc.sectorID=pd.categoryID) ";
      $primarycatjoin= " LEFT JOIN cscan_sector cspc on (cspc.sectorID=(SELECT scsc_categoryID FROM cscan_scsc_product WHERE productID=pd.productID AND scsc_sort=1)) ";
      
     // $primarycatjoin= " LEFT JOIN cscan_sector cspc find_in_set(cspc.sectorID=pd.categoryID) ";   

      //$subcatjoin= " LEFT JOIN cscan_sector csc on (csc.sectorID=pd.subCategoryID) ";
      $primarysubcatjoin= " LEFT JOIN cscan_sector cspsc on (cspsc.sectorID=(SELECT scsc_subCategoryID FROM cscan_scsc_product WHERE productID=pd.productID AND scsc_sort=1)) ";
      
     // $primarysubcatjoin= " LEFT JOIN cscan_sector cspsc find_in_set(cspsc.sectorID=pd.subCategoryID) "; 

      //$subsubcatjoin=" LEFT JOIN cscan_sector cssc on (cssc.sectorID=pd.subSubCategoryID)";
     $primarysubsubcatjoin= " LEFT JOIN cscan_sector cspssc on (cspssc.sectorID=(SELECT scsc_subSubCategoryID FROM cscan_scsc_product WHERE productID=pd.productID AND scsc_sort=1)) ";
      //cssc.subSubCategoryID as Sub Sub Category
      
     //$primarysubsubcatjoin= " LEFT JOIN cscan_sector cspssc find_in_set(cspssc.sectorID=pd.subSubCategoryID) ";
      ...
      $agejoin=" LEFT JOIN cscan_age_product cap on (cap.age_pID=pd.age) ";
      $incomejoin=" LEFT JOIN cscan_incometype cit on (cit.incomeID=pd.incomeID) ";
      $mtypejoin="  LEFT JOIN cscan_mtype cmt on (cmt.mTypeID=pd.mTypeID) ";
      ...
      $affcatjoin=" LEFT JOIN cscan_affinity_category csafc on csafc.AffinityCategoryID=(
        ...
        */
        ...
        //SELECT AffinityCategoryName FROM cscan_affinity_category WHERE AffinityCategoryID
		...
		$ethinicityjoin="  LEFT JOIN cscan_ethnicity_code csethc on (csethc.code=cspsp_dt.ET_ETHNICITY) ";
		$religionjoin="  LEFT JOIN cscan_religion_code csrlc on (csrlc.code=cspsp_dt.ET_RELIGION) ";
		$languagejoin="  LEFT JOIN cscan_language_code cslnc on (cslnc.code=cspsp_dt.ET_LANGUAGE) ";
		$groupjoin="  LEFT JOIN cscan_group_code csgrpc on (csgrpc.code=cspsp_dt.ET_GROUP) ";
		$countrycjoin="  LEFT JOIN cscan_country_code cscntc on (cscntc.code=cspsp_dt.ET_COUNTRY) ";
		...
		$dmacodejoin="  LEFT JOIN cscan_dma_code csdmac on (csdmac.code=cscwc.DMA_CODE) ";
		$genderjoin="  LEFT JOIN cscan_gender_code csgender on (csgender.code=cscwc.Gender_code) ";     
		...
		IF(cslnc.description IS NULL or cslnc.description = '', '', cslnc.description) as 'LANGUAGE_DESC'
		,IF(csgrpc.description IS NULL or csgrpc.description = '', '', csgrpc.description) as 'GROUP_DESC',IF(cscntc.description IS NULL or cscntc.description = '', '', cscntc.description) as 'COUNTRY_DESC',IF(csassc.description IS NULL or csassc.description = '' or csassc.description = 'UNKNOWN', '', csassc.description) as 'ASSIMILATION_DESC',IF(cscwc.ValueScore_for_Household IS NULL or cscwc.ValueScore_for_Household = '', '', cscwc.ValueScore_for_Household) as 'ValueScore for Household Code',IF(csvsfhh.description IS NULL or csvsfhh.description = '', '', csvsfhh.description) as 'ValueScore for Household',
		
		IF(cscwc.HH_Income_Index IS NULL or cscwc.HH_Income_Index = '', '', cscwc.HH_Income_Index) as 'HH Income Index',IF(cscwc.Birth_date_of_person_for_first_person_in_household IS NULL or cscwc.Birth_date_of_person_for_first_person_in_household = '', '', cscwc.Birth_date_of_person_for_first_person_in_household) as 'Birth date of person for first person in household',IF(csipasc.description IS NULL or csipasc.description = '', '', csipasc.description) as 'Income Producing Assets Segment Code  *R*',IF(cscwc.Household_Income_Identifier_Narrow_Band IS NULL or cscwc.Household_Income_Identifier_Narrow_Band = '', '', cscwc.Household_Income_Identifier_Narrow_Band) as 'Household Income Identifier Narrow Band Code',IF(cshiinb.description IS NULL or cshiinb.description = '', '', cshiinb.description) as 'Household Income Identifier Narrow Band',
		
		IF(csahorc.description IS NULL or csahorc.description = '', '', csahorc.description) as 'Advantage Home  Owner / Renter Code',IF(cscwc.Advantage_Home_Owner_Renter_Level IS NULL or cscwc.Advantage_Home_Owner_Renter_Level = '', '', cscwc.Advantage_Home_Owner_Renter_Level) as 'Advantage Home Owner / Renter Level',IF(cscwc.DMA_CODE IS NULL or cscwc.DMA_CODE = '', '', cscwc.DMA_CODE) as 'DMA CODE',
		IF(csdmac.description IS NULL or csdmac.description = '', '', csdmac.description) as 'DMA Name',IF(csgender.description IS NULL or csgender.description = '', '', csgender.description) as 'Gender code',IF(cscwc.Occupation_code IS NULL or cscwc.Occupation_code = '', '', cscwc.Occupation_code) as 'Occupation code',IF(cscwc.MSA_CODE IS NULL or cscwc.MSA_CODE = '', '', cscwc.MSA_CODE) as 'MSA CODE',IF(cscwc.inq_win_past_6_mnths_except_promo_and_eval IS NULL or cscwc.inq_win_past_6_mnths_except_promo_and_eval = '', '', cscwc.inq_win_past_6_mnths_except_promo_and_eval) AS '# inq w/in past 6 mnths except promo and eval',IF(cscwc.Age_of_oldest_account_months IS NULL or cscwc.Age_of_oldest_account_months = '', '', cscwc.Age_of_oldest_account_months) AS 'Age of oldest account (months)',IF(cscwc.Age_of_newest_account_months IS NULL or cscwc.Age_of_newest_account_months = '', '', cscwc.Age_of_newest_account_months) AS 'Age of newest account (months)',
		...
		/* End For Additional Details field */
      
		//$selectQuery .= " IF((SELECT GROUP_CONCAT(CS2.sectorName) FROM cscan_sector CS2 WHERE  find_in_set(CS2.sectorID,TRIM(pd.sectorID))) IS NULL, '',(SELECT GROUP_CONCAT(CS2.sectorName) FROM cscan_sector CS2 WHERE find_in_set(CS2.sectorID,TRIM(pd.sectorID))))  AS 'Sector Name2',";
		
		//$selectQuery .= " IF((SELECT GROUP_CONCAT(CS5.sectorName) FROM cscan_sector CS5 WHERE  find_in_set(CS5.sectorID,TRIM(pd.subSubCategoryID))) IS NULL, 'Not mentioned',(SELECT GROUP_CONCAT(CS4.sectorName) FROM cscan_sector CS4 WHERE find_in_set(CS5.sectorID,TRIM(pd.subSubCategoryID))))  AS 'Sub Sub Category',";
		
		//IF(cssc.sectorName IS NULL or cssc.sectorName = '', 'Not mentioned', cssc.sectorName) as 'Sub Sub Category'
		//$subsubcatjoin=" LEFT JOIN cscan_sector cssc on (cssc.sectorID=pd.subSubCategoryID)";
    ...
    //print_r($mediaarr); 	
      ...
      $selectQuery .= " pd.company as Company,pd.secondCompany as 'Second Company',IF((SELECT GROUP_CONCAT(CS2.sectorName) FROM cscan_sector CS2 WHERE  find_in_set(CS2.sectorID,TRIM(pd.sectorID))) IS NULL, '',(SELECT GROUP_CONCAT(CS2.sectorName) FROM cscan_sector CS2 WHERE find_in_set(CS2.sectorID,TRIM(pd.sectorID))))  AS 'Sector',csps.sectorName as 'Primary Sector',IF((SELECT GROUP_CONCAT(CS3.sectorName) FROM cscan_sector CS3 WHERE  find_in_set(CS3.sectorID,TRIM(pd.categoryID))) IS NULL, '',(SELECT GROUP_CONCAT(CS3.sectorName) FROM cscan_sector CS3 WHERE find_in_set(CS3.sectorID,TRIM(pd.categoryID))))  AS 'Category',cspc.sectorName as 'Primary Category',IF((SELECT GROUP_CONCAT(CS4.sectorName) FROM cscan_sector CS4 WHERE  find_in_set(CS4.sectorID,TRIM(pd.subCategoryID))) IS NULL, 'Not mentioned',(SELECT GROUP_CONCAT(CS4.sectorName) FROM cscan_sector CS4 WHERE find_in_set(CS4.sectorID,TRIM(pd.subCategoryID))))  AS 'Sub Category',IF(cspsc.sectorName IS NULL or cspsc.sectorName = '', '', cspsc.sectorName) as 'Primary Sub Category',IF((SELECT GROUP_CONCAT(CS5.sectorName) FROM cscan_sector CS5 WHERE  find_in_set(CS5.sectorID,TRIM(pd.subSubCategoryID))) IS NULL, 'Not mentioned',(SELECT GROUP_CONCAT(CS5.sectorName) FROM cscan_sector CS5 WHERE find_in_set(CS5.sectorID,TRIM(pd.subSubCategoryID))))  AS 'Sub Sub Category',IF(cspssc.sectorName IS NULL or cspssc.sectorName = '', '', cspssc.sectorName) as 'Primary Sub Sub Category',pd.entryID,CONCAT('http://www.competiscan.com/index.php?product=',pd.productID) as 'EntryID Link',pd.productHeadline as Headline, IF(cacom.type IS NULL or cacom.type = '', '',cacom.type) as 'Communications Type',cmc.mChannelName as MediaChannel, cmp.mPanelName as Audience,IF(cst.stateName IS NULL or cst.stateName = '', '', cst.stateName) as 'State/Province',IF(cscount.country IS NULL or cscount.country = '', cscount3.country, cscount.country) as 'Country'";
      
      
      //csps.sectorName as 'Primary Sector'
      //cspc.sectorName as 'Primary Category'
      //IF(cspsc.sectorName IS NULL or cspsc.sectorName = '', '', cspsc.sectorName) as 'Primary Sub Category',
      //IF(cspssc.sectorName IS NULL or cspssc.sectorName = '', '', cspssc.sectorName) as 'Primary Sub Sub Category',
  ...
  ################## End For Energy Sector  ######################
      ...
      $selectQuery .= ", IF(csafc.AffinityCategoryName IS NULL or csafc.AffinityCategoryName = '', '', csafc.AffinityCategoryName) as 'Affinity/Association Category', IF(csafsc.AffinityCategoryName IS NULL or csafsc.AffinityCategoryName = '', '', csafsc.AffinityCategoryName) as 'Affinity/Association Sub Category',pd.firstSeen,pd.lastSeen,pd.productName as 'Product Name',(csdoc.document_size_byte/1024) as 'File Size (KB)',cdm.delmethname as 'Delivery Method', IF(pd.incentive IS NULL or pd.incentive = '', 'N/A', pd.incentive) as 'Sign-on Incentive',IF(pd.incentive_ongoing IS NULL or pd.incentive_ongoing = '', 'N/A', pd.incentive_ongoing) as 'Ongoing Incentive',IF((SELECT GROUP_CONCAT(crm.responseMechName) FROM cscan_response_mechanism crm WHERE  find_in_set(crm.responseMechID,TRIM(pd.responseMechID))) IS NULL, '',(SELECT GROUP_CONCAT(crm.responseMechName) FROM cscan_response_mechanism crm WHERE find_in_set(crm.responseMechID,TRIM(pd.responseMechID))))  AS 'Response Mechanism',";
  ...
  ################## For 'Insurance' & 'Investments/Annuities' Sector ##################
   ...
   ################## End For 'Insurance' & 'Investments/Annuities' Sector ##################		   
      ...
      // pd.agentCommunicationID
       //pd.addedToDatabase,   
      $selectQuery .= " IF(is_prescreen = 1, 'Yes', '') AS 'Pre-Screen',IF(pd.OfferExpiryDate='0000-00-00', '',pd.OfferExpiryDate) AS 'Offer Expiry Date'"; 
   ...
   ################ For Banking Sector ###################
    ...
    if(in_array('87',$sectrarr) ){    
        ...
        $selectQuery .=",IF(csbank.MinimumDeposit IS NULL or csbank.MinimumDeposit = '', '', csbank.MinimumDeposit) as 'Banking - Minimum Deposit ($)',IF(csbank.FreeChecking IS NULL or csbank.FreeChecking = '0', '', 'Yes') as 'Banking - Free Checking',IF(csbank.Checking_APR IS NULL or csbank.Checking_APR = '0', '', csbank.Checking_APR) as 'Banking - Checking APR (%)',IF(csbank.Checking_APY IS NULL or csbank.Checking_APY = '0', '', csbank.Checking_APY) as 'Banking - Checking APY (%)',IF(csbank.Savings_APR IS NULL or csbank.Savings_APR = '0', '', csbank.Savings_APR) as 'Banking - Savings APR (%)',IF(csbank.Savings_APY IS NULL or csbank.Savings_APY = '0', '', csbank.Savings_APY) as 'Banking - Savings APY (%)',IF(csbank.MoneyMarket_APR IS NULL or csbank.MoneyMarket_APR = '0', '', csbank.MoneyMarket_APR) as 'Banking - Money Market APR (%)',IF(csbank.MoneyMarket_APY IS NULL or csbank.MoneyMarket_APY = '0', '', csbank.MoneyMarket_APY) as 'Banking - Money Market APY (%)',IF(csbank.CD_APR IS NULL or csbank.CD_APR = '0', '', csbank.CD_APR) as 'Banking - C/D APR (%)',IF(csbank.CD_APY IS NULL or csbank.CD_APY = '0', '', csbank.CD_APY) as 'Banking - C/D APY (%)',IF(csbank.DebitCardMentioned IS NULL or csbank.DebitCardMentioned = '0', 'No', 'Yes') as 'Banking - Debit Card Mentioned',IF(csbank.DebitCardMentioned IS NULL or csbank.DebitCardMentioned = '0', 'No', 'Yes') as 'Banking - Card Type'";
...
################ End For Travel & Leisure Sector ########################  
    ...
    $ignoreindex = " ignore index(cscan_product_company_index,c_p_d_productName_index,c_p_d_entryID_index,c_p_d_productStatus_index,c_p_d_productHeadline_index,c_p_d_actual_addedToDatabase_index,  c_p_d_variantID_index,c_p_d_DMSource_index,c_p_d_mPanelID_index,c_p_d_mChannelID_index,c_p_d_sectorID_index,addedToDatabase_c_p_d_index)";
    $selectQuery .= " FROM cscan_product_detail pd $ignoreindex$ocrtext$pjoin$cjoin$ccjoin$afjoin$affjoin$dmajoin$edcjoin$pcjoin$bjoin$ejoin$mljoin$rjoin $tljoin$ajoin$sjoin$ojoin $mchanneljoin $mpanneljoin $statejoin $countryjoin $agejoin $incomejoin $mtypejoin $delmethodjoin $commjoin $primarysectorjoin  $primarycatjoin $primarysubcatjoin $primarysubsubcatjoin $affcatjoin $affsubcatjoin $filesizejoin $panelistsjoin $panelists_detailjoin $ecohortcodejoin $psycodejoin $pzmcodejoin $cnxcodejoin $wealthcompjoin $ethinicityjoin $religionjoin $languagejoin $groupjoin $countrycjoin $assimilationjoin $incomproducingjoin $valuescorejoin $householdincomejoin $homeownerrenterjoin $dmacodejoin $bankingJoin $cardtypeJoin $rewardtypeJoin $paymentcardsJoin $apptypeJoin  $cardtypeJoin2 $cardlevelJoin  $cardlevelJoin2	$rewardJoin	$ratetypJoin $ratetypJoin2 $ratetypJoin3 $creditaccesJoin $ratetypJoin4 $ratetypJoin5 $ratetypJoin6 $energyJoin $erateJoin $etermlengthJoin $erateJoin2 $etermlengthJoin2 $erateJoin3 $etermlengthJoin3 $erateJoin4 $etermlengthJoin4 $mortgageJoin $appmortJoin $ratetypJoin7 $retailconsJoin $telecomJoin $travelJoin $cardtypeJoin3 $publicationjoin $genderjoin $countryjoin2 $where$owhere ";
//$sortby
    if ($dograph != 0) {
        $field = getDoGraph($dograph);
        $selectQuery = "SELECT COUNT($field) AS field_count,$field AS field_name FROM ($selectQuery) as t1 GROUP BY $field"; // SQL_NO_CACHE ORDER BY field_count DESC,field_name ASC
...
/*
	 $selectQuery .= " FROM " . $selecttable . " pd $forceIndexaddedToDatabase$ocrtext$pjoin$cjoin$ccjoin$afjoin$affjoin$dmajoin$edcjoin$pcjoin$bjoin$ejoin$mljoin$rjoin$tljoin$ajoin$sjoin$ojoin $mchanneljoin $mpanneljoin $statejoin $countryjoin $agejoin $incomejoin $mtypejoin $delmethodjoin $responsemechjoin $commjoin $primarysectorjoin  $primarycatjoin $primarysubcatjoin $primarysubsubcatjoin $affcatjoin $affsubcatjoin $filesizejoin $panelistsjoin $panelists_detailjoin $ecohortcodejoin $psycodejoin $pzmcodejoin $cnxcodejoin $wealthcompjoin $ethinicityjoin $religionjoin $languagejoin $groupjoin $countrycjoin $assimilationjoin $incomproducingjoin $valuescorejoin $householdincomejoin $homeownerrenterjoin $dmacodejoin $bankingJoin $cardtypeJoin $rewardtypeJoin $paymentcardsJoin $apptypeJoin  $cardtypeJoin2 $cardlevelJoin  $cardlevelJoin2	$rewardJoin	$ratetypJoin $ratetypJoin2 $ratetypJoin3 $creditaccesJoin $ratetypJoin4 $ratetypJoin5 $ratetypJoin6 $energyJoin $erateJoin $etermlengthJoin $erateJoin2 $etermlengthJoin2 $erateJoin3 $etermlengthJoin3 $erateJoin4 $etermlengthJoin4 $mortgageJoin $appmortJoin $ratetypJoin7 $retailconsJoin $telecomJoin $travelJoin $cardtypeJoin3 $publicationjoin $where$owhere$wheresearchproduct  ";
...
function doQuerytestsphinx_latest($search_id, $docount = false, $addedtodatabaseover = '', $dograph = false, $bid = -1, $relev = false, $expans = false, $unapproved = false, $clear_ps = false, $search_panelist_date_over = -1, $search_values = array(), $sess_userID = 0) {
    global $DRW, $DRW_read, $DRW_main, $DRW_crm, $SPHINX_name;
    $totalfetchid = '';
    $ocrtext = '';
    $where = '';
    ...
    if ($bid < 0) {
        ...
        $multArray = array();
        $otherArray = array('company' => $company, 'affinity_association' => $affinity_association, 'addedToDatabase' => $addedToDatabase, 'month' => $month, 'pd.approved_date' => $approved_date, 'pd.search_competi_id' => $search_competi_id, 'ApplicationType_mult' => $ApplicationType_mult, 'pd.cg_id' => $cg_id, 'rstate' => $rstate, 'pcountry' => $pcountry);
        $panelistArray = array('ppageID' => $age, 'pincomeID' => $income_mult, 'isBiz' => $isBiz, 'dmap.code' => $DMA_ID_mult, 'ppdate' => $ppdate, 'ppdate_month' => $ppdate_month, 'ppstateID' => $ppstateID, 'pgender' => $pgender, 'Income_Producing_Assets_Segment_Code' => $Income_Producing_Assets_Segment_Code, 'edc_id' => $edc_id_mult);
        if ($productName != '') {
            $keyArray = preg_split('/"\\s+or\\s+"/i', $productName, -1, PREG_SPLIT_NO_EMPTY);
            if (count($keyArray) > 1 || preg_match('/^"([^"]+)"$/', $productName)) {
        ...
        foreach ($panelistArray as $field => $val) {
            if ($val != '') {
                ...
                if ($field == 'ppdate') {
                    //$forceIndexaddedToDatabase    =   " force index(idx_comp8) ";
                    $forceindex = 1;
                    $forceIndexaddedToDatabase = addForceIndex('idx_comp8', '1');
        ...
        if ($addedtodatabaseover != '') {
            // $forceIndexaddedToDatabase   =   " force index(idx_comp8) ";
            $forceindex = 1;
            $forceIndexaddedToDatabase = addForceIndex('idx_addedToDatabase', '1');
            // $forceIndexaddedToDatabase   =   " force index(idx_addedToDatabase) ";
        ...
        foreach ($otherArray as $field => $val) {
            if ($val != '') {
                ...
                if ($field == 'pd.search_competi_id' || $field == 'pd.cg_id') {
                    ...
                    //$where .= " pp.panelist_id IN (10288, 174, 15248 ) AND ";
                    // echo " pp.panelist_id IN (10288, 174, 15248 ) AND ";
                    // echo $val;exit;
                    // exit;
...
//                                    }
                ...
                if ($search_id != '') {
                    ...
                    if (!empty($productidsarray)) {
                        ...
                        //$forceindex=1;
                        $forceIndexaddedToDatabase = addForceIndex('idx_productID', '1');

                        $andUnion = '';
                        $newarray = array_chunk($productidsarray, 5000);
                        // echo count($productidsarray).'===='.count($newarray); exit;
                        for ($u = 2; $u < 100; $u++) {
                            if (count($newarray) >= $u) {
                        ...
                        $wheresearchproduct = " where pd.productID in (" . $andcond . ") ";
                    }
    ...
    ################### for select main table #################
    $selecttable = 'pd_scsc_vw';

    $extrasomefield = '';
    if ($statevwtable != '') {
        $selecttable = $statevwtable;
        $extrasomefield = ' , pd.is_panelist,pd.countryCode_copy';
    }

    ################### end for select main table ##################
    // SQL_NO_CACHE
    if ($docount && $wheresearchproduct == '') {
        $selectQuery = " SELECT ";
        $selectQuery .= "COUNT(DISTINCT pd.productID)";
        $sortby = '';
        if ($forceindex != '')
        ...
        $selectQuery .= "DISTINCT pd.productID AS theproductID,pd.entryID_sort1,pd.scsc_sectorID,pd.scsc_categoryID,pd.scsc_subCategoryID,pd.productStatus,
      pd.entryID_sort2,pd.mChannelID, pd.mPanelID,pd.productHeadline,pd.sectorID, pd.categoryID,pd.subCategoryID,pd.entryID,
            pd.addedToDatabase, pd.company,pd.productName, pd.compaignLanguage, pd.firstSeen,pd.lastSeen, pd.mTypeID,pd.state,pd.agentCommunicationID,pd.secondCompany, pd.variantID,pd.affinityAssociation,pd.age,pd.gender,pd.incomeID,pd.publication,pd.isVariant,pd.isDemographic,pd.isInsight,pd.fa_ids, pd.tl_ids,pd.isFICO,pd.incentive_ongoing,pd.incentive,pd.$incentive_set,pd.delmethid,pd.responseMechID,pd.FeeProductType,pd.external_updates,pd.external_fans, pd.external_link,pd.prescription,pd.is_hphsa,pd.subSubCategoryID,
                pd.OfferExpiryDate,pd.is_citi, pd.riders,pd.is_prescreen,pd.isSurvey,pd.IssueTypeID, pd.traffic_sources,pd.social_media_name,pd.worksiteVoluntary,pd.groupSize$matchtext$extrasomefield";

        //for the latest out query
        $selectQuerynew = "DISTINCT pd.productID AS theproductID,pd.entryID_sort1,pd.scsc_sectorID,pd.scsc_categoryID,pd.scsc_subCategoryID,pd.productStatus,
      pd.entryID_sort2,pd.mChannelID, pd.mPanelID,pd.productHeadline,pd.sectorID, pd.categoryID,pd.subCategoryID,pd.entryID,
            pd.addedToDatabase, pd.company,pd.productName, pd.compaignLanguage, pd.firstSeen,pd.lastSeen, pd.mTypeID,pd.state,pd.agentCommunicationID,pd.secondCompany, pd.variantID,pd.affinityAssociation,pd.age,pd.gender,pd.incomeID,pd.publication,pd.isVariant,pd.isDemographic,pd.isInsight,pd.fa_ids, pd.tl_ids,pd.isFICO,pd.incentive_ongoing,pd.incentive,pd.$incentive_set,pd.delmethid,pd.responseMechID,pd.FeeProductType,pd.external_updates,pd.external_fans, pd.external_link,pd.prescription,pd.is_hphsa,pd.subSubCategoryID,
                pd.OfferExpiryDate,pd.is_citi, pd.riders,pd.is_prescreen,pd.isSurvey,pd.IssueTypeID, pd.traffic_sources,pd.social_media_name,pd.worksiteVoluntary,pd.groupSize$matchtext$extrasomefield";
    }
    ...
    if ($wheresearchproduct != '') {
        if ($docount) {
            $newquery = " Select count(*) from (";
        } else {

            $newquery = " Select pd.* from (";
        }
        $newquery.=$selectQuery . " FROM " . $selecttable . " pd $wheresearchproduct";
        $selectQuery = $newquery . " )pd   $ocrtext$pjoin$cjoin$ccjoin$afjoin$affjoin$dmajoin$edcjoin$pcjoin$bjoin$ejoin$mljoin$rjoin$tljoin$ajoin$sjoin$ojoin $where$owhere ";
    } else {
    ...
    //echo $wheresearchproduct;
//echo $totalfetchid;
    // echo $selectQuery;exit;
    //return array($selectQuery, $saved, $_SESSION['totalfetchid']);
    return array($selectQuery, $saved, $sortby);
}


function doQuerytestsphinx2($search_id, $docount = false, $addedtodatabaseover = '', $dograph = false, $bid = -1, $relev = false, $expans = false, $unapproved = false, $clear_ps = false, $search_panelist_date_over = -1, $search_values = array(), $sess_userID = 0,$sql_P2='',$more='',$sql_P3='') {
    global $DRW, $DRW_read, $DRW_main, $DRW_crm, $SPHINX_name;
    $totalfetchid = '';
    $ocrtext = '';
    $where = '';
...
//                                }
    ...
    if ($docount) {
      ...
      $sectorjoin= " LEFT JOIN cscan_sector cs on (cs.sectorID=pd.sectorID) ";
      //$primarysectorjoin= " LEFT JOIN cscan_scsc_product csp on (csp.productID=pd.productID AND csp.scsc_sectorID=cs.sectorID) ";
      $primarysectorjoin= " LEFT JOIN cscan_sector csps on (csps.sectorID=(SELECT scsc_sectorID FROM cscan_scsc_product WHERE productID=pd.productID AND scsc_sort=1)) ";
      ...
      $selectQuery .= " pd.company as Company,pd.secondCompany as 'Second Company',cs.sectorName as Sector,csps.sectorName as 'Primary Sector',cc.sectorName as Category,cspc.sectorName as 'Primary Category',IF(csc.sectorName IS NULL or csc.sectorName = '', 'Not mentioned', csc.sectorName) as 'Sub Category',IF(cspsc.sectorName IS NULL or cspsc.sectorName = '', '', cspsc.sectorName) as 'Primary Sub Category',IF(cssc.sectorName IS NULL or cssc.sectorName = '', 'Not mentioned', cssc.sectorName) as 'Sub Sub Category',IF(cspssc.sectorName IS NULL or cspssc.sectorName = '', '', cspssc.sectorName) as 'Primary Sub Sub Category',pd.entryID,CONCAT('http://www.competiscan.com/index.php?product=',pd.productID) as 'EntryID Link',pd.productHeadline as Headline, cacom.type as 'Communications Type',cmc.mChannelName as MediaChannel, cmp.mPanelName as Audience,cst.stateName as 'State/Province',cscount.country,cap.age_pname as Age,cit.incomeName as Income";
     ...
     $mailvolmonth_query .= ' GROUP BY pd.productID ';
      ...
      /*
       ,IF((SELECT pp.ppmv FROM cscan_panelists_product pp LEFT JOIN cscan_dma_code_postalcode dmap ON (pp.pppostalcode=dmap.pppostalcode), cscan_panelists cp WHERE pp.panelist_id=cp.panelist_id AND productID=pd.productID limit 1)>0 , '1' ,(SELECT COUNT(*) as pieces FROM cscan_panelists_product pp WHERE pp.productID=pd.productID)) AS 'Mail Pieces' */
...
// Added by Pradeep
   /*
    ...
    //End Added by Pradeep
      ...
      //$selectQuery .= "CONCAT('<a href=\"http://www.competiscan.com/productDocuments.php?did=1\">PDF Content</a>') as PDF,";
   /*
      $selectQuery .= ",CONCAT('<a href=";

      $selectQuery .= 'http://www.competiscan.com/productDocuments.php?did=1&id=';
      $selectQuery .= "',";
      $selectQuery .= 'pd.productID,';
      $selectQuery .= "'".'>PDF Content</a>'."'";
      $selectQuery .= ") as 'PDF Content'";
    ...
    */
      ...
      //, pd.groupSize$matchtext

     // pd.entryID_sort1, pd.entryID_sort2,pd.variantID,pd.affinityAssociation,pd.age,pd.gender,pd.incomeID,pd.publication,pd.isVariant,pd.isDemographic,pd.isInsight,pd.fa_ids, pd.tl_ids,pd.isFICO,pd.incentive_ongoing,pd.incentive,pd.$incentive_set,pd.delmethid,pd.responseMechID,pd.FeeProductType,pd.external_updates,pd.external_fans, pd.external_link,pd.prescription,pd.is_hphsa,pd.subSubCategoryID, pd.is_citi, pd.riders,pd.is_prescreen,pd.isSurvey,pd.IssueTypeID, pd.traffic_sources,pd.social_media_name,pd.worksiteVoluntary,
    ...
    $selectQuery .= " FROM " . $selecttable . " pd $forceIndexaddedToDatabase$ocrtext$pjoin$cjoin$ccjoin$afjoin$affjoin$dmajoin$edcjoin$pcjoin$bjoin$ejoin$mljoin$rjoin$tljoin$ajoin$sjoin$ojoin $mchanneljoin $mpanneljoin $statejoin $countryjoin $agejoin $incomejoin $mtypejoin $delmethodjoin $responsemechjoin $commjoin $primarysectorjoin  $primarycatjoin $primarysubcatjoin $primarysubsubcatjoin $affcatjoin $affsubcatjoin $filesizejoin $where$owhere$wheresearchproduct  ";
