...
class MailVolumeCalculator
{
    ...
    function doMailVolumeold($year, $month, $factor = 1.88, $doprint = false)
    {
        global $DRW, $DRW_read2, $DRW_main, $DRW_crm;
        $curr_date = date('Y-m');
        ...
        */
        ...
        /* $sql_P = "SELECT SQL_NO_CACHE ppage,pincomeID,ppstateID,pp.homeownershipID,ppdate,pp.panelist_id,pp.productID,pgender,ppfico_score,isBiz,mChannelID,delmethid,pweight,mTypeID,parent_panelist_id,document_size_byte
            FROM cscan_product_detail pd JOIN cscan_panelists_product pp ON (pd.productID=pp.productID) JOIN cscan_panelists pa ON (pp.panelist_id=pa.panelist_id AND contactTypeID=2) LEFT JOIN cscan_document d ON (pp.productID=d.productID AND d.document_id=1)
            WHERE  ppdate>='2017-01-01' AND pp.ageId=0";
          
         */
         $sql_P = "SELECT SQL_NO_CACHE ppage,pincomeID,ppstateID,pp.homeownershipID,ppdate,pp.panelist_id,pp.productID,pgender,ppfico_score,isBiz,mChannelID,delmethid,pweight,mTypeID,parent_panelist_id,document_size_byte
            FROM cscan_product_detail pd JOIN cscan_panelists_product pp ON (pd.productID=pp.productID) JOIN cscan_panelists pa ON (pp.panelist_id=pa.panelist_id AND contactTypeID=2) LEFT JOIN cscan_document d ON (pp.productID=d.productID AND d.document_id=1)
            WHERE  pp.ageId=0 AND ppdate>='$calc_date_range1' AND ppdate<'$calc_date_range2'";
        
         
         //  AND ppdate>='$calc_date_range1' AND ppdate<'$calc_date_range2' AND ppmv_onupdate<'2018-05-16'";
        ...
        echo  $sql_P = "SELECT SQL_NO_CACHE ppage,pincomeID,ppstateID,pp.homeownershipID,ppdate,pp.panelist_id,pp.productID,pgender,ppfico_score,isBiz,mChannelID,delmethid,pweight,mTypeID,parent_panelist_id,document_size_byte
            FROM cscan_product_detail pd JOIN cscan_panelists_product pp ON (pd.productID=pp.productID) JOIN cscan_panelists pa ON (pp.panelist_id=pa.panelist_id AND contactTypeID=2) LEFT JOIN cscan_document d ON (pp.productID=d.productID AND d.document_id=1)
            WHERE productStatus=1 AND (mPanelID=1 OR mPanelID=2) AND ppdate>='$calc_date_range1' AND ppdate<'$calc_date_range2' AND pp.panelist_id=7131";
        */
        ...
        while ($row = $DRW->fetch_row($result_P)) {
          ...
          //  if($ageID<=0){
             $upd = "UPDATE cscan_panelists_product SET ageID=$ageID,ir_ID=$ir_ID,regionID=$regionID,ppmv_onupdate='".$currentdatetime."'
                    WHERE panelist_id=$panelist_id AND productID=$productID AND ppdate='$ppdate'";
                $DRW->query($upd, $DRW_main);   
                //  echo "processed---".$panelist_id.'panelists==='.$productID.'+++++'.$ageID.'-----'.$ir_ID.'==='.$regionID;                     
            //} else{
                //echo "Not processed---".$panelist_id.'panelists==='.$productID.'+++++'.$ageID.'-----'.$ir_ID.'==='.$regionID;
        ...
        $productArray = array(); 
//        echo '<pre>';
        //echo $panelist_id.'panelists==='.$productID.'+++++'.$ageID.'-----'.$ir_ID.'==='.$regionID;
        //echo"<br>";
        //die('kill here');
        //print_r($comboArray);
        //die;
        ...
        foreach ($comboArray as $key => $arry) {
            ...
            foreach ($ppArray as $pp) {
                ...
                if ($DRW->num_rows($result_comp) > 0) { 
                    $data_comp = $DRW->fetch_assoc($result_comp);
                    $company_chk_id = $data_comp['companyID'];
                }
                
                $sql_chk_sc = "SELECT SQL_NO_CACHE scsc_sectorID,scsc_categoryID FROM cscan_scsc_product where scsc_sort=1 AND productID='".$productID."'";
                $result_scsc = $DRW->query($sql_chk_sc, $DRW_read2);
                if ($DRW->num_rows($result_scsc) > 0) {
                    $data_scsc = $DRW->fetch_assoc($result_scsc);
                    $sector_chk_id = $data_scsc['scsc_sectorID'];
                    $cat_chk_id = $data_scsc['scsc_categoryID'];                   
                }
                $multiplier='';
                if($company_chk_id>0 && $sector_chk_id>0 && $cat_chk_id>0 && $pp_country>0){ 
                ...
                if($multiplier=='' && $sector_chk_id>0 && $pp_country>0){
                    $sql_chk_mv = "SELECT SQL_NO_CACHE multiplier FROM cscan_mv_multiplier where m_sectorID='".$cat_chk_id."' AND m_countryID='".$pp_country."' limit 0,1";
                    $result_mv = $DRW->query($sql_chk_mv, $DRW_read2);
                    if ($DRW->num_rows($result_mv)>0) {
                        $data_multiplier = $DRW->fetch_assoc($result_mv);
                        $multiplier = $data_multiplier['multiplier'];                        
    ...
    function doMailVolume($year, $month, $factor = 1.88, $doprint = false) {
        ...
        $comboArray = array();
        $noComboArray = array();
        echo  $sql_P = "SELECT SQL_NO_CACHE ppage,pincomeID,ppstateID,pp.homeownershipID,ppdate,pp.panelist_id,pp.productID,pgender,ppfico_score,isBiz,mChannelID,delmethid,pweight,mTypeID,parent_panelist_id,document_size_byte,DATEDIFF(CURDATE(),pa.birthdate) as agedays,pa.stateID,pa.homeownershipID,pa.incomeID 
            FROM cscan_product_detail pd JOIN cscan_panelists_product pp ON (pd.productID=pp.productID) JOIN cscan_panelists pa ON (pp.panelist_id=pa.panelist_id AND contactTypeID=2) LEFT JOIN cscan_document d ON (pp.productID=d.productID AND d.document_id=1)
            WHERE productStatus=1 AND (mPanelID=1 OR mPanelID=2) AND ppdate>='2022-07-01 00:00:00' AND ppdate<'2022-08-01 00:00:00' AND pd.productID=9504442";
        echo"<br>"; 
      /*echo  $sql_P = "SELECT SQL_NO_CACHE ppage,pincomeID,ppstateID,pp.homeownershipID,ppdate,pp.panelist_id,pp.productID,pgender,ppfico_score,isBiz,mChannelID,delmethid,pweight,mTypeID,parent_panelist_id,document_size_byte,DATEDIFF(CURDATE(),pa.birthdate) as agedays,pa.stateID,pa.homeownershipID,pa.incomeID 
            FROM cscan_product_detail pd JOIN cscan_panelists_product pp ON (pd.productID=pp.productID) JOIN cscan_panelists pa ON (pp.panelist_id=pa.panelist_id AND contactTypeID=2) LEFT JOIN cscan_document d ON (pp.productID=d.productID AND d.document_id=1)
            WHERE productStatus=1 AND (mPanelID=1 OR mPanelID=2) AND ppdate>='$calc_date_range1' AND ppdate<'$calc_date_range2' AND pd.company = 'Globe Life'";
echo"<br>"; die;*/
        /* added for running march month data */
        /*
          $sql_P = "SELECT SQL_NO_CACHE ppage,pincomeID,ppstateID,pp.homeownershipID,ppdate,pp.panelist_id,pp.productID,pgender,ppfico_score,isBiz,mChannelID,delmethid,pweight,mTypeID,parent_panelist_id,document_size_byte
        ...
        foreach ($comboArray as $key => $arry) {
            ...
            foreach ($ppArray as $pp) {
               ...
               // if (!empty($dataD_ppmv[0])) {
                    ...
                    //                    die;
                    //                }
                    //$multiplier       =   '0.34';//comment by devendra
                    $mail_volume_save = $mail_volume * $multiplier;
