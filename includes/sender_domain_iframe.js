var parent_object_field = 'domainName';
function getParentObj(pof){
	var parent_text_obj = false;
	if(window.parent && window.parent.document && window.parent.document.forms && window.parent.document.forms.searchForm && window.parent.document.forms.searchForm[pof]){
		parent_text_obj = window.parent.document.forms.searchForm[pof];
	}
...
function showCoSel(showall){
	...
	if(showall==1 || wholeval.length>0 || wholeval2.length>0){
		...
		// if(parent_object_field=='sender_domain'){
		// 	var parent_text_objc = getParentObj('sender_domain');
		// 	if(parent_text_objc){
		...
		processajax('sender_domain_list.php', true, 'POST', 'companytext='+encodeURIComponent(wholeval)+'&companysel='+encodeURIComponent(wholeval2)+'&parent_field='+encodeURIComponent(parent_object_field)+satext, document.getElementById('seltext'), 'doResponseCoSel');	
	}
	else{
		hideCoSel();
	}
