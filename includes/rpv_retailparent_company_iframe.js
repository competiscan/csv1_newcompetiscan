var parent_object_field = 'retail_pcompany';
...
function showCoSel(showall){
	...
	if(parent_text_obj){
		...
		processajax('rpv_lookup_retailparent_company_list.php', true, 'POST', 'companytext='+encodeURIComponent(wholeval)+'&companysel='+encodeURIComponent(wholeval2)+'&parent_field='+encodeURIComponent(parent_object_field)+satext, document.getElementById('seltextretailparent'), 'doResponseCoSel');	
	}
	else{
              hideCoSel();
	}
...
function hideCoSel(){
	document.getElementById('seltextretailparent').style.display = 'none';
}
...
function doSelAll(){
	if(document.getElementById('seltextretailparent').style.display != 'none'){
		var compobj = document.forms.companyForm['companysel[]'];
		if(compobj.length){
			for(var i=0;i<compobj.length;i++){
