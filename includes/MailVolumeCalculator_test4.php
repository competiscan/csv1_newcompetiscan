...
class MailVolumeCalculator
{
    ...
    function doMailVolume($year, $month, $factor = 1.88, $doprint = false,$doall=false)
    {
        global $DRW, $DRW_read, $DRW_main, $DRW_crm;
        $curr_date = date('Y-m');
        ...
        if($doall){
            ...
            WHERE productStatus=1 AND (mPanelID=1 OR mPanelID=2) AND ppdate>='$calc_date_range1' AND ppdate<'$calc_date_range2' AND ppaddeddate>DATE_SUB(CURDATE(),INTERVAL 5 DAY)"; 
		}else{
           $sql_P = "SELECT SQL_NO_CACHE ppage,pincomeID,ppstateID,pp.homeownershipID,ppdate,pp.panelist_id,pp.productID,pgender,ppfico_score,isBiz,mChannelID,delmethid,pweight,mTypeID,parent_panelist_id,document_size_byte
            FROM cscan_product_detail pd JOIN cscan_panelists_product pp ON (pd.productID=pp.productID) JOIN cscan_panelists pa ON (pp.panelist_id=pa.panelist_id AND contactTypeID=2) LEFT JOIN cscan_document d ON (pp.productID=d.productID AND d.document_id=1)
        ...
        while ($row = $DRW->fetch_row($result_P)) {
            ...
            if ($mChannelID != 1 || $delmethid == 4 || $delmethid == 5) { //only direct mail for demographics and no FSI (also in productDetail.php display) // || $parent_panelist_id>0 //and no subpanelists (only here)
                ...
                $DRW->query($sqlu, $DRW_main);
                $sqlu = "UPDATE cscan_panelists_product SET ppmv=0,actual_ppmv=0,ppmv_w=0,ppmv_m=0,ppspend=0,cron_update_status=1 WHERE panelist_id=$panelist_id AND productID=$productID AND ppdate='$ppdate'";
                $DRW->query($sqlu, $DRW_main);
        ...
        foreach ($comboArray as $key => $arry) {
            ...
            foreach ($ppArray as $pp) {
                ...
                $ppspend = doSpend($mail_volume, $document_size_byte);
                 ...
                 $upd = "UPDATE cscan_panelists_product SET ppmv=$mail_volume,ageID=$ageID,ir_ID=$ir_ID,regionID=$regionID,actual_ppmv=$actual_mail_volume,ppmv_w=$ppmv_w,ppmv_m=$mail_volume,ppspend=$ppspend,cron_update_status=1 WHERE panelist_id=$panelist_id AND productID=$productID AND ppdate='$ppdate'"; 
                ...
                foreach ($multAs as $multA) {
                    foreach ($multA as $pp_sector => $a) {
                        foreach ($a as $pp_category => $a2) {
                            ...
                            $upd = "UPDATE $ppctables SET ppmv_m=$ppmv_m,ppmv=$ppmv_m,cron_update_status=1 
    ...
    function doMailVolume_Before4Months($year, $month, $factor = 1.88, $doprint = false)
    {
        global $DRW, $DRW_read, $DRW_main, $DRW_crm;
        $curr_date = date('Y-m');
        $calc_date = $year . '-' . $month;
        ...
        while ($row = $DRW->fetch_row($result_P)) {
            ...
            if ($mChannelID != 1 || $delmethid == 4 || $delmethid == 5) { //only direct mail for demographics and no FSI (also in productDetail.php display) // || $parent_panelist_id>0 //and no subpanelists (only here)
                ...
                $sqlu = "UPDATE cscan_product_detail SET isDemographic=1$ficosql WHERE productID=$productID";
                $DRW->query($sqlu, $DRW_main);
                $sqlu = "UPDATE cscan_panelists_product SET ppmv=0,actual_ppmv=0,ppmv_w=0,ppmv_m=0,ppspend=0 WHERE panelist_id=$panelist_id AND productID=$productID AND ppdate='$ppdate'";
                $DRW->query($sqlu, $DRW_main);
        ...
        foreach ($comboArray as $key => $arry) {
            ...
            foreach ($ppArray as $pp) {
                ...
                $ppspend = doSpend($mail_volume, $document_size_byte);
                 ...
                 $upd = "UPDATE cscan_panelists_product SET ppmv=$mail_volume,ageID=$ageID,ir_ID=$ir_ID,regionID=$regionID,actual_ppmv=$actual_mail_volume,ppmv_w=$ppmv_w,ppmv_m=$mail_volume,ppspend=$ppspend
                ...
                foreach ($multAs as $multA) {
                    foreach ($multA as $pp_sector => $a) {
                        foreach ($a as $pp_category => $a2) {
                            ...
                            $upd = "UPDATE $ppctables SET ppmv_m=$ppmv_m,ppmv=$ppmv_m
                                WHERE panelist_id=$panelist_id AND cscan_panelists_product.productID=$productID AND ppdate='$ppdate' AND cscan_panelists_product.productID=cscan_scsc_product.productID AND cscan_scsc_product.scsc_sectorID=$pp_sector AND cscan_scsc_product.scsc_sort=1$ppctext";
