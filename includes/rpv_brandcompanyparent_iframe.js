var parent_object_field = 'brand_pcompany';
function getParentObj(pof){
        var parent_text_obj = false;
	if(window.parent && window.parent.document && window.parent.document.forms && window.parent.document.forms.rpvdashboardForm && window.parent.document.forms.rpvdashboardForm[pof]){
		parent_text_obj = window.parent.document.forms.rpvdashboardForm[pof];
	}
...
function showCoSel(showall){
	...
	if(parent_text_obj){
		...
		processajax('rpv_lookup_brandcompanyparent_list.php', true, 'POST', 'companytext='+encodeURIComponent(wholeval)+'&companysel='+encodeURIComponent(wholeval2)+'&parent_field='+encodeURIComponent(parent_object_field)+satext, document.getElementById('seltextbrandcompanyparent'), 'doResponseCoSel');	
	}
	else{
              hideCoSel();
	}
...
function hideCoSel(){
	document.getElementById('seltextbrandcompanyparent').style.display = 'none';
}
...
function doSelAll(){
	if(document.getElementById('seltextbrandcompanyparent').style.display != 'none'){
		var compobj = document.forms.companyForm['companysel[]'];
		if(compobj.length){
			for(var i=0;i<compobj.length;i++){
