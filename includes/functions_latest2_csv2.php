...
function doQuery_latest_csv2($search_id, $docount = false, $addedtodatabaseover = '', $dograph = false, $bid = -1, $relev = false, $expans = false, $unapproved = false, $clear_ps = false, $search_panelist_date_over = -1, $search_values = array(), $sess_userID = 0) {
...
//echo $where;exit;
        ...
        if (($searchKey != '' && $searchKey != 'NULL') || ($searchKey2 != '' && $searchKey2 != 'NULL')) {
            //echo "HEEEEEEEEEEEEEEEEEEEEEEEEE"."<br/>";
            $search_rulesArray = array();
            $search_rulesArrayf = array();
            $check_search_sql = "SELECT * FROM cscan_search where userID =".$sess_userID." AND ID='".$search_id."'";
            ...
            if($dataSearchCheck['search_rules']!='' && $dataSearchCheck['search_rules']!='NULL'){
                ...
                if($searchKey!='' && $searchType=='fulltext2'){
                ...
                }elseif($searchKey!='' && $searchType=='ocr2'){
                    if (strpos ($searchKey, '"') !== false || strpos ($searchKey, "'") !== false) {
                ...
                }elseif(($searchKey!='' && $searchType=='ocr_fulltext2') AND ($searchKey2!='' && $search_type_and=='1')){
                ...
                }elseif(($searchKey!='' && $searchType=='fulltext2') OR ($searchKey2!='' && $search_type_and=='0')){
                $postdata['dts_val'] = strtoupper($searchKey);
                ...
                if($dataSearchCheck['minmaxmortgage']!='NULL' && $dataSearchCheck['minmaxmortgage']!='' && $dataSearchCheck['minmaxmortgage']!='0-2000000'){
                ...
                if($dataSearchCheck['addedToDatabase']!=''){
                    ...
                    $exp_addedtodatabaseover= explode(' ', $addedtodatabaseover);
                    $from_addedtodatabaseover="S".$exp_addedtodatabaseover[0]."T".$exp_addedtodatabaseover[1];
                    $postdata['added_to_database']=$from_addedtodatabaseover;
                    //$addedtodatabaseover
                }
                ...
                if($dataSearchCheck['approved_date']!='0000-00-00 00:00:00' && $dataSearchCheck['approved_date_to']!='0000-00-00 00:00:00'){
                    ...
                    $exp_addedtodatabaseover= explode(' ', $addedtodatabaseover);
                    $from_addedtodatabaseover="S".$exp_addedtodatabaseover[0]."T".$exp_addedtodatabaseover[1];
                    $postdata['approved_date']=$from_addedtodatabaseover;
                ...
                /* 
                if($dataSearchCheck['publication_name']!=''){
                    $postdata['publication_id']=$dataSearchCheck['publication_name'];
                }*/
                ...
                if($dataSearchCheck['value_score']!='' && $dataSearchCheck['value_score']!='NULL'){
    ...
    if ($docount) {
        ...
        $selectQuery .= " DISTINCT pd.productID AS theproductID,mChannelID,mPanelID,productHeadline,sectorID,categoryID,subCategoryID,entryID,
            ...
            OfferExpiryDate,is_citi,riders,is_prescreen,isSurvey,IssueTypeID,traffic_sources,social_media_name,worksiteVoluntary,is_digital,panelist_sort,is_mobile,simple_domain,faux_check,socialmedia_adtype,personalization,groupSize,is_panelist_score$matchtext$score_range_field";
    }
