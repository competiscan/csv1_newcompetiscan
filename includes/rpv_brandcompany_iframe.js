var parent_object_field = 'brand_company';
...
function showCoSel(showall){
	...
	if(parent_text_obj){
		...
		processajax('rpv_lookup_brandcompany_list.php', true, 'POST', 'companytext='+encodeURIComponent(wholeval)+'&companysel='+encodeURIComponent(wholeval2)+'&parent_field='+encodeURIComponent(parent_object_field)+satext, document.getElementById('seltextbrandcompany'), 'doResponseCoSel');	
	}
	else{
              hideCoSel();
	}
...
function doSelAll(){
	if(document.getElementById('seltextbrandcompany').style.display != 'none'){
		var compobj = document.forms.companyForm['companysel[]'];
		if(compobj.length){
			for(var i=0;i<compobj.length;i++){
