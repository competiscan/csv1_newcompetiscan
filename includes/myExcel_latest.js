function chexStart(chexblock,chexstart,nameobj){
	var chexend = document.form1['chexEnd'+chexblock].value;
	console.log(chexblock,"obname", chexstart)
	console.log("name",nameobj)
	var obj = 0;
	var first_box = true;
	var obj_checked = true;
	if(chexblock >=1001){
		var additional_v = document.getElementById("additional_v");
	}else{
		var additional_v=null;
	}
	for(var i=chexstart;i<chexend;i++){
		...
		if(obj){
			...
			//let additional_v = document.getElementById("additional_v");
			//let additional_a2 = document.getElementById("additional_a2");
			obj.checked = obj_checked;
			console.log("ttttttttttt",obj.checked,"dddddddd",additional_v)
			if(additional_v != null){
				if(obj.checked){
					//console.log("testttttttttttttttttttttttttt");
					if(additional_v.innerText.search("Variables")) {
						//console.log("now api send1")
					document.getElementById("competi_id").value = "competi_id";
					document.getElementById("additional_ppdate").value = "additional_ppdate";
					}else if(additional_v.innerText.search("Variables")  && chexstart >= 261) {
						//console.log("now api send")
					document.getElementById("competi_id").value = "competi_id";
					document.getElementById("additional_ppdate").value = "additional_ppdate";
					}else if(nameobj=='adddition_detail' && additional_v.innerText.search("Variables") && chexstart >= 261) {
						//console.log("now api send2222222")
					document.getElementById("additional_ppdate").value = "additional_ppdate";
					document.getElementById("competi_id").value = "competi_id";
					}else if(nameobj=='allField' && additional_v.innerText.search("Variables") && chexstart >= 261) {
						//console.log("now api send5555555")
					document.getElementById("additional_ppdate").value = "additional_ppdate";
					document.getElementById("competi_id").value = "competi_id";
					}else{
						document.getElementById("additional_ppdate").value = "";
						document.getElementById("competi_id").value = "";
					}
				}
				else{
					document.getElementById("additional_ppdate").value = "";
	...
	if(document.form1.allField.checked == true) {
		for(i=1;i<document.form1.elements.length;i++) {
			if(document.form1.elements[i].type == 'checkbox'){
				console.log(document.form1.elements[i].checked)
