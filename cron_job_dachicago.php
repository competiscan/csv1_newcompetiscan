...
//       }
            ...
            if(!empty($productID)){
                ...
                if($DRW->num_rows($q_chk_dup) == 0){
                    ...
                    if ($DRW->num_rows($rs_check) > 0) {//echo $file;//die;
                        ...
                        if(preg_match('/(\\d+)_(\\d+)_(\\d+)/', $DMSource, $matches)){//pr($matches);
...
//                                    $ppdate = date("Y-m-d H:i:s", strtotime($rs_chk_p_ppdate." -1 day")); 
                                ...
                                if($DRW->query($sql_pd, $DRW_main)){
                                    // for sinking purpose
                                    InsertAndUpdateProductID($productID);
...
//                }
                ...
                foreach ($pdffile_arr as $filename) {
                    ...
                    if($DRW->query($sqlc,$DRW_main)){
                        ...
                        // for sinking purpose
                        InsertAndUpdateProductID($pdtID);
                        if(!empty($sectorID)){
                            $sid = $sectorID;
                        }else{
                            $sid = '0';
                       ...
                       if (copy($ofilename, $latestpath)) {
                            ...
                            createPreviewJPGWithPythonCode(dirname(__FILE__)."/".$newpath.$pdtID.'/',$fname,$pdtID);
                           // echo"createdjep";
                        }
