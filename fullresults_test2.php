...
if($ssid==0 && $bid<0) {
        ob_end_clean();
        header("Location: fullsearch_test2.php");
        exit;
}
$gets = "&amp;ssid=".$ssid;
if($bid>=0) {
...
if(isset($_POST['basket_action'])){
        ...
        if($basket_action>0){
                if($basket_action==7 || $basket_action==8 || $basket_action==9 || $basket_action==10){//Copy To/Add page/all || Move To page/all
                    if($bid<0){
                                $chcsv2 = curl_init(API_URL_EMAIL_ALERT_UAT);
...
.image_ann_icon {
  mix-blend-mode:red; /* Change to other blend modes for different effects */
...
if($search_num_of_rows > 0) {
        $buttons .= '<div style="float:left;margin-left:0px;"><form action="myExcel.php" method="post" name="myexceller"><input type="hidden" name="ssid" value="'.$ssid.'" /><input type="hidden" name="bid" value="'.$bid.'" /><input type="hidden" name="page" value="'.$page_no.'" /><input type="hidden" name="num_rec" value="'.$search_num_of_rows.'" /><input type="hidden" name="sort" value="'.$sort.'" /><input class="submitbutton" type="submit" name="myExcel" value="My Excel" /></form>';
...
?>
        ...
        <!--<td class="bodytext" valign="top" style="border-bottom:none;"><strong>Headline:</strong><br /><a href="productDetail.php?id=<?php echo $productID.$pdf_key.'&amp;ssid='.$ssid; ?>&checkstatus=1" onclick="productDescription_digital_testing(<?php echo $productID; ?>,'<?php echo singleQuoteSafe($pdf_key.'&amp;ssid='.$ssid); ?>'); return false;" class="HyperLink" title="See complete product details"><?php echo mb_convert_encoding(html_entity_decode( str_replace("\xE2\x80\x8B", "",$productHeadline),ENT_QUOTES, "UTF-8"), "HTML-ENTITIES","UTF-8"); ?></a></td>-->
        ...
        if ($_SESSION['sess_plevel'] > 0) {
            ...
            } elseif ($resultProdData['mchanne_id']==3 || $resultProdData['mchanne_id']==1 || $resultProdData['mchanne_id']==2 || $resultProdData['mchanne_id']==6 || $resultProdData['mchanne_id']==14) {
...
//         $nextsid=implode(",",$sidNextArray['sid']);
     ...
     /*echo "dsdsd111".$firstsid."<br/>";
     echo "nnnnnnn".$nextsid."<br/>";*/
     //die;
     //echo "<pre>";
...
<input type="hidden" name="page" value="<?php echo $page_no; ?>" />
