...
if($pp || $pdf){
	echo '<form name="ppForm" method="post" action="exportDocument_test.php" target="_blank">
	<div style="margin:5px;"><input class="submitbutton" type="submit" name="submit1" value="Export To '.$ptext.'" /></div><div style="margin-left:7px;"><a href="#" class="bluelink" onclick="select_all_checkbox(); return false;">Select All</a></div><div>&nbsp;</div>';
	$full = '';
}
else{
	$full = '&amp;full=1';
...
if($products > 0) {
	foreach($productIDArray as $pid){
		...
		$is_full_one = false;
		 ...
		 $query2 = "SELECT img_document_sort,UNIX_TIMESTAMP(img_document_createddate),img_document_path,img_document_filename,img_document_content_type FROM cscan_img_document WHERE productID=$pid $condQuery ORDER BY img_document_sort";
