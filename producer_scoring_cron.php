...
function insertMonthData($curdate){
    global $DRW,$DRW_main,$DRW_read,$DRW_read2,$DRW_digital;
        ...
        $sqlquery = "select id,panelist_id,competi_id,parent_panelist_id,SUM(email_piece) as email_piece ,SUM(email_piece_point) as email_piece_point,entry_date,insert_date from cscan_producer_scoring_daily_basis_reports 
    ...
    if ($sqlqueryres) {
        while ($row = $DRW->fetch_assoc($sqlqueryres)) {
            ...
            $selsql     =   "select id,panelist_id,competi_id from cscan_producer_scoring_monthly_reports 
            ...
            if ($DRW->num_rows($ressql) > 0) {  
                ...
                $id      = $rowdata['id'];                
                $sql  = "update cscan_producer_scoring_monthly_reports set 
                            email_piece = '".$email_piece."',
                            email_piece_point='".$email_piece_point."'
                            where id='".$id."' AND competi_id='".$rowdata['competi_id']."'";    
            } else{
                $sql  = "insert  into cscan_producer_scoring_monthly_reports set 
