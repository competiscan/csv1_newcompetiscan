...
require_once('includes/function_test5.php');  //latest function
...
if(isset($_POST['basket_action'])){
	...
	if($basket_action>0){
		if($basket_action==7 || $basket_action==8 || $basket_action==9 || $basket_action==10){//Copy To/Add page/all || Move To page/all
			if($bid<0){
				list($selectQuery,$saved) = doQuery_test5($ssid,false,'',false,-1,$dorelev,$doexpans);
				list($countQuery) = doQuery_test5($ssid,true);
			}
			else{
				list($selectQuery,$saved) = doQuery_test5(0, false, '', false, $bid);
				list($countQuery) = doQuery_test5(0, true, '', false, $bid);
			}
			$selectQuery .= $orderby;
...
if($bid<0){
	...
	if(false && $_SESSION['sess_plevel']==2){
        ...
        list($countQuery) = doQuery_test5($ssid,true,'',false,-1,false,false,$unapproved);   
        ...
        list($selectQuery,$saved) = doQuery_test5($ssid,false,'',false,-1,$dorelev,$doexpans,$unapproved,false,-1,array(),0,((!empty($orderby)?$orderby:false)) ,((!empty($limit1)?$limit1:0)));
