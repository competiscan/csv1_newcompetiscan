...
<script src="includes/productDetail.js?v=20140201" type="text/JavaScript"></script>
<?php
...
if($productRs['productID']!=''){
	...
	$thepdf = 'productDocuments.php?id='.$productID.'&amp;did=1';
    $combined = 'productDetail.php?id='.$productID.'&amp;did=1&makepdf=1';
	...
	?>
	<div id="div1">
	<div style="padding-bottom:6px;"><img src="images/competiscan_logo.jpg" width="244" height="61" border="0" /></div>
<?php if(!$makePDF):?>
	...
	?>
      ...
      <td class="bodytext"><a onclick="printAll(<?php echo $productID; ?>,'<?php echo $combined; ?>',2); return false;" class="HyperLink" href="#" title="Print All">Print All</a></td>
     </tr>
    </table>
<?php endif;?>
	...
	<tr>
		...
		if(($productRs['mChannelID']==5 || $productRs['mChannelID']==7) && ($is_flash || $is_image)){
			?>
			<td valign="top" colspan="2"><img src="productImg.php?<?php echo $pi; ?>" style="border: solid 1px #000000;float:left;" alt="" />
			<?php
			if($document_size_byte>0){
				if($is_flash){
					...
					var attributes = {};
					swfobject.embedSWF("<?php echo 'productDocuments.php?id='.$productID.'&did='.$document_id; ?>", "flashContent", "<?php echo $fwidth; ?>", "<?php echo $fheight; ?>", "9.0.0","includes/expressInstall.swf", flashvars, params, attributes);
					//-->
					</script>
					...
					?>
					<div style="border: solid 1px #000000;float:right;"><img src="<?php echo 'productDocuments.php?id='.$productID.'&amp;did='.$document_id; ?>" alt="" /></div>
					<?php 
				}
			}
			?>
			...
			?>
			<td valign="top"><img src="productImg.php?<?php echo $pi; ?>" style="border: solid 1px #000000;" alt="" /></td><td class="bodytext" valign="bottom"><?php
			if($document_size_byte>0 && !$makePDF){
				?>
				<a class="bluelink" href="<?php echo $thepdf; ?>" onclick="printAll(<?php echo $productID; ?>,'<?php echo $thepdf; ?>',3); return false;">View PDF Content <img src="images/pdf.jpg" border="0" style="vertical-align:top;" /></a> &nbsp; [<?php echo $DisplaySize; ?>] <br />
				<?php 
			}
			else {
				echo '&nbsp;';
		...
		if(($productRs['mPanelID']==1 || $productRs['mPanelID']==2) && ($productRs['mChannelID']==1 || $productRs['mChannelID']==3)){//consumer ||  Emp/Biz direct mail
			...
			if($ssid>0){
				...
				if($search_panelist_date || $consumer_only){
					if($addedToDatabase!='') {
						if($addedToDatabase=='week') $ppdatetext .= ' pp.ppdate>=CONCAT(DATE_SUB(CURDATE(),INTERVAL 7 DAY),\' 00:00:00\') AND ';
						elseif($addedToDatabase=='2week') $ppdatetext .= ' pp.ppdate>=CONCAT(DATE_SUB(CURDATE(),INTERVAL 14 DAY),\' 00:00:00\') AND ';
						elseif($addedToDatabase=='1month') $ppdatetext .= ' pp.ppdate>=CONCAT(DATE_SUB(CURDATE(),INTERVAL 1 MONTH),\' 00:00:00\') AND ';
						elseif($addedToDatabase=='3month') $ppdatetext .= ' pp.ppdate>=CONCAT(DATE_SUB(CURDATE(),INTERVAL 3 MONTH),\' 00:00:00\') AND ';
						elseif($addedToDatabase=='6month') $ppdatetext .= ' pp.ppdate>=CONCAT(DATE_SUB(CURDATE(),INTERVAL 6 MONTH),\' 00:00:00\') AND ';
						elseif($addedToDatabase=='1year') $ppdatetext .= ' pp.ppdate>=CONCAT(DATE_SUB(CURDATE(),INTERVAL 1 YEAR),\' 00:00:00\') AND ';
					}
					elseif($month!='') {
			...
			if($mail_volume_tot>0 && $productRs['mPanelID']!=2){
				...
				<tr><td class=\"bodytext\" width=\"40%\"><strong>Historical EMV</strong></td><td style=\"border:none;\">&nbsp;</td><td class=\"bodytext\"><strong>Mail Piece Observance</strong></td></tr>";
				
				$table .= "<tr><td style=\"border:none;\"><table border=\"0\" cellspacing=\"1\" cellpadding=\"3\" class=\"bodytext_small\" width=\"100%\">";
				$table .= '<tr><td><strong>Estimated Mail Volume</strong></td><td>'.number_format($mail_volume_tot).'</td></tr>';
				foreach($yearMonthArray as $y=>$mv){
					$table .= '<tr><td><strong>'.$y.' EMV</strong></td><td>';
					$tmp = 0;
					foreach($mv as $m=>$v){
						$tmp+=$v;
					}
				...
				foreach($yearMonthArray as $y=>$mv){
					...
					for($i=1;$i<=12;$i++){
						...
						if(strlen($i)<2) $i = '0'.$i;
						if(isset($mv[$i]) && $mv[$i]>0) $table .= ' style="background-color:#0055E3;color:#FFFFFF;font-weight:bold;">X';//number_format($mv[$i]);
						else $table .= '>&nbsp;';
						$table .= "</td>";
					}
					$table .= "</tr>";
				...
				<tr><td colspan=\"3\" class=\"bodytext\"><strong>Estimated Mail Volume Transparency</strong></td></tr>
				<tr><td colspan=\"3\" style=\"border:none;\"><table border=\"0\" cellspacing=\"1\" cellpadding=\"3\" class=\"bodytext_small\" width=\"100%\">
				<tr><td valign=\"bottom\" style=\"border:none;\" align=\"center\"><strong>Panelist</strong></td><td valign=\"bottom\" align=\"center\"><strong>Age</strong></td><td valign=\"bottom\" align=\"center\"><strong>Income</strong></td><td valign=\"bottom\" align=\"center\"><strong>State / Province</strong></td><td valign=\"bottom\" align=\"center\"><strong>ValueScore&reg;</strong></td><td valign=\"bottom\" align=\"center\"><strong>ISO Risk Quality Index-Auto</strong></td><td valign=\"bottom\" align=\"center\"><strong>ISO Risk Quality Index-Home</strong></td><td valign=\"bottom\" align=\"center\"><strong>EMV</strong></td></tr>"; // <td valign=\"bottom\" align=\"center\"><strong>Gender</strong></td>
				
				$sql_P = "SELECT SUM(ppmv),COUNT(*) as pieces,ppage,incomeName,stateName,pgender,SUM(ppmv_w),competi_id,pp.panelist_id,pproductFICO 
				...
				while($row = $DRW->fetch_row( $result_P )){
					...
					if($pgender=='') $pgender = '&nbsp;';
					if($ppmv>0){
						$table .= "<tr><td style=\"border:none;\" nowrap=\"nowrap\"><a href=\"#\" onclick=\"showCalcCos($pid); return false;\" class=\"bluelink\" style=\"font-size: 10px;\" id=\"thecalccoslink$pid\">$competi_id</a></td><td align=\"center\" valign=\"top\">$age</td><td align=\"center\" valign=\"top\">$ir_name</td><td align=\"center\" valign=\"top\">$stateName2</td>
						<td align=\"center\" valign=\"top\">$ficotext</td>
						<td align=\"center\" valign=\"top\">$RAPA_EMLC_ZIP_REL_txt</td>
				...
				echo '<strong>Estimated Mail Volume:</strong> <a href="#" onclick="showCalc('.$prow.'); return false;" class="bluelink" id="thecalclink">'.number_format($mail_volume_tot).'</a>';//$productRs['mail_volume']
				echo '<div style="display:none;position:absolute;background:#ffffff;padding:0px;border:solid 2px #000000;z-index:100;width:620px;" id="thecalc"><div style="float:right;height:26px;"><a href="#" onclick="hideCalc(); return false;" class="bluelink">close</a> &nbsp; </div><div style="clear:both;height:1px"><img src="images/spacer.gif" width="1" height="1" /></div><div id="thecalc_inner" style="padding:4px;">'.$table.'</div></div>';
				...
				while($row = $DRW->fetch_row( $result_P )){
					...
					$pid = $row[1];
					////Panelist Profile?
					echo "<a href=\"#\" onclick=\"showCalcCos($pid); return false;\" class=\"bluelink\" style=\"font-size: 10px;\" id=\"thecalccoslink$pid\">$competi_id</a> ";
				}
			}
	...
	$showArray[] = "<strong>Company:</strong> ".htmlspecialchars($company);
	$secondCompany = '';
	$resultC = $DRW->query("SELECT companyName FROM cscan_company pa,cscan_company_product pp 
	...
	$showArray[] = "<strong>Second Company:</strong> ".htmlspecialchars($secondCompany);//$productRs['secondCompany']
	...
	if($productRs['mChannelID']==6){
		...
		if(!empty($panelist_edc)){
			$showArray[] = "<strong>EDC:</strong> ".htmlspecialchars($panelist_edc);
		}
		$showArray[] = "<strong>Sector:</strong> ".htmlspecialchars($sector);
		$showArray[] = "<strong>Category:</strong> ".htmlspecialchars($category);
		$showArray[] = "<strong>Sub Category:</strong> ".htmlspecialchars($subcategory);
...
<?php if(!$makePDF):?>
	<tr>
	 	<td align="left" valign="top" class="bodytext"><a href="<?php echo "sendLink.php?id=".$productID.'&amp;send_mode=1'; ?>" class="bluelink" title="Click this if you want to send the products details of this product as a link to your colleague" onclick="sendColleague(<?php echo $productID; ?>,1); return false;">Send this as a link to your colleague</a></td>
		...
		if($document_size_byte>0 && ($productRs['mChannelID']==5 || $productRs['mChannelID']==7) && ($is_flash || $is_image)){
			echo '<a href="productDocuments.php?id='.$productID.'&amp;did='.$document_id.'" onclick="var winy = window.open(\'productDocuments.php?id='.$productID.'&amp;did='.$document_id.'\'); winy.focus(); return false;" class="bluelink">Download Creative Execution</a>';
	...
	?>
	<tr>
	 	<td align="left" valign="top" class="bodytext"><a href="<?php echo "sendLink.php?id=".$productID.'&amp;send_mode=2'; ?>" class="bluelink" style="font-style:italic;" title="Click this if you want to send the products details of this product as a QA link to your colleague" onclick="sendColleague(<?php echo $productID; ?>,2); return false;">Send this as a QA link to your colleague</a><br /><form name="qaform" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>" style="display:inline;"><label style="font-style:italic;"><input type="checkbox" name="qa" value="1" onclick="doQA(<?php echo $productID; ?>);"<?php if(!empty($productRs['is_qa'])) echo ' checked="checked"'; ?> />QA Certified</label></form></td>
		<td align="left" valign="top" class="bodytext"><?php 
		if($document_size_byte>0) {
		?>
...
<?php endif;?>
	...
	<tr><td>&nbsp;</td></tr>
<?php if(!$makePDF):?>
	<tr><td><a href="#" class="HyperLink" onclick="window.close(); return false;" title="Click to Close this window">Close-Window</a><?php 
