...
print " &nbsp; <form action=\"panelist_report_iframe.php\" style=\"display:inline;\" method=\"get\" name=\"exer\"><input class=\"button\" type=\"submit\" name=\"exportbutton\" value=\"Export\" />
...
</form> &nbsp; <form action=\"{$_SERVER['PHP_SELF']}\" style=\"display:inline;\" method=\"post\" name=\"older\"><input class=\"button\" type=\"submit\" name=\"go\" value=\" Report by Month \" onclick=\"document.location.href='panelist_report_month.php'; return false;\" /></form>";
//print " &nbsp; <form action=\"panelist_report_iframe.php\" style=\"display:inline;\" method=\"get\" name=\"uper\"><input class=\"button\" type=\"submit\" name=\"updater\" value=\"Update CRM Points\" /><input type=\"hidden\" name=\"update\" value=\"1\" /></form>";
...
print "</select> <select name=\"start_dr\" size=\"1\" class=\"textinput\"><option value=\"00\">&nbsp;</option>";
for($i=1;$i<=31;$i++){
...
print "</select> <select name=\"start_yr\" size=\"1\" class=\"textinput\"><option value=\"0000\">&nbsp;</option>";
...
for($i=11;$i>=0;$i--){
	...
	$m = date('m',$tstamp);
	$Y = date('Y',$tstamp);
	print "<a href=\"{$_SERVER['PHP_SELF']}?start_mr=$m&amp;start_dr=01&amp;start_yr=$Y&amp;end_mr=$m&amp;end_dr=31&amp;end_yr=$Y&amp;do_points_sent=0&amp;sendsearch=1\" class=\"HyperLink\">{$month_name[$m]}";
...
print "</select> <select name=\"end_dr\" size=\"1\" class=\"textinput\"><option value=\"00\">&nbsp;</option>";
for($i=1;$i<=31;$i++){
	$day = str_pad($i,2,'0',STR_PAD_LEFT);
	print "<option value=\"$day\"";
...
print "</select> <select name=\"end_yr\" size=\"1\" class=\"textinput\"><option value=\"0000\">&nbsp;</option>";
for($i=$start_year;$i<=$to_year;$i++){
	print "<option value=\"$i\"";
...
print '</table>';
print "<iframe name=\"rlist\" style=\"border: solid 1px #0055E3;padding:0px;margin:0px;\" src=\"panelist_report_iframe.php\" width=\"970\" height=\"500\" frameborder=\"0\" marginheight=\"0\" marginwidth=\"0\" scrolling=\"auto\"></iframe>";
?>
