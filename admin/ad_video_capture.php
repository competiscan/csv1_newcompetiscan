...
if (isset($_REQUEST['delID']) && ($_REQUEST['delID'] > 0)) {
    ...
    for ($i = 0; $i < count($delID); $i++) {
        ...
        if ($move == 'Move to Online Video Filter') {
    ...
    if ($movemsg == '1') {
        $message = "<b>$i</b> Record(s) has been moved in online Video Filter.";
    } elseif ($delmsg == '1') {
...
$sql = "SELECT creative_id,creative_path,ad_md5,digital_channel,file_type,lastUpdated,capture_date FROM " . $selecttable . " WHERE digital_channel='Online Video' AND capture_status=1 $where_pan ORDER BY capture_date DESC  LIMIT $p, $limit";
$rs = $DRW->query($sql, $DRW_digital);
$resultCount = $DRW->num_rows($rs);


$numquery = "Select COUNT(creative_id) as numrows FROM " . $selecttable . " WHERE digital_channel='Online Video' AND capture_status=1 $where_pan";
...
<form method='post' name='frm1'>
    <table width='100%' border='0' cellspacing='0' cellpadding='5' class='text' rules='none' bordercolor='#B9B9B9' align='center' style='border-collapse:collapse'>
        <tr><td class="adminhead" align='center' colspan='7'>ONLINE VIDEO CAPTURE PRODUCT</td></tr>
                        ...
                        <td align='right' colspan='2'>
                            <table width="100%" border="0" cellspacing="0" cellpadding="0" class="text">
                                ...
                                <tr>
                                    ...
                                    <td align='center' width="40%" style="padding-bottom: 10px;padding-top: 10px;"><input class='button' style='width:60px' type='submit' name='submit1' ID='delBt' value='Delete' onclick=' return confirmDel()'></td>

                                    <td align='left' width="50%"><input class='button'  type='submit' name='move' ID='moveBt' value='Move to Online Video Filter' onclick=' return confirmMove()'></td>
                                    <td align='right' width="35%"><a href="ad_video_display.php" style="text-decoration:none;"><input class='button' style='width:140px' type='button' name='submit1' ID='delBt' value='Go to Filter Page'></a><br /></td>
                                </tr>
                            </table>
                        </td>
                    </tr>
        ...
        <tr>
            <td colspan='6' align='center' class='error'><?php echo $message; ?></td>
        </tr>
        <?php
        if ($resultCount > 0) {
            $className = '';
...
//                     $devicename = 'Online Display';
                ...
                <tr valign=top class="<?php echo $className; ?>">
                    ...
                    <td align="right"><a style="text-decoration:none;" href="addproduct-digital.php?old=<?php echo $ID; ?>&add=3&tbl=<?php echo $creative_tblid; ?>"><input class='button' style='width:120px;' type='button' name='moveproduct' value='Move To Product'></a></td>
                </tr>
                <?php
            }
