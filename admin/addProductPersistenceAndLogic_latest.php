...
if (isset($_POST['advertiser_address'])) {
    $advertiser_address = $_POST['advertiser_address'];
} else{
    $advertiser_address = '';
}
if (isset($_POST['advertiser_city'])) {
    $advertiser_city = $_POST['advertiser_city'];
} else{
    $advertiser_city = '';
}
if (isset($_POST['advertiser_state'])) {
    $advertiser_state = $_POST['advertiser_state'];
} else{
    $advertiser_state = '';
}
if (isset($_POST['advertiser_zipcode'])) {
    $advertiser_zipcode = $_POST['advertiser_zipcode'];
} else{
    $advertiser_zipcode = '';
}
if (isset($_POST['advertiser_phone_number'])) {
    $advertiser_phone_number = $_POST['advertiser_phone_number'];
} else{
    $advertiser_phone_number = '';
}
if (isset($_POST['advertiser_url'])) {
    $advertiser_url = $_POST['advertiser_url'];
} else{
    $advertiser_url = '';
}
...
print_r($_FILES); die;*/
    $redirect_url1='';
    if(isset($_POST['productStatus']) && $_POST['productStatus']==1){
        
        if(empty($_POST['mChannelID']) || empty($_POST['mPanelID']) || (empty($fileArray['PDFFILE']['name']) AND $process_pdf=='' AND $_POST['curpdf']=='a') || empty($_POST['scsc_comboIDs']) || empty($_POST['co_comboIDs'])){
            $redirect_url1 = "addproduct_latest.php?new=1&error_req=1";
            if($updID!=''){
             $redirect_url1 = "addproduct_latest.php?id=$updID&error_req=1";
            }
            ob_end_clean();
...
//                }
       ...
       /*####### START NEW PROMOTION FIELD #########*/
        ...
        /*####### END NEW PROMOTION FIELD #########*/
        /*###############START Direct Mail advertised##############*/
        if($sectorID!='' AND trim($mChannelID)!='' AND trim($mPanelID)!=''){
            $check_sectorid=explode(',',$sectorID);
            $check_categoryid=explode(',',$categoryID);
            $check_subcategoryid=explode(',',$subCategoryID);
            if(($mChannelID==1) AND ($mPanelID==1 || $mPanelID==2 ) AND (in_array(266,$check_sectorid) || (in_array(4,$check_sectorid) && in_array(454,$check_categoryid) && in_array(506,$check_subcategoryid)))){
               if($advertiser_address!='' || $advertiser_city!='' || $advertiser_state!='' || $advertiser_zipcode!='' ||$advertiser_phone_number!='' || $advertiser_url){
                    $query_advertiser_delete = "DELETE FROM cscan_advertisers WHERE productID='".$DRW->real_escape_string($updID)."'";
                    $DRW->query($query_advertiser_delete, $DRW_main); 
                    $sqlAdvertiserInsert = " INSERT INTO  cscan_advertisers set
                                       productID='".$DRW->real_escape_string($updID)."',
                                       address='".$advertiser_address."',
                                       city='".$advertiser_city."',
                                       stateID='".$advertiser_state."',
                                       zip_code='".$advertiser_zipcode."',
                                       phone_number='".$advertiser_phone_number."',
                                       url='".$advertiser_url."'";                        

                             $DRW->query($sqlAdvertiserInsert, $DRW_main);
               }
            }else{
                $query_advertiser_delete = "DELETE FROM cscan_advertisers WHERE productID='".$DRW->real_escape_string($updID)."'";
                $DRW->query($query_advertiser_delete, $DRW_main); 
        ...
        if($sectorID=='' || trim($mChannelID)=='' || trim($mPanelID)==''){
            $query_advertiser_delete = "DELETE FROM cscan_advertisers WHERE productID='".$DRW->real_escape_string($updID)."'";
            $DRW->query($query_advertiser_delete, $DRW_main);  
        }
        /*###############END Direct Mail advertised##############*/
        $error_pdf2=0;
        if (empty($fileArray['PDFFILE']['name']) AND $process_pdf=='') {
            ...
            if(empty($fileArray['PDFFILE']['name']) AND $process_pdf!=''){
                $error_pdf2 = uploadPdfFromProcessPdf($updID, $fileArray, $PDFContent, $productStatus,$process_pdf);
       ...
       /* if ($productHeadline != '' && $rowcount != 0) {
        ...
        if ($error_pdf > 0) {
            ...
            $sqlUpdt = "Update cscan_product_detail set productStatus=2 WHERE productID='".$updID."'";
            $DRW->query($sqlUpdt, $DRW_main);
            $redirect_url = "addproduct.php?id=$updID&error_pdf2=$error_pdf2";
            ...
            if ($productStatus == 1 && ($productStatus != $old_productStatus || $new_preview)) {
                $redirect_url = "managepdfSample_latest.php?productID=$updID&next=manageproduct.php";
        ...
        if(!empty($company)){
            $exludeCompany = ['liberty mutual', 'nationwide','Anthem BlueCross','Anthem BlueCross BlueShield','Arkansas BlueCross BlueShield','BlueCross BlueShield of Alabama','BlueCross BlueShield of Arizona','BlueCross BlueShield of Delaware','BlueCross BlueShield of Florida','BlueCross BlueShield of Georgia','BlueCross BlueShield of Illinois','BlueCross BlueShield of Kansas','BlueCross BlueShield of Kansas City','BlueCross BlueShield of Louisiana','BlueCross BlueShield of Massachusetts','BlueCross BlueShield of Michigan','BlueCross BlueShield of Minnesota','BlueCross BlueShield of Mississippi','BlueCross BlueShield of Missouri','BlueCross BlueShield of Montana','BlueCross BlueShield of Nebraska','BlueCross BlueShield of New Mexico','BlueCross BlueShield of North Carolina','BlueCross BlueShield of North Dakota','BlueCross BlueShield of Oklahoma','BlueCross BlueShield of Rhode Island','BlueCross BlueShield of South Carolina','BlueCross BlueShield of Tennessee','BlueCross BlueShield of Texas','BlueCross BlueShield of Vermont','BlueCross BlueShield of Western New York','BlueCross BlueShield of Wisconsin','BlueCross BlueShield of Wyoming','Delta Dental','Highmark BlueCross BlueShield of Delaware','Highmark BlueCross BlueShield of West Virginia','HMSA BlueCross BlueShield of Hawaii','Horizon BlueCross BlueShield of New Jersey','Mountain State BlueCross BlueShield','Premera BlueCross BlueShield of Alaska','Regence BlueCross BlueShield of Oregon','Regence BlueCross BlueShield of Utah','Wellmark BlueCross BlueShield of Iowa','Wellmark BlueCross BlueShield of South Dakota'];
...
/*###############START Direct Mail advertised##############*/
if(!empty($updID)){
    $sql_advertiser_select = "SELECT address,city,stateID,zip_code,phone_number,url FROM cscan_advertisers WHERE productID=".$updID; //primary document
    ...
    $advertiser_state=$row_advertiser[2];
            ...
            if (is_uploaded_file($pdfTempNameArr)) {
                ...
                if (move_uploaded_file($pdfTempNameArr, $pdfPath . $pdfName)) {
                    ...
                    if ($filekey == 'PDFFILE') {                        
                        ...
                        /*################## Start PHP PDF TO IMAGE DATE 22-06-2020############*/
                        /*if(ENV == 'localhost' || ENV == 'demo.competiscan.com'){
                           // saveImagefromPdf($pdfs3newPath, $pdfName, $productID);
                            createPreviewJPGWithPythonCode($pdfPath, $pdfName, $productID, $back);
                        }else{
                            $cr= createPreviewJPG($pdfPath, $pdfName, $productID, $back);
                        }*/
                       /*################## END PHP PDF TO IMAGE DATE 22-06-2020############*/
                        /*####################### START ADD PRODUCT PDF OPTMIZIZE ######################*/
                        if($mChannelID==6){
                           createPreviewJPG1($pdfPath, $pdfName, $productID, $back); 
                        }else{
                         createPreviewJPGWithPythonCode($pdfPath, $pdfName, $productID, $back);
                        }
...
//For process pdf
function uploadPdfFromProcessPdf($productID, $fileArray, $pdfContent, $productStatus = 0,$muid='') {
    $AUTH_DATA = $GLOBALS['AUTH_DATA'];    
    global $s3,$bucket_name,$DRW, $DRW_read, $DRW_main, $DRW_crm;
    if($muid!=''){
        //$pdf_url='https://html-prod.competiscan.com:5447/temppdf/'.$muid;
        //$pdf_url='https://html-pdf.competiscan.com/temppdf/'.$muid;
        $pdf_url='https://api2.competiscan.com/html-pdf/v1/temppdf/'.$muid;
    }    
    $message = 0;
    $pdfSizeArr=0;
    $pdfNameArr='';    
    if(does_url_exists($pdf_url)){            
        $filekey='PDFFILE';
        $yearpath = date('Y/');
        $monthpath = date('m/');
        $datepath = $yearpath . $monthpath;
        ...
        $pdfSizeArr = get_fileSize_PDF($pdf_url); 
        $pdfNameArr = $muid.'.pdf';         
    }    
    if ($pdfNameArr != '') {        
        $valid_types = array("pdf");
        ...
        if (in_array($ext_type, $valid_types) || in_array($ext_name, $valid_types)) {  
            if($pdfSizeArr>0){
                ...
                if(savePdfFromProcessPdf($pdf_url,$pdfPath.$pdfName)){
                    $result = $s3->putObject([
                        'Bucket' => $bucket_name,
                        'Key'    => $pdfs3newPath . $pdfName,
...
//For process pdf
function savePdfFromProcessPdf($url,$path){
    $success=false;
    $ch     =   curl_init($url);   
    $fp     =   fopen($path, 'wb');
