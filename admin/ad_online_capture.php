...
if (isset($_REQUEST['delID']) && ($_REQUEST['delID'] > 0)) {
    ...
    for ($i = 0; $i < count($delID); $i++) {
        ...
        if ($move == 'Move to Online Display Filter') {          
    ...
    if ($movemsg == '1') {
        $message = "<b>$i</b> Record(s) has been moved in online filter.";
    }else if ($movemsg == '2') {
        $message = "<b>$i</b> Record(s) has been already processed.";
    } elseif ($delmsg == '1') {
...
$sql = "SELECT creative_id,creative_path,ad_md5,digital_channel,file_type,DATE_FORMAT(lastUpdated,'%m/%d/%Y - %h:%i %p') AS lastUpdated,DATE_FORMAT(capture_date,'%m/%d/%Y - %h:%i %p') AS capture_date1 FROM " . $selecttable . " WHERE digital_channel like '%Online Display%' AND capture_status=1 $where_pan ORDER BY capture_date DESC  LIMIT $p, $limit";
$rs = $DRW->query($sql, $DRW_digital);
$resultCount = $DRW->num_rows($rs);

$numquery = "Select COUNT(creative_id) as numrows FROM " . $selecttable . " WHERE digital_channel like '%Online Display%' AND capture_status=1 $where_pan";
...
<form method='post' name='frm1'>
    <table width='100%' border='0' cellspacing='0' cellpadding='5' class='text' rules='none' bordercolor='#B9B9B9' align='center' style='border-collapse:collapse'>
        <tr><td class="adminhead" align='center' colspan='7'>ONLINE DISPLAY CAPTURE PRODUCT</td></tr>
                    ...
                    <tr valign='top'>
                        <td align='right' colspan='2'>
                            <table width="100%" border="0" cellspacing="0" cellpadding="0" class="text">
                                ...
                                <tr>
                                    <td align='center' width="50%" style="padding-bottom: 10px;padding-top: 10px;"><input class='button'  type='submit' name='submit1' ID='delBt' value='Delete' onclick=' return confirmDel()'></td>

                                    <td align='left' width="50%"><input class='button'  type='submit' name='move' ID='moveBt' value='Move to Online Display Filter' onclick=' return confirmMove()'></td>
                                    <td align='right' width="35%"><a href="ad_online_display.php" style="text-decoration:none;"><input class='button' style='width:140px' type='button' name='submit1' ID='delBt' value='Go to Filter Page'></a><br /></td> 
                                </tr>
                            </table>
                        </td>
                    </tr>
        ...
        <tr>
            <td width='1%' class="adminhead" height='15px'><b><input type='checkbox' name='setUnset' onclick='setAll()'></td>
                    ...
                    <td align="right" width='10%' class="adminhead" height='10px' ><b>Source</td>
...
//                 $devicename ='Online Display';
                ...
                <tr valign=top class="<?php echo $className; ?>">
                    ...
                    <td>
                        <?php if (strstr(strtolower($creative_path), '.html')) { ?>
                            <iframe  border="0" src="fetchhtml_image.php?hid=<?php echo $ID; ?>&tbl=<?php echo $selecttable; ?>" width="350" height="100">
                            </iframe>
                    <?php } else { ?>
                    ...
                    <td align="right"><a style="text-decoration:none;" href="addproduct-digital.php?old=<?php echo $ID; ?>&add=1&tbl=<?php echo $creative_tblid; ?>"><input class='button' style='width:120px;' type='button' name='moveproduct' value='Move To Product'></a></td>
                </tr>
                <?php
            }
    ...
    <tr>
        <td colspan="5" align="left" style="padding-left:30px;">
            <form method='post' name='form2' id='form2'>
                ...
                Creative Table:
                <select name="creative_tables" onchange="form2.submit();">
...
<script type="text/JavaScript">
    ...
    { 
        ...
        if(goAheadFlag)
        {
            if(confirm("Are you sure to move these records into online filter?"))
            {
                return true;
            }
            else
        ...
        {
            alert('Please select at least one record to move in to filter !!!');
            return false;
        }
    }
