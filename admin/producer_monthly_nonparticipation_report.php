...
if (!empty($_REQUEST['clear']) && trim($_REQUEST['clear']) == 'Clear') {
    header("location:producer_monthly_nonparticipation_report.php");
}
$filter = array();
if (!empty($_REQUEST)) {
...
if (!empty($_REQUEST['export']) && trim($_REQUEST['export']) == 'Export') {//pr($_POST);die;
    $arrExport = array();    
    $exp_sql = "SELECT SQL_CALC_FOUND_ROWS cp.panelist_id, cp.competi_id,cp.first_name,cp.last_name,cp.address,cp.city,cp.state,cp.postalcode,cp.email FROM cscan_panelists cp $conditions AND competi_id NOT IN (SELECT competi_id from cscan_producer_scoring_monthly_reports $conditions2 ) AND panelist_id NOT IN(select panelist_id from cscan_removing_panelists where product_count<=0) AND cp.contact_type='prod_panelist' $order_bytxt $order_by ";
    ...
    download_send_headers("producer_monthly_nonparticipation_report" . date("Y-m-d") . ".csv");
...
$sql = "SELECT SQL_CALC_FOUND_ROWS cp.panelist_id, cp.competi_id,cp.first_name,cp.last_name,cp.address,cp.city,cp.state,cp.postalcode,cp.email FROM cscan_panelists cp $conditions AND competi_id NOT IN (SELECT competi_id from cscan_producer_scoring_monthly_reports $conditions2 ) AND panelist_id NOT IN(select panelist_id from cscan_removing_panelists where product_count<=0) AND cp.contact_type='prod_panelist' $order_bytxt $order_by LIMIT $p, $limit";
//echo $sql;
$rs = $DRW->query($sql, $DRW_read);
$numquery = $DRW->query("SELECT FOUND_ROWS()", $DRW_read); 
$nrow = $DRW->fetch_row($numquery);
$numrows = $nrow[0];
$sql_check = "SELECT count(*) FROM cscan_panelists Where active=1 And parent_panelist_id=0 AND panelist_id NOT IN(select panelist_id from cscan_removing_panelists where product_count<=0) AND contact_type='prod_panelist'";
...
<table width='100%' border='0' cellspacing='0' cellpadding='8' class='text' rules='none' bordercolor='#B9B9B9' align='center' style='border-collapse:collapse'>
    <tr><td class="adminhead" align='center'>Producer Non Participation Monthly Report</td></tr>
    <tr><td align='right'>

            <a href="producer_scoring_monthly_reports.php" class="button" style="padding:5px;text-decoration: none;float:right;margin-right: 10px; color: #fff;">Back</a>
