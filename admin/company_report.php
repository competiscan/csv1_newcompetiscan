...
if(!empty($_POST['export']) && trim($_POST['export']) == 'Export'){
    $arrExport = array();
    $exp_sql = "SELECT SQL_CALC_FOUND_ROWS em.user_id, (CASE WHEN firstName != '' AND lastName != '' THEN CONCAT(firstName, ' ', lastName) WHEN firstName != '' AND lastName = '' THEN firstName WHEN firstName = '' AND lastName != '' THEN lastName ELSE emailAddress END) AS userName, SUM(em.is_send) AS is_send,SUM(em.is_opened) AS is_opened, SUM(em.is_clicked) AS is_clicked FROM cscan_email_track AS em INNER JOIN cscan_users AS u ON (em.user_id=u.userID) WHERE LCASE(company_name)='".strtolower($company_name)."' AND DATE_FORMAT(insert_date, '%Y-%m-%d') BETWEEN '".$fdt."' AND '".$tdt."' AND is_send = 1 GROUP BY user_id ORDER BY $order_by";
    ...
    if(!empty($exp_numrows)){        
        //total email sent
        $exp_total_sql = "SELECT count(id) AS total_sent, SUM(is_opened) AS total_opened, SUM(is_clicked) AS total_clicked FROM cscan_email_track WHERE LCASE(company_name)='".strtolower($company_name)."' AND DATE_FORMAT(insert_date, '%Y-%m-%d') BETWEEN '".$fdt."' AND '".$tdt."' AND is_send = 1";
        ...
        $arrExport['total'] = array("Company Name: $company_name","Total E-mail Sent: $exp_total_sent", "Total Email Opened: $exp_total_opened", "Total Email Clicked: $exp_total_clicked");
        $arrExport['data'][] = array("User Name", "E-mail Sent", "E-mail Opened", "E-mail Clicked");
        while($exp_row = $DRW->fetch_assoc($exp_rs)){
            $arrExport['data'][] = array($exp_row['userName'], $exp_row['is_send'], $exp_row['is_opened'], $exp_row['is_clicked']);
        }
    }
    download_send_headers("company_report_" . date("Y-m-d") . ".csv");
...
if(empty($company_name)){
    ...
    function doSort($sort, $order, $dosort, $p, $company_name,$fdt, $tdt, $spacer = ' : ') {
        if(empty($order)){
            $order = 1;
        }else{ 
            $order = 0;
        ...
        if ($sort == ($dosort * -1) || $sort != $dosort) {
            print "$spacer<a href=\"" . $_SERVER['PHP_SELF'] . "?cname=$company_name&fdt=$fdt&tdt=$tdt&sort=$dosort&order=$order&p=$p\" class=\"blue\">sort</a>";
        } else {
            print "$spacer<a href=\"" . $_SERVER['PHP_SELF'] . "?cname=$company_name&fdt=$fdt&tdt=$tdt&sort=$dosort&order=$order&p=$p\" class=\"blue\">sort</a>";
        }
    }
    $sql = "SELECT SQL_CALC_FOUND_ROWS em.user_id, (CASE WHEN firstName != '' AND lastName != '' THEN CONCAT(firstName, ' ', lastName) WHEN firstName != '' AND lastName = '' THEN firstName WHEN firstName = '' AND lastName != '' THEN lastName ELSE emailAddress END) AS userName, SUM(em.is_send) AS is_send,SUM(em.is_opened) AS is_opened, SUM(em.is_clicked) AS is_clicked FROM cscan_email_track AS em INNER JOIN cscan_users AS u ON (em.user_id=u.userID) WHERE LCASE(company_name)='".strtolower($company_name)."' AND DATE_FORMAT(insert_date, '%Y-%m-%d') BETWEEN '".$fdt."' AND '".$tdt."' AND is_send = 1 GROUP BY user_id ORDER BY $order_by LIMIT $p, $limit";
...
<table width='100%' border='0' cellspacing='0' cellpadding='8' class='text' rules='none' bordercolor='#B9B9B9' align='center' style='border-collapse:collapse'>
    <tr><td class="adminhead" align='center'>Company Report <span style="float:right;"><a href="email_track_report.php?<?php echo $_SERVER['QUERY_STRING'];?>" style="color:#fff;"><< Back</a></span></td></tr>
    ...
    if(!empty($numrows)){
        //total email sent
        $total_sql = "SELECT count(id) AS total_sent, SUM(is_opened) AS total_opened, SUM(is_clicked) AS total_clicked FROM cscan_email_track WHERE LCASE(company_name)='".strtolower($company_name)."' AND DATE_FORMAT(insert_date, '%Y-%m-%d') BETWEEN '".$fdt."' AND '".$tdt."' AND is_send = 1";
            ...
            <table width='100%' border='0' cellspacing='0' cellpadding='8' class='text' rules='none' bordercolor='#B9B9B9' align='center' style='border-collapse:collapse'>
                ...
                <tr>
                    <td colspan="4">
                        <table width='100%' border="0" cellspacing="0" cellpadding="1" class="text">
                            <tr>
                                <td align="left" width='40%'><strong>Company Name: <?php echo $company_name;?></strong></td>
                ...
                <tr>
                    <td align="left" width='25%' class="adminhead" height='15px' ><b>User Name</b><?php doSort($sort,$order,1,$p,$company_name,$fdt,$tdt); ?></td>
                    <td align="left" width='25%' class="adminhead" height='15px' ><b>E-mail Sent</b><?php doSort($sort,$order,2,$p,$company_name,$fdt,$tdt); ?></td>
                    <td align="left" width='25%' class="adminhead" height='15px' ><b>E-mail Opened</b><?php doSort($sort,$order,3,$p,$company_name,$fdt,$tdt); ?></td>
                    <td align="left" width='25%' class="adminhead" height='15px' ><b>E-mail Clicked</b><?php doSort($sort,$order,4,$p,$company_name,$fdt,$tdt); ?></td>
                </tr>
                <?php
                $className='';
                while($row = $DRW->fetch_assoc($rs)){
                    ...
                    <tr valign=top class="<?php echo $className; ?>">
                        <td><a href="user_report.php?uid=<?php echo $row['user_id'];?>&fdt=<?php echo $fdt?>&tdt=<?php echo $tdt?>&cname=<?php echo urlencode($company_name);?>"><?php echo $row['userName'];?></a></td>  
                        <td align="left"><?php echo $row['is_send'];?></td>                             
                        <td align="left"><?php echo $row['is_opened'];?></td>
                        <td align="left" colspan="2"><?php echo $row['is_clicked'];?></td>
                    </tr>
                <?php }?>
            </table>
        </td>
            ...
            <table border="0" width="100%" cellspacing = "0"  cellpadding ="7">
                ...
                for($i=$loopstart; $i<$loopend; $i++){
                    ...
                    if($startnum!=$limstart) {
                        $middlelinks .= "<a href=\"{$_SERVER['PHP_SELF']}?p=$startnum&cname=$company_name&fdt=$fdt&tdt=$tdt&order=$order\">".($i+1)."</a> ";
                    }
                ...
                if($limstart<$rowcnt && (($limstart+($limiter*2))<$rowcnt || ($rowcnt - ($limstart + $limiter))>0)){
                    ...
                    $nextlink = "<a href=\"{$_SERVER['PHP_SELF']}?p=$next&cname=$company_name&fdt=$fdt&tdt=$tdt&order=$order\">Next $limiter &raquo;</a>";
                    $lastlink = "[<a href=\"{$_SERVER['PHP_SELF']}?p=".(($numbers-1)*$limiter)."&cname=$company_name&fdt=$fdt&tdt=$tdt&order=$order\">Last</a>]";
                }
