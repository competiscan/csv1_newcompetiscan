       ...
       if(isset($_REQUEST['old'])){
        ...
        if (isset($_REQUEST['add']) && $_REQUEST['add'] == '3') {
            ...
            if(isset($_REQUEST['id'])){
                ...
                $pdftext = '';
                $sql2 = "SELECT SQL_NO_CACHE document_id,document_filename,document_size_byte,document_path FROM cscan_document WHERE productID=$updIDs AND document_id=1";
                $rs2 = $DRW->query($sql2, $DRW_read);
                $row2 = $DRW->fetch_row($rs2);
            ...
            if ($updID != '') {
                $pdftext = '';
                $sql2 = "SELECT SQL_NO_CACHE document_id,document_filename,document_size_byte,document_path,document_content_type FROM cscan_document WHERE productID=$updID AND document_id=1";
                $rs2 = $DRW->query($sql2, $DRW_read);
                $row2 = $DRW->fetch_row($rs2);
                $document_id = (int) $row2[0];
                $document_filename = $row2[1];
...
if ($fromtemp) {
    ...
    $displayArray[$dak][$dai]['value'] .= '<input class="button" type="submit" name="prob" value="Problem" onclick="return doSaved(4);" /> &nbsp; <input class="button" type="button" value="Cancel" onclick="location.href=\'manageproduct-digital.php\'; return false;" />';
    ...
    if ($updID != '') {
        $displayArray[$dak][$dai]['value'] .= ' &nbsp; <input class="button" type="button" value="Delete" onclick="doProdDelete_digital(' . $updID . '); return false;" />';
    }
