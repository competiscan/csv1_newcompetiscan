...
include_once __DIR__ . '/../includes/clean.php';
...
if (isset($_POST['save'])) { 
 ...
 //#####################SHOW PREVIEW IN HTML####################//
       /* $query = "SELECT `esproduct`,`muid` FROM `cscan_email_save` WHERE `muid`='".$DRW->real_escape_string($_REQUEST['muid'])."'";
        $result = $DRW->query($query, $DRW_read);
        if ($DRW->num_rows($result) > 0) {
                $data2 = $DRW->fetch_row($result);
                $cettext = $data2[0];
                $muid = $data2[1];
                $html_content= html_entity_decode($cettext);
                CreateHtmlFileUpload($productID='9876541',$html_content,$productStatus = 0,$filetype='html');
...
//                }
      ...
      $query = "SELECT `esproduct`,`muid` FROM `cscan_email_save` WHERE `muid`='".$DRW->real_escape_string($_REQUEST['muid'])."'";
        ...
        if ($DRW->num_rows($result) > 0) {
                ...
                $muid = $data2[1];
                $html_content= html_entity_decode($cettext);
                $create_html= CreateHtmlFileUpload($updID,$html_content,$productStatus,$filetype='html');
       ...
       /* if ($productHeadline != '' && $rowcount != 0) {
        ...
        if ($error_pdf > 0) {
            $redirect_url = "addproduct_test.php?id=$updID&error_pdf=$error_pdf";
        } elseif ($error_media > 0) {
            $redirect_url = "addproduct_test.php?id=$updID&error_media=$error_media";
        } elseif ($img_error > 0) {
            $redirect_url = "addproduct_test.php?id=$updID&img_err=1";
        } elseif (isset($_POST['saveAndAdd'])) {
            $redirect_url = "addproduct_test.php?new=1&more=1";
        } else {
...
//#####################SHOW PREVIEW IN HTML####################//
function CreateHtmlFileUpload($productID,$html_content,$productStatus = 0,$filetype='html') {
   ...
   // $productID='123456789';
    ...
    $htmlpart = $root . '/PDF/';
    $htmlPath = "$htmlpart$datepath$productID/";
     if (!is_dir($htmlpart . $yearpath)) {
        mkdir($htmlpart . $yearpath, 02755);
    }
    if (!is_dir($htmlpart . $datepath)) {
        mkdir($htmlpart . $datepath, 02755);
    }
    if (!is_dir($htmlPath)) {
        mkdir($htmlPath, 02755); 
    } 
     $file_name="$productID$file_ext";
     $htmlPath1 = "$htmlpart$datepath$productID/$file_name"; 
    $htmls3newPath = "PDF/$datepath$productID/";
    
    $fp = fopen($htmlPath1,"w");
   ...
   if ($filetype == 'html') {
     ...
     $result = $s3->putObject([
        'Bucket' => $bucket_name,
        'Key'    => $htmls3newPath . $file_name,
        'SourceFile' => $htmlPath1,
        'ACL'    => 'public-read',
  ...
  /*############### Create image from html file and upload at s3 ###############*/
     ...
     $html_url=$serverbaseurl.'/'.$htmls3newPath.$file_name;
     $output_path=$serverbaseurl.'/'.$htmls3newPath.$productID.'0.jpg';
     $s=exec("wkhtmltoimage --zoom 0.2 --height 400 --width 300 ".$html_url.' '.$output_path." 2>&1", $output);
   ...
   /*echo "<pre>";
   print_r($result); exit; */
     /*if (file_exists($htmlPath.$file_name) ) {
  ...
  if ($productStatus == 1) {
   ...
   /* if ($filetype == 'html') {
      $cr= createPreviewJPG($htmlPath, $file_name, $productID, $back);
        $content_type = "text/html";

    }  die;
*/
          $document_id = saveHtmlData($productID, $htmlPath, $file_name, $html_content, '/PDF/' . $datepath . $productID . '/', false, $content_type);
          $GLOBALS['new_preview'] = true;
} 
//#####################SHOW PREVIEW IN HTML####################//

//#####################SHOW PREVIEW IN HTML####################//
function saveHtmlData($productID, $pdfPath, $pdfName, $pdfContent = "", $document_path = '', $do_original = false, $document_content_type = 'text/html') {
    global $DRW, $DRW_read, $DRW_main, $DRW_crm;
    global $s3,$bucket_name,$serverbaseurl;
    $maxbytes = 500000;
    ...
    if ($did != 0) {
        ...
        $sql = "UPDATE cscan_document SET document_placement='$document_placement', document_filename='" . $DRW->real_escape_string($pdfName) . "',document_createddate=NOW(),document_content_type='" . $DRW->real_escape_string($document_content_type) . "',document_size_byte=$document_size_byte,document_createdby={$GLOBALS['AUTH_DATA']['userID']},document_path='" . $DRW->real_escape_string($document_path) . "'
...
//                 echo '<br><br><br><br> ';
    ...
    if ($document_content_type == "text/html") {
        ...
        //echo $pdfContent;die;
        saveHtmlText($productID, $document_id, $pdfContent, $maxbytes);
        if (is_file($file)) {
            //echo ' pppppp22';
           // echo '<br><br>';
           unlink($file);
...
function saveHtmlText($productID, $document_id, $pdfContent = '', $maxbytes = 500000) {
    global $DRW, $DRW_read, $DRW_main, $DRW_crm,$htmldom;
    ...
    $pdfContent= cleanHTML($pdfContent); 
     $htmldom   =   str_get_html($pdfContent);  
     
    $pdfContent=$htmldom->plaintext;
    $pdfContent = html_entity_decode(strip_tags($pdfContent));
    $pdfContent= trim(preg_replace("/[^a-zA-Z0-9\s]/", " ", $pdfContent));
    $pdfContent = preg_replace('/\s+/', ' ', $pdfContent);
    //echo $pdfContent= trim(preg_replace("/[^A-Za-z0-9\ -@$%&©)(]'/", '', $pdfContent));exit;
    //$wrap = wordwrap($pdfContent, $maxbytes, "\n");
   // $stringArray = preg_split('/\\n/', $wrap, -1, PREG_SPLIT_NO_EMPTY);
     $document_text_part = 1;
    //echo"<br>stringarray===>";
    //print_r($stringArray);
    $sql = "INSERT INTO cscan_document_text_search (productID,document_id,dts_val,dts_part) values ($productID,$document_id,'" . $DRW->real_escape_string($pdfContent) . "',$document_text_part)";
    $DRW->query($sql, $DRW_main);
