...
[<a href="manageUserAccounts.php">Manage IP Addresses by Count</a>] &nbsp; [<a href="manageUserLog.php">Manage IP Addresses by Date</a>] &nbsp; [<a href="manageUserLog3.php">Logins by Browser</a>] &nbsp; [<a href="manageUserLog4.php">Logins by IP Address and Browser</a>] &nbsp; [<a href="manageUserLog5.php">Logins by IP Location</a>]
<?php
...
FROM 
(SELECT t1.userID,COUNT(*) as cnt FROM (SELECT DISTINCT userID,LEFT(date,10) as ldate,LEFT(IPAddress,9) FROM cscan_user_tracker) as t1
...
while($data = $DRW->fetch_row($result)){
	...
	$query2 = "SELECT t1.ldate,COUNT(*) as cnt FROM (SELECT DISTINCT LEFT(date,10) as ldate,LEFT(IPAddress,9) FROM cscan_user_tracker WHERE userID=$userID) as t1
	...
	while($data2 = $DRW->fetch_row($result2)){
		...
		$currip = '';
		$query3 = "SELECT DISTINCT IPAddress,DATE_FORMAT(loginTime,'%l:%i %p') FROM cscan_user_tracker WHERE userID=$userID AND date='$ldate' ORDER BY loginTime";
		...
		while($data3 = $DRW->fetch_row($result3)){
			...
			if($IPAddress!=$currip){
				...
				$ip = '<br /><a href="#" onclick="var win = window.open(\'ip_location.php?ip='.urlencode($IPAddress).'\',\'ipdetail\',\'toolbar=no, menubar=no, location=no, status=no, scrollbars=yes, resizable=yes, width=500, height=400\'); win.focus(); return false;">'.$ipLocation[$IPAddress].'</a> '.$ip;
			}
			else continue;
