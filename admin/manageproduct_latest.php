...
//    $numrow = (int) $data[0];
    ...
    if (!empty($SPHINX_name)) {
        ...
        if($_SESSION['pstat']==1){
            //$inds = 'base_index_' . $SPHINX_name . ',delta_index_' . $SPHINX_name;
            $inds = 'base_index_prod_approved,delta_index_prod_approved';
        }else{
...
//                            $DRW->query($query, $DRW_main);
               ...
               if ($search_id != '') {
                    ...
                    if (!empty($productidsarray)) {
                       ...
                       else if($total>900000 && $total<=100000){
                                $chunkdata=90000;
                        }
...
?>
				...
				WHERE pa.companyID=pp.companyID AND pp.productID=$productID AND primary_co=1", $DRW_read);
                            ...
                            ?>>
                                <td valign="top"><?php
                            if (checkGroup(23) || ($actual_addedToDatabase == date('m/d/Y') && $admin_userID == $AUTH_DATA['userID']))
