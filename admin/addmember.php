...
if(isset($_REQUEST['id'])) {
	...
	if( $updID != '' ) {
		...
		if( $DRW->num_rows( $result ) > 0 ) {
			...
			$emailAddress = strtolower(trim($row['emailAddress']));
...
if(isset($_POST['save'])) {
	...
	$old_emailAddress = strtolower(trim($_POST['old_emailAddress']));
	...
	################# Start for Sender Domain Permission################
	//if(ENV == 'localhost' || ENV == 'demo.competiscan.com'){ 
	$sender_domain_allow=array();
	 if(isset($_POST['sender_domain_allow'])){
		 $sender_domain_allow = $_POST['sender_domain_allow'];
	 }
	//}
	################# End Sender Domain Permission################
	...
	if( $DRW->num_rows($result) == 0 ) {
		...
		if($DRW->num_rows($chk_result) == 0) {
			...
			if(isset($_REQUEST['updateInterest']) && $_REQUEST['updateInterest']!=''){
				...
				#############Sync User_CSV2_log#############
				// $sql_syn_user_log = "insert INTO cscan_users_sync (userID,status) VALUES ('".$updID."','2')";
				// $DRW->query($sql_syn_user_log,$DRW_main);
				...
				foreach($mpAllow as $m){
					...
					$DRW->query($sql,$DRW_main);
                               ...
                               ################# End for Additional Fields ################
                				...
                				//if(ENV == 'localhost' || ENV == 'demo.competiscan.com'){ 
									if(!empty($sender_domain_allow)){
										$sql_sender_domain = "DELETE FROM cscan_user_sender_domain_allowed WHERE userID=$u";
										$DRW->query($sql_sender_domain,$DRW_main);
										foreach($sender_domain_allow as $sender_domain){
											$sqlsenderpage = "insert INTO cscan_user_sender_domain_allowed (name,userID) VALUES ('".$sender_domain."',$u)";
											$DRW->query($sqlsenderpage,$DRW_main);
										}
									}else{
										$sql_sender_delete = "DELETE FROM cscan_user_sender_domain_allowed WHERE userID=$u";
										$DRW->query($sql_sender_delete,$DRW_main);
									}  
								//}                              
                                ################# End for Sender Domain Permission ################
			...
			if(isset($_REQUEST['updateInterest']) && $_REQUEST['updateInterest']!='' && $updID!= ''){
				$payload = json_encode([
					"user_id" => $updID
				]);
				$apiuserurl= USER_LOGIN_API_URL_PROD.'user-permission-sync';
		   ...
		   ############ START REALTIME USER added AND UPDATED API#############
			if($updID!= '') {
				...
				$apiuserurl= USER_LOGIN_API_URL_PROD.'user-sync';
				$getuserdata= callAPI('POST', $apiuserurl, $payload);
				$resuserdata = json_decode($getuserdata, true);
				if($resuserdata['code']==200){
				}
...
<table width="100%" border="0" cellspacing="0" cellpadding="5" class="text">
  ...
  //if(ENV == 'localhost' || ENV == 'demo.competiscan.com'){
   ...
   if($updID!=''){ 
       ?>
    <tr><td align="right">
            <form id="myFormLogin" action="../login_csv1.php" method="post" target="_blank">
            ...
            <!-- Start for sender domain Permission -->
			 <?php 
                ...
                <tr>
                    ...
                    <td class="bodytext">
                    <fieldset style="border-color:#000000;border-width:1px;">
                    <legend><strong>Sender Domain Name Permission</strong></legend>
                    <div>
                    <?php
                    $senderDomainAllow = array();
                    if($updID!=''){
                        $sql_sender = "SELECT DISTINCT name FROM cscan_user_sender_domain_allowed where userID='".$updID."'";
                        $rsdata = $DRW->query($sql_sender,$DRW_read );
                        while($row_data = $DRW->fetch_array($rsdata)) {
                            $senderDomainAllow[] = $row_data[0];
                        }
                    }
                    
                    
                    $senderdomaindataArray = array('sender_domain_permission'=>'Sender Domain Permission');   
                    foreach($senderdomaindataArray as $senderdomainid=>$senderdomainname ) {
                        echo "<div><label><input type=\"checkbox\" name=\"sender_domain_allow[]\" value=\"$senderdomainid\"";
                        if(in_array($senderdomainid,$senderDomainAllow) OR $updID==''){
                        echo ' checked="checked"';
                        }
                        echo " />".htmlspecialchars($senderdomainname).'</label></div>';
                    }
                    ?>
                    </div>
                    </fieldset>
