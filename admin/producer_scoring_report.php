...
if (!empty($_POST['directmail_point'])) {
    ...
    foreach ($_POST['directmail_point'] as $key => $val) {
        $sqlhistory = "INSERT INTO cscan_producer_scoring_report_history (id,panelist_id,competi_id,parent_panelist_id,direct_mail_piece,direct_mail_point,
                                    ...
                                    email_piece,email_piece_point,bonus_point,total_point,bag_remaining,entry_date,bagupdate_by,bagupdate_date,reset_by,reset_date,add_bonus_point_date,add_bonus_point_by
                                    FROM   cscan_producer_scoring_report_total
                                WHERE  id ='" . $key . "'
                                "; 
        ...
        $updatesql = "update cscan_producer_scoring_report_total set
        ...
        $sqlselpanelist    =   "SELECT panelist_id FROM cscan_producer_scoring_report_total
                                WHERE  id ='".$key."'
                            ";
    ...
    foreach ($_POST['remainingbags'] as $key => $val) {
        ...
        if ($DRW->num_rows($rsselect) > 0) {
            ...
            if ($val != $rowsel['bag_remaining']) {
                ...
                $updatesl = "update cscan_producer_scoring_report_total set
                                      bag_remaining='" . $val . "'
                                      $addsql   
                                      where id = '" . $key . "'
                                    ";
        ...
        $msg=1;
        //header("location:producer_scoring_report.php?msg=2");
    }
}
...
function resetdata($id) {
    ...
    $sqlrest = " SELECT id,panelist_id, competi_id, direct_mail_piece, 
                ...
                FROM cscan_producer_scoring_report_total where panelist_id = '" . $id . "'";
...
if (!empty($_REQUEST['clear']) && trim($_REQUEST['clear']) == 'Clear') {
    ...
    $tdt = date('Y-m-d');
    header("location:producer_scoring_report.php");
}
...
if (!empty($_REQUEST['export']) && trim($_REQUEST['export']) == 'Export') {//pr($_POST);die;
    ...
    $exp_sql = "SELECT SQL_CALC_FOUND_ROWS id, psrt.panelist_id, psrt.competi_id,direct_mail_point, email_piece, email_piece_point, 
        ...
        FROM cscan_producer_scoring_report_total psrt left join cscan_panelists cp on psrt.competi_id=cp.competi_id 
                 $conditions GROUP BY  
...
?>
   <tr>  <td align='right'>
           <a href="producer_scoring_monthly_report.php" class="button" style="padding:5px;text-decoration: none;float:right;margin-right: 10px; color: #fff;">Monthly Report</a>
                ...
                <table width='100%' border="0" cellspacing="0" cellpadding="10" class="text">
                    <tr>
                        ...
                        <td align="left" colspan="3"><strong>Search By:</strong>
                         ...
                         <!--<td align="right"  width='5%'><input class="button" style="width:60px;margin-right: 10px;" type="submit" name="import" value="Import" onclick="document.location='producer_score_import.php'; return false;"/>
                        </td>-->
                    </tr>
                    <tr>
                    ...
                    <tr>
                      <td align="right"  width='85%'><input class="button" style="width:60px;" type="submit" name="import" value="Import" onclick="document.location='producer_score_import.php'; return false;"/>
                        </td>  
                    ...
                    <table width='100%' border='0' cellspacing='0' cellpadding='8' class='text' rules='none' bordercolor='#B9B9B9' align='center' style='border-collapse:collapse'>
                        <tr><td colspan="15">&nbsp;</td>
                         ...
                         <tr>
                            ...
                            Direct Mail Point</a></b><?php doSort($sort, $order, 2, $p, $competi_id, $panelist_type); ?></td>
...
<script type="text/javascript">
    ...
    function openpopup(id) {
        var wind = window.open('producer_score_log.php?id=' + id, "winpop", "left=0, top=0, scrollbars=yes, resizable=yes, width=400, height=300");
        wind.focus();
    }
