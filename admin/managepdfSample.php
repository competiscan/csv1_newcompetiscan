...
if(isset($_GET['next'])){
	...
	<div>&nbsp;</div>
	<div><div><strong>Product Image:</strong></div><img src="../productImg.php?id='.$productID.'&amp;new='.date('YmdHis').'" border="0" style="border:solid 1px #000000;" id="prod_img" width="150" height="100" /></div>
	<div>&nbsp;</div>';
...
if($productID!=0){
	...
	if($document_id==1){
                               ...
                               if($document_content_type=='text/html'){
                                   ...
                                   /*################## Start PHP PDF TO IMAGE DATE 22-06-2020############*/
                                    /*if(ENV == 'localhost' || ENV == 'demo.competiscan.com'){    
                                        ...
                                        createPreviewJPG($imagePath,$document_filename,$productID,false,false,$document_id);
                                    }*/
                                   /*####################### START ADD PRODUCT PDF OPTMIZIZE ######################*/
                                    $sql_query = "select mChannelID from  cscan_product_detail where productID=$productID";   
                                    $sql_query_result = $DRW->query($sql_query,$DRW_read);
		...
		while($data2 = $DRW->fetch_row($query_result2)){
			...
			print '"><a href="'.$_SERVER['PHP_SELF'].'?productID='.$productID.$eget.'&amp;page='.$page.'"><img src="../pdfSample.php?id='.$productID.'&amp;page='.$page.'&amp;new='.$new.'" style="border:none;" /></a>';
			if($page==$img_document_sort) {
