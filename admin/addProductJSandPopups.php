...
function checkChannel(){
	...
	if(chan==1 || chan==2){
        ...
        //alert(chan);
        if(chan==3 || chan==1 || chan==2 || chan==6 || chan==14){
		...
		document.forms.prodForm['electronicID[]'].disabled = false;
                ...
                <?php }else{ ?>
                    //alert("OKKKKKKKK");
                    <?php if($updID=='' && !isset($_GET['muid'])) { ?>
                    document.getElementById("btnProdSaveAndAdd").disabled = true;
                    <?php } ?>
                    <?php if($updID!='') { ?>
                        document.getElementById("btnRep").disabled = true;
                     <?php } ?>
                    document.getElementById("btnSaver").disabled = true;
                    document.getElementById("btnUna").disabled = true;
                    document.getElementById("btnProb").disabled = true;
               ...
               <?php if($fromtemp) { ?>
                ...
                //alert("OKKKKKKKKELSE");
                <?php if($updID=='' && !isset($_GET['muid'])) { ?>
                    document.getElementById("btnProdSaveAndAdd").disabled = false;
                <?php } ?>
                <?php if($updID!='') { ?>
                    document.getElementById("btnRep").disabled = false;
                 <?php } ?>
                document.getElementById("btnSaver").disabled = false;
                document.getElementById("btnUna").disabled = false;
...
function htmlWin(pid) {
    ...
    5000); */
    var winy = window.open('https://api3.competiscan.com/html-pdf/v2/temppdf/'+pid,"pdf"+pid,"menubar=yes, status=no, location=no, toolbar=yes, left=0, top=0, scrollbars=yes, resizable=yes, width=900, height=600");
    winy.focus();
}
