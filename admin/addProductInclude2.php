...
$displayArray[$dak][$dai]['value'] .= '<div style="height:60px;width:380px;overflow-y:scroll;border:solid 1px #DDF9EE;" id="pans_scroll"><div id="pans">';
...
elseif((isset($_GET['muid']) && $competi_ids_tmp!='') || ($fromtemp && $muid!='')){
	foreach($p_idsArray as $k=>$p_id){
		...
		if(isset($tmp[1])){
			$p_date = trim($tmp[1]);
			/*if(!$fromtemp && isset($_GET['muid'])){
				//overwrite temp pieces with current date
				$pt = strtotime($p_date);
				if($pt<=$currentt && ($currentt-$pt)<172800 && count($p_idsArray)==1){//(86400*2)
					$p_date = date('Y-m-d H:i:s',$currentt);
				}
			}*/
		}
		else{
	...
	elseif(isset($_GET['muid']) && $pub_ids_tmp!=''){
		...
		foreach($pub_idsArray as $pub_id_date){
			...
			if($pub_id_date!=''){
				...
				/* //overwrite temp pieces with current date
				$pt = strtotime($publicationDatef);
				if($pt<=$currentt && ($currentt-$pt)<172800 && count($pub_idsArray)==1){//(86400*2)
					$publicationDatef = date('Y-m-d',$currentt);;
					$pub_date = preg_replace('/\\D+/','',$publicationDatef);
					$pub_id_date = $pub_id.'|'.$pub_date;
				}*/
...
if(!$fromtemp){	
	...
	if(isset($_GET['muid'])){
		...
		/* //overwrite temp pieces with current date
		$pt = strtotime($entryID);
		if($pt<=$currentt && ($currentt-$pt)<172800){//(86400*2)
			$entryID = date('Y-m-d',$currentt);
		}*/
	}
...
/*
$displayArray[$dak][$dai]['title'] = 'Last Seen';
$displayArray[$dak][$dai]['value'] = '<input class="input_box" name="lastSeen" type="text" value="'.$lastSeen.'" size="15" readonly="readonly"'.$disabled.' />';
$disfields[] = 'lastSeen';
if(!isset($_GET['muid']) || $fromtemp) {
	$displayArray[$dak][$dai]['value'] .= ' <a href="#" onclick="displayCalendar(document.prodForm.lastSeen,\'yyyy-mm-dd\',this); return false;"><img name="popcal2" src="js_calendar/images/getcal.gif" border="0" alt="" style="vertical-align:bottom;" /></a>';
}
$dai++;*/
...
$dai++;
/*$displayArray[$dak][$dai]['title'] = 'Affinity/Association';//<label for="affinityAssociation"></label>
$displayArray[$dak][$dai]['value'] = '<input type="checkbox" id="affinityAssociation" name="affinityAssociation" value="1"';
if($affinityAssociation==1) {
	$displayArray[$dak][$dai]['value'] .= ' checked="checked"';
}
$displayArray[$dak][$dai]['value'] .= $disabled;
$disfields[] = 'affinityAssociation';
$displayArray[$dak][$dai]['value'] .= ' />';
$dai++;*/
...
$displayArray[$dak][$dai]['title'] = 'Facebook Page Name/Twitter Handle';
$displayArray[$dak][$dai]['value'] = '<input type="text" name="social_media_name" size="40" maxlength="255" class="input_box" value="'.htmlspecialchars($social_media_name,ENT_QUOTES).'"';
if($mChannelID!=6) {
	$displayArray[$dak][$dai]['value'] .= ' disabled="disabled"';
}
else {
	$displayArray[$dak][$dai]['value'] .= $disabled;
	$disfields[] = 'social_media_name';
}
$displayArray[$dak][$dai]['value'] .= ' />';
$dai++;*/
$saveDIArray['mc_external'][] = $dak.'_'.$dai;
...
$dai++;
/*
$displayArray[$dak][$dai]['title'] = 'Military';
$displayArray[$dak][$dai]['value'] = '<input type="checkbox" id="is_military" name="is_military" value="1"';
if($is_military==1) {
	$displayArray[$dak][$dai]['value'] .= ' checked="checked"';
}
$displayArray[$dak][$dai]['value'] .= $disabled;
$disfields[] = 'is_military';
$displayArray[$dak][$dai]['value'] .= ' />';
$dai++;
$displayArray[$dak][$dai]['title'] = 'Mover';
$displayArray[$dak][$dai]['value'] = '<input type="checkbox" id="is_mover" name="is_mover" value="1"';
if($is_mover==1) {
	$displayArray[$dak][$dai]['value'] .= ' checked="checked"';
}
$displayArray[$dak][$dai]['value'] .= $disabled;
...
$displayArray[$dak][$dai]['value'] .= ' /><input type="hidden" name="consumer_insights" value="'.$consumer_insights.'" /><input type="hidden" name="is_subp" value="'.$is_subp.'" />';
$dai++;
...
$displayArray[$dak][$dai]['title'] .= '>Sign-on </span>Incentive';
...
$displayArray[$dak] = array();
$displayKeys[$dak] = '';
/*$displayArray[$dak][$dai]['title'] = 'Affinion';
$displayArray[$dak][$dai]['value'] = '<label><input type="radio" name="is_affinion" value="1"';
if($is_affinion=='1') {
	$displayArray[$dak][$dai]['value'] .= ' checked="checked"';
}
$displayArray[$dak][$dai]['value'] .= $disabled.' />Yes</label> &nbsp; <label><input type="radio" name="is_affinion" value="0"';
if($is_affinion=='0'){
	$displayArray[$dak][$dai]['value'] .= ' checked="checked"';
}
$displayArray[$dak][$dai]['value'] .= $disabled.' />No</label>';
$disfields[] = 'is_affinion';
$dai++;*/
...
if($fromtemp){
	...
	/*$displayArray[$dak][$dai]['title'] = 'Possible Variants:';
	$displayArray[$dak][$dai]['value'] = '';
	
	$poss_variants = $DRW->query("SELECT * FROM cscan_possible_variant WHERE productID = '$_GET[id]'",$DRW_read);
	while($poss_variants && $poss_variant = $DRW->fetch_assoc($poss_variants)) {
		$displayArray[$dak][$dai]['value'] .= "<a href=\"addproduct.php?id=$poss_variant[relProductID]\">Product $poss_variant[relProductID] ($poss_variant[probability]% match)</a> <br />";
	}	
	$dai++;*/
