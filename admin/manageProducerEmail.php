...
<tr><td class="adminhead" align='center' colspan='12'>Producer/Advisor Mail</td></tr>
...
if(isset($_REQUEST['delID']) && ($_REQUEST['delID'] > 0)) {
	...
	for($i=0;$i<count($delID);$i++){
		$delThis = $delID[$i];
		$sql_sel = "SELECT id FROM cscan_sbkc_producer WHERE id = '$delThis'";
		$res = $DRW->query($sql_sel,$DRW_read);
                
		if ($DRW->num_rows($res) > 0) {
			$sql="DELETE FROM cscan_sbkc_producer WHERE id = '$delThis'";			
                    /* Added for track on delete operation */
                        ...
                        $track_delete_data = [
                                ...
                                'delete_type' => 'SBKC Producer Mail',
                                'is_mobile' => isMobile(),
        ...
        /* Added for track on delete */
            if(count($emailData)>0){
                ...
                $html .= '</table>';
                sendDevAlert('Caution! Data Deleted From Producer/Advisor Mail',$html);
            }
...
$sql = "SELECT id,first_name,last_name,email,company,address,city,state,zip,phone,refered_by,inserted_date FROM cscan_sbkc_producer order by id DESC  LIMIT $p, $limit";
$rs = $DRW->query($sql,$DRW_read);
$resultCount = $DRW->num_rows($rs);


$numquery = "Select COUNT(id) as numrows FROM cscan_sbkc_producer";
