...
$limit = 50;
if (isset($_REQUEST['p']))
    $p = $_SESSION['manageCategory_pq'] = $_REQUEST['p'];
elseif (isset($_SESSION['manageCategory_pq']))
    $p = $_SESSION['manageCategory_pq'];
else
    $p = 0;
$sort = '';
$message = '';
...
$sql = "SELECT creative_id,creative_path,ad_md5,digital_channel,file_type, lastUpdated FROM " . $selecttable . " WHERE digital_channel='Online Display' AND file_type!='None' AND capture_status='0' AND created_date<= DATE_SUB(NOW(), INTERVAL 61 minute) $where_pan ORDER BY creative_id DESC  LIMIT $p, $limit";
$rs = $DRW->query($sql, $DRW_digital);
$resultCount = $DRW->num_rows($rs);
$numquery = "Select COUNT(creative_id) as numrows FROM " . $selecttable . " WHERE digital_channel='Online Display' AND file_type!='None' AND capture_status=0 AND created_date<= DATE_SUB(NOW(), INTERVAL 61 minute) $where_pan";
...
<form method='post' name='frm1'>
    <table width='100%' border='0' cellspacing='0' cellpadding='5' class='text' rules='none' bordercolor='#B9B9B9' align='center' style='border-collapse:collapse'>
        ...
        <tr><td class="adminhead" align='center' colspan='6'>ONLINE DISPLAY PRODUCT</td></tr>
                        ...
                        <td align='center' colspan='2'>
                            <table width="100%" border="0" cellspacing="0" cellpadding="0" class="text">
                                ...
                                <tr>
                                    ...
                                    <td align='right' width="15%"><a href="ad_online_capture.php" style="text-decoration:none;"><input class='button' style='width:180px' type='button' name='submit1' ID='delBt' value='Go to Capture Page'></a><br /></td>                                    
                                </tr>
                            </table>
                        </td>
                    </tr>
        ...
        <tr>
            <td width='1%' class="adminhead" height='15px'><b><input type='checkbox' name='setUnset' onclick='setAll()'></td>
                    <td width='40%' class="adminhead" height='15px' >&nbsp;</td>        
                    ...
                    <td align="right" width='10%' class="adminhead" height='15px' ><b>Device</td>
                    <td align="center" width='14%' class="adminhead" height='15px' ><b>Date Captured</td>
        </tr>
        ...
        if ($resultCount > 0) {
            ...
            while ($row = $DRW->fetch_array($rs)) {
                ...
                <tr valign=top class="<?php echo $className; ?>">
                    ...
                    <td><?php if (strstr(strtolower($creative_path), '.html')) { ?>
                        ...
                        <?php } ?> 
                      <!--  <iframe  border="0" src="<?php //echo $creative_path; ?>" width="350" height="100">
                          </iframe>     -->
                    </td>
...
<?php
/*
  function ShowDeviceBymd5($ad_md5){
  global $DRW, $DRW_read;
  $device = '';
  $sql = "select device from cscan_digital_observation where ad_md5='".$ad_md5."' order by device DESC limit 0,1";
