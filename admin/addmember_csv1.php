	...
	if( $DRW->num_rows($result) == 0 ) {
		...
		if($DRW->num_rows($chk_result) == 0) {
			if($updID == '') {
				...
				$update_sql = "UPDATE cscan_users SET firstName='".$DRW->real_escape_string($firstName)."', lastName='".$DRW->real_escape_string($lastName)."', emailAddress='".$DRW->real_escape_string($emailAddress)."', companyName='".$DRW->real_escape_string($companyName)."', clientName='".$DRW->real_escape_string($clientName)."', streetAddress='".$DRW->real_escape_string($streetAddress)."', city='".$DRW->real_escape_string($city)."', state='".$DRW->real_escape_string($state)."', country='".$DRW->real_escape_string($country)."', zipCode='".$DRW->real_escape_string($zipCode)."', phone='".$DRW->real_escape_string($phone)."', fax='$fax',number_machines='$number_machines',bypass='$bypass',plevel=$plevel
					WHERE userID='$updID'";  
      ...
      <td align="center">
		...
		?>
          ...
          <tr>
            ...
            <td><input type="text" name="emailAddress" size="40" maxlength="255" class="input_box" value="<?php echo htmlspecialchars($emailAddress,ENT_QUOTES);?>" /><input type="hidden" name="old_emailAddress" value="<?php echo htmlspecialchars($emailAddress,ENT_QUOTES);?>" /></td>
          </tr>
          <!-- Password -->
          <tr>
            ...
            <?php /*if(!defined('ENV')){
					define('ENV',getenv('SERVER_NAME'));
			   	  ...
			   	  if(ENV == 'localhost' || ENV == 'demo.competiscan.com'){*/
			   	  	if(!empty($password)){
			   	  ?>
			   	  	<td><a href="javascript:void(0)" id="reset-password" style="font-size: smaller;font-size:13px;color:red;">Click Here</a><span style="font-size: 15px;"> to send reset password</span></td>
			<?php }else{
                            $password=generateRandomPassword();
                            ?>
				<td class="bodytext"><input type="text" id="input_password" name="password" size="40"  maxlength="40" class="input_box" value="<?php echo htmlspecialchars($password,ENT_QUOTES);?>" /></td>
			<?php }/*}else{ ?>
            	<td><input type="text" name="password" size="40" maxlength="40" class="input_box" value="<?php echo htmlspecialchars($password,ENT_QUOTES);?>" /></td>
            <?php }*/ ?>
            
            <input type="hidden" value="<?= $emailAddress; ?>" id="reset-us"/>
          </tr>
