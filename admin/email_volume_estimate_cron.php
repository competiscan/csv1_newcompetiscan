...
if ($l == 'Saturday') {
    $doall = true;
    $newts = strtotime('-9 months');
    $start_year = date('Y', $newts);
    $start_month = date('n', $newts);
    $dopost = true;
}else if($l=='Sunday'){
    $doall = true;
    $newts = strtotime('-18 months');
    $start_year = date('Y',$newts);
    $start_month = date('n',$newts); 
    $newts2 = strtotime('-8 months');
    $to_year = date('Y',$newts2);
    $to_month = date('n',$newts2);	
    $dopost = true;
} else {
    $doall = false;
...
for ($i = $start_year; $i <= $to_year; $i++) {
    foreach ($month_name as $key => $value) {
        if($to_month!='' || $l=='Sunday'){
            if(($i==$start_year && intval($key)<$start_month) || (intval($key)>$to_month)) {
                continue;
            }
        }elseif ($i == $start_year && intval($key) < $start_month) {
            continue;
        }
        $calc_date = $i . '-' . $key;
        ...
        $mvcalc->doEMailEstimateVolume($i, $key, $factor); // for eve calculation
        /* End to added for running march month data */
    }
}
?>