...
<?php echo $javascript; ?>
    // one parameter added for digital product content
function doPDFText(pid,add) {
	var winy = window.open("<?php if(!$fromtemp) echo '../'; ?>pdfContentDetail.php?id="+pid+"&add="+add,null,"scrollbars=yes, resizable=yes, height=450,width=650,status=yes,toolbar=no,menubar=no,location=no,addressbar=yes");
	winy.focus();
}
...
function removeCmp(cmp_id){
	...
	var obj2 = document.forms.prodForm.cmp_ids;
        var cmp_ids_val = obj2.value;
	var valArray = cmp_ids_val.split(',');
	if(valArray.length==1){
            document.getElementsByName('productHeadline')[0].readOnly = true;
	    document.getElementsByName('productHeadline')[0].innerHTML='';
        }
	...
	if(scsc_comboIDs_val!=''){
            var valArray = scsc_comboIDs_val.split('|');
              var obj3 = document.forms.prodForm.cmp_ids;
                var cmp_ids_val_array = obj3.value;
                var val3Array = cmp_ids_val_array.split(',');
                if(val3Array.length==1){
...
function addPan_Digital(panelist_id,competi_id,mChannelIDval,mPanelIDval,genderval,ageIDval,incomeIDval,stateIDval,ownbizval,date_observed){
	
	addPanOnly_Digital(panelist_id,competi_id,false,date_observed);
...
function addPanOnly_Digital(panelist_id,competi_id,edit,date_observed){
	var obj = document.getElementById('pans');
	...
	var timeidc = ''+hh+mm+ss;
        ...
        //var dateStrings = convertDate(currdate);
        var dateStrings = date_observed;
	var objd = document.selform_date;
	if(edit){
		objd = document.selform_date_edit;
	}
	if(objd){ 
		datedisplay = dateval = objd.ymd.value;
                if(datedisplay==''){
                    datedisplay = dateval = dateStrings;                  
                    
                }
                
		dateid = dateStrings.substring(0,4)+dateStrings.substring(5,7)+dateStrings.substring(8,10);
	}
...
function doProdDelete_digital(pid){
	if(confirm("Are you sure you want to delete?") ) {
		document.location.href = 'manageproduct-digital.php?delID='+pid;
	}
}
<?php 
if($fromtemp){
...
function validate() {
  ...
  /*  if (pdf_is_required && document.prodForm.curpdf.value == 'a' && PDFFile_obj.value == '' && mediafile_obj.value == '') {
    ...
    */
	...
	if (categoryID == ''){
        ...
        <?php //if(!strstr($_SERVER['REQUEST_URI'],'addproduct-digital')){?>
       /* if(document.forms.prodForm.mChannelID.selectedIndex<1){
   ...
   if((document.forms.prodForm.mChannelID.value=='7' || document.forms.prodForm.mChannelID.value=='5' || document.forms.prodForm.mChannelID.value=='2' || document.forms.prodForm.mChannelID.value=='6') && document.forms.prodForm.competi_ids.value=='' && document.forms.prodForm.primary_country.value=='' ){ 
...
/**
 * Expands the extra set of incentives if some of the fields have been filled from before
 */
...
function add_SCSC(){
   ...
   //alert(scsc_comboIDsedit);
     if( (trimspace(scsc_comboIDsval)==''||trimspace(primarysector[0])==scsc_comboIDsedit) && (document.forms.prodForm.combo_sid.selectedIndex)){
        <?php if($is_digital_temp==''){?>
            add_digital_headlines();
        <?php } ?>
...
function sortSCSC(idval,sort){
	...
	checkProductName();
        ...
        add_digital_headlines();
        <?php } ?>
        //var add_SCSC_link    =   document.getElementById('add_SCSC_link');
       // add_SCSC_link.click();
       /* ##############end for mobile digital headline automatic writing ################# */
...
/* ############## for mobile digital headline automatic writing ################# */
function add_digital_headlines(){
    var cmp_idsval= document.getElementsByName('cmp_ids')[0].value;
  
    var companyvalArray = cmp_idsval.split(',');
    var selectedcompId=companyvalArray[0];
    var companydiv=document.getElementById('cmp'+selectedcompId);
    var comapnynamearray=companydiv.innerHTML.split('<a');
    //var companyName=trimspace(comapnynamearray[0].trim('('));
    var companyName = trimspace(comapnynamearray[0].replace(/\(+$/g, ''));
    
    
  // alert(companyName+'hello');
    var mchaneltext = document.forms.prodForm.mChannelID.options[document.forms.prodForm.mChannelID.selectedIndex].text;
    // alert(mchaneltext);
    var scsc_names  = new Array('combo_sid','combo_cid','combo_scid','combo_sscid');
    var scsc_values = new Array();
    var scsc_text   = new Array();
    var category_is_selected = false;
    var productNameval= document.getElementsByName('productName')[0].value;
    ...
    for (var j = 0; j < scsc_names.length; j++) {
        ...
        for (var k = 0; k < obj.options.length; k++) {
            if (obj.options[k].selected && obj.options[k].value.length > 0) {
                ...
                scsc_text[j] = obj.options[k].text;
               //alert(scsc_text[j]+'=='+j);
                if(trimspace(scsc_text[0])=='Credit Cards'){
                    if(trimspace(productNameval)==''){
                        alert('Please insert the products name');
                        return false;
                    }else{
                        headlinetxt=trimspace(productNameval)+' '+mchaneltext+' advertisement from '+companyName;
                    }
                }else{
                   // alert('hi');
                    if(document.forms.prodForm.combo_sscid.selectedIndex){
                        var combo_sscidval = document.forms.prodForm.combo_sscid.options[document.forms.prodForm.combo_sscid.selectedIndex].text;
                        headlinetxt=trimspace(combo_sscidval)+' '+mchaneltext+' advertisement from '+companyName;
                     // alert('hello');
                    }else if(document.forms.prodForm.combo_scid.selectedIndex){
                        var cocombo_scidval = document.forms.prodForm.combo_scid.options[document.forms.prodForm.combo_scid.selectedIndex].text;
                        headlinetxt=trimspace(cocombo_scidval)+' '+mchaneltext+' advertisement from '+companyName;
                     }else if(document.forms.prodForm.combo_cid.selectedIndex){
                        var combo_cidval = document.forms.prodForm.combo_cid.options[document.forms.prodForm.combo_cid.selectedIndex].text;
                        headlinetxt=trimspace(combo_cidval)+' '+mchaneltext+' advertisement from '+companyName;
                     }else if(document.forms.prodForm.combo_sid.selectedIndex){
                        var combo_sidval = document.forms.prodForm.combo_sid.options[document.forms.prodForm.combo_sid.selectedIndex].text;
                        headlinetxt=trimspace(combo_sidval)+' '+mchaneltext+' advertisement from '+companyName;
                     }
       ...
       document.getElementsByName('productHeadline')[0].innerHTML=headlinetxt;
       document.getElementsByName('productHeadline')[0].readOnly = true;
return true;
}

function update_digital_headlines(){
