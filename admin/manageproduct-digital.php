...
if(isset($_REQUEST['cstat'])) {
	...
	$_SESSION['cstat'] = (int) $_REQUEST['cstat'];
        if($_SESSION['cstat']=='14'){
            $add='1';
            
        }else if($_SESSION['cstat']=='15'){
            $add='2';
            
        }else if($_SESSION['cstat']=='16'){
            $add='3';
        }
        $_SESSION['add'] =$add;
}
...
switch($_SESSION['cstat']){
	...
	case 12:
		...
		break;
        case 14:
		$ctext = 'Online Display ';
		break; 
         case 15:
		$ctext = 'Search Engine Marketing ';
		break;
         case 16:
		$ctext = 'Online Video ';
		break;    
...
<?php
	...
	//$sql = "SELECT DISTINCT pd.productID,productName,pd.sectorID,categoryID,entryID,addedToDatabase,DATE_FORMAT(actual_addedToDatabase,'%m/%d/%Y') as actual_addedToDatabasef,admin_userID,productStatus,pd.mPanelID,pd.mChannelID,DMSource,DATE_FORMAT(approved_date,'%m/%d/%Y') as approved_datef,product_priority,special_handling";
	...
	$numquery = "SELECT COUNT(DISTINCT pd.productID) as numrows";
       // $sql = "SELECT  pd.productID,productName,pd.sectorID,categoryID,entryID,addedToDatabase,DATE_FORMAT(actual_addedToDatabase,'%m/%d/%Y') as actual_addedToDatabasef,admin_userID,productStatus,pd.mPanelID,pd.mChannelID,DMSource,DATE_FORMAT(approved_date,'%m/%d/%Y') as approved_datef,product_priority,special_handling";
	//$numquery = "SELECT COUNT( pd.productID) as numrows";
	$jointext = '';
	$sql2 = '';
	...
	if($_SESSION['ocr_search_text']!='') {
		...
		if($numrow==0 && !empty($SPHINX_name)){
			$s = startSphinx();
                        ...
                        $inds = 'base_index_prod_digital,base_index_prod_digital_od,base_index_prod_digital_sem,digital_od_deltaindex,digitaldeltaindex,digital_sem_deltaindex'; 
	...
	if($_SESSION['cstat']==7){
		$from .= " AND pd.mChannelID=5 And pd.is_digital='0'";
	}
	elseif($_SESSION['cstat']==12){
		...
		}elseif($_SESSION['cstat']==15){
			$from .= " AND pd.mChannelID=9 And pd.is_digital='1'";
		}elseif($_SESSION['cstat']==16){
			$from .= " AND pd.mChannelID=10 And pd.is_digital='1'";
		}
		else{
	...
	$sql .= $fromsel.$from.$sql2;
        ...
        //echo $numquery.=" ) A, cscan_product_detail pd WHERE A.productID = pd.productID";
        //echo $numquery;
	 ...
	 ?>>
		...
		<td valign="top">
                 <!--   <img src="../images/arrow.gif" id="<?php //echo 'pimg'.$productID; ?>" alt="" title="Preview this Product" style="cursor:pointer;" onclick="doPreview('<?php //echo $productID; ?>',<?php echo $img_createddate_ts; ?>);" onmouseover="showPreview(<?php //echo $productID; ?>,<?php echo $img_createddate_ts; ?>,1); return true;" onmouseout="hidePreview(<?php //echo $productID; ?>); return true;" />  -->
                    ...
                    <a class="hlinks" href="addproduct-digital.php?id=<?php echo $productID;?>&add=<?php echo $_SESSION['add'];?>"><strong><?php echo ucfirst($productName);?></strong></a></td>
        <td><?php 
