...
if(!isset($_REQUEST['show_All'])){
	if(isset($_REQUEST['search_text'])) {
		...
		$_SESSION['tmp_product_name'] = trim($_REQUEST['product_name']);
	}
...
//                }
                 ...
                 ############ remove extra condition for increase performance ##############
                /*                
		if($or!=''){
			$addq .= " AND (pe.sectorID='0' OR pe.sectorID=''$or)";
			$inor = true;
	...
	if($_SESSION['tmp_product_searchText']!='')  { 
		...
		//$addq .= " AND (productName like '%$search_key%' OR muid like '%$search_key%')";
                $addq .= " AND (muid = '$search_key')";
	}
        if($_SESSION['tmp_product_name']!='')  { 
		if(strpos($_SESSION['tmp_product_name'],'tmp')!==false){
			$searchtext_productname = preg_replace('/tmp$/','',$_SESSION['tmp_product_name']);
			$addq .= " AND isTmp=1";
		}
		else $searchtext_productname = $_SESSION['tmp_product_name'];
		$search_key = mysqlLike($searchtext_productname);
		$addq .= " AND (productName like '%$search_key%')";
	}
...
<table border="0" cellspacing="0" cellpadding="1" class="text">
	<tr>
	<td align="right"><strong>Search Temp ID:</strong></td>
	...
	<td>&nbsp;</td>
        ...
        <tr>
	<td><strong>Search Product by Name:</strong></td>
	<td><input type="text" name="product_name" size="40" maxlength="255" class="input_box" value="<?php echo htmlspecialchars($_SESSION['tmp_product_name'],ENT_QUOTES); ?>" /></td>
	<td>&nbsp;</td>
	</tr>
...
<?php if(checkGroup(5)){ ?>
    ...
    <!--<input class="button" style="width:180px;" type="button" value="Add Temp Product" onclick="winPopProduct('../temp_product_latest.php?isTmp=1'); return false;" />-->
    <?php //}else { ?>
...
?>
	...
	if(!$isTmp){
				...
				$( "#showmessage_dialog_'.$muid.'" ).dialog({
					...
					open: function( event, ui ) {
						...
						if(old_src.indexOf("blank.html")!=-1){
							$( "#showmessage_iframe_'.$muid.'" ).attr("src", "https://api3.competiscan.com/html-pdf/v2/html/'.$muid.'");   
						}
					}
				});
			}
        ...
        if($productID<=0) {
        	...
        	if(checkGroup(5)) {
                    /*if(ENV == 'localhost' || ENV == 'demo.competiscan.com'){
                        print "[<a class=\"hlinks\" href=\"../temp_product_latest.php?muid=$muid&amp;hy=$hy";
	        	if($isTmp==1) print '&isTmp=1';
	        	print "\" onclick=\"winPopProduct('../temp_product_latest.php?muid=$muid&amp;hy=$hy";
	        	if($isTmp==1) print '&isTmp=1';
        ...
        elseif(checkGroup(5)) {
			...
			else print "<br />[<a href=\"https://cs.competiscan.com/email/$muid\" onclick=\"winPopProduct('https://cs.competiscan.com/email/$muid'); return false;\" id=\"div_{$muid}_eid\">Forward</a>]";
        	################################# Start S3 Implementation Code #################################
        	$query2 = "SELECT COUNT(*) FROM `cscan_email_forward$hy` WHERE `muid`='".$DRW->real_escape_string($muid)."' AND isTmp=$isTmp";
        ...
        $savedFiles = array();
        ################################# Start S3 Implementation Code #################################
		$qf = "SELECT `ceafpath`,`ceaftype`,`ceafid` FROM `cscan_email_attach_file$hy` WHERE `muid`='".$DRW->real_escape_string($muid)."' AND isTmp=$isTmp";
		################################# End S3 Implementation Code #################################
		...
		while($dataf = $DRW->fetch_row($query_resultf)){
			...
			################################# Start S3 Implementation Code #################################
			$img_link= $displays3URL.$dataf[0];
			$tmp = "<a href=\"$img_link\" target=\"_blank\">".$bname."</a>";
		...
		if($isTmp!=1) {
			################################# Start S3 Implementation Code #################################
	        $query2 = "SELECT `cefid`,`cefname`,`ceftype` FROM `cscan_email_file$hy` WHERE `muid`='".$DRW->real_escape_string($muid)."' ORDER BY `cefpart` ASC";
	        ################################# End S3 Implementation Code #################################
			$query_result2 = $DRW->query($query2,$DRW_read);
...
function check_searchform() {
	...
	var product_name = document.prodForm.product_name.value = trimspace(document.prodForm.product_name.value);
	...
	if((search=='' && product_name=='') && search2=='' && search3<1 && search4=='') {
