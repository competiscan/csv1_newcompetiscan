...
if (!$fromtemp) {
   ...
   // $displayArray[$dak][$dai]['value'] = '<input type="file" class="input_box" name="mediafile" size="40" id="mediafile" />';
    ...
    if ($mChannelID == 5 || $mChannelID == 7) {
        $sql3 = "SELECT SQL_NO_CACHE document_id,document_filename,document_size_byte,bucket_name FROM cscan_document WHERE productID=$updID AND document_id=2";
    ...
    if (isset($_REQUEST['add']) && $_REQUEST['add'] == '3') {
     $displayArray[$dak][$dai]['title'] = '<span class="error">*</span>Product Video';
    }else{
         $displayArray[$dak][$dai]['title'] = '<span class="error">*</span>Product Image';
    }
...
$oldID='';
$img_link='';
######################## for the default image  in digital ###########################

    if ((isset($_REQUEST['old']) && ($_REQUEST['add'] == '1' || $_REQUEST['add'] == '2' || $_REQUEST['add'] == '3')) || (isset($_REQUEST['id']) && $_REQUEST['id']!='')) {
       if(isset($_REQUEST['old'])){
        ...
        IF($oldID!=''){  
          $sqls = "SELECT creative_path,ad_md5 FROM ".$selecttable." where creative_id='" . $oldID . "'";
          $rss      = $DRW->query($sqls, $DRW_digital);
          $dataC    = $DRW->fetch_row($rss);
          $img_link = $dataC[0];
          $ad_md5   = $dataC[1];
           $imglinks="viewfile-digital.php?id=".$oldID; 
          ...
          //        }
          //        if(!empty($arr_sql)){
          //            $sql_union = implode(" UNION ", $arr_sql);//echo $sql_union;die;
          //            $res_union_query = $DRW->query($sql_union, $DRW_digital);
          //            if ($DRW->num_rows($res_union_query) > 0) {
          //                $arr_impressions = $unique_pan = array();
          //                while($res_union_row = $DRW->fetch_assoc($res_union_query)){
          //                    if(!in_array($res_union_row['panelist_id'], $unique_pan)){
          //                        $arr_impressions[] = $res_union_row;
          //                        $unique_pan[] = $res_union_row['panelist_id'];
          //                    }
          //                }
          //                if(!empty($arr_impressions)){
          //                    $impressions = serialize($arr_impressions);
          //                    $impressions=htmlentities($impressions);
          //                }
          //            }            
          //        }
            ##################################
        ...
        if (isset($_REQUEST['add']) && $_REQUEST['add'] == '3') {
             $videolink='Video Link: '.$img_link. '<br />';
            if(isset($_REQUEST['id'])){
                $updIDs=$_REQUEST['id'];

                $pdftext = '';
                $sql2 = "SELECT SQL_NO_CACHE document_id,document_filename,document_size_byte,document_path,bucket_name FROM cscan_document WHERE productID=$updIDs AND document_id=1";
                $rs2 = $DRW->query($sql2, $DRW_read);
                $row2 = $DRW->fetch_row($rs2);
                $document_id = (int) $row2[0];
                $document_filename = $row2[1];
                $document_size_byte = (int) $row2[2];
                $document_path=$row2[3];
                $check_bucket_name=$row2[4];
                //$img_link='..'.$document_path.$document_filename;
                $img_link=$displays3URL.substr($row2[3],1).$document_filename;
                if($check_bucket_name!='NULL' AND $check_bucket_name!=''){
                    $img_link=$displays3CSV2URL.substr($row2[3],0).$document_filename;
                ...
                if($videolink!=''){
                    ...
                    $viewLink='../productDocuments.php?id='.$updIDs;
                }
                $displayArray[$dak][$dai]['value'] = '
                  ...
                  <br /></br />  ' . $videolink . '<a onclick="window.open(\'' . $viewLink . '\',\'newwindow\', \'width=1000, height=650\'); return false;"  href="' . $viewLink . '"><b>Click Here To View in Full Screen</b></a>          
                    </div><input type="hidden" name="img_capture_id" value="' . $oldID . '" /><input type="hidden" name="red_digital_id" value="' . $addID . '" /><input type="hidden" name="temp" value="' . $is_digital_temp . '" />';
        }else if(isset($_REQUEST['id'])){
           
            if ($updID != '') {
                $pdftext = '';
                $sql2 = "SELECT SQL_NO_CACHE document_id,document_filename,document_size_byte,document_path,document_content_type,bucket_name FROM cscan_document WHERE productID=$updID AND document_id=1";
                $rs2 = $DRW->query($sql2, $DRW_read);
                $row2 = $DRW->fetch_row($rs2);
                $document_id = (int) $row2[0];
                $document_filename = $row2[1];
                $document_size_byte = (int) $row2[2];
                $document_path='..'. $row2[3];
                $document_content_type=$row2[4];
                $check_bucket_name=$row2[5];
                if($document_content_type=='html'){
                 ...
                 //$img_link='..'.$row2[3].$document_filename;                 
                 $img_link=$displays3URL.substr($row2[3],1).$document_filename;
                 if($check_bucket_name!='NULL' AND $check_bucket_name!=''){
                        $img_link=$displays3CSV2URL.substr($row2[3],0).$document_filename;
		...
		<div id="prod_img_div" style="display:block;">
               <iframe border="0" src="'. $img_link. '" width="250" height="100">
			 </iframe>
                    <br /><br />    
                <a onclick="window.open(\'' . $img_link . '\',\'newwindow\', \'width=700, height=650\'); return false;"  href="'. $img_link . '"><b>Click Here To View in Full Screen</b></a> ';     
			...
			<br /><br /><a onclick="window.open(\'' . $img_link . '\',\'newwindow\', \'width=700, height=650\'); return false;"  href="' . $img_link . '"><b>Click Here To View in Full Screen</b></a>         
			</div><input type="hidden" name="img_capture_id" value="' . $oldID . '" /><input type="hidden" name="red_digital_id" value="' . $addID . '" />';
            */
            
            if(strstr(strtolower($img_link),'.html')){ 
                        
                $displayArray[$dak][$dai]['value'] = '		
			<div id="prod_img_div" style="display:block;">

			<iframe border="0" src="fetchhtml_image.php?hid='.$oldID.'&tbl='.$selecttable.'" width="250" height="100">
			 </iframe>
			<br /><br /><a onclick="window.open(\'' . $img_link . '\',\'newwindow\', \'width=700, height=650\'); return false;"  href="' . $img_link . '"><b>Click Here To View in Full Screen</b></a>         
			</div><input type="hidden" name="img_capture_id" value="' . $oldID . '" /><input type="hidden" name="red_digital_id" value="' . $addID . '" /><input type="hidden" name="temp" value="' . $is_digital_temp . '" />';
...
######################## end for the default image in digital #########################
    ...
    if ($updID != '') {
        $pdftext = '';
        $sql2 = "SELECT SQL_NO_CACHE document_id,document_filename,document_size_byte,bucket_name FROM cscan_document WHERE productID=$updID AND document_id=1";
        $rs2 = $DRW->query($sql2, $DRW_read);
        $row2 = $DRW->fetch_row($rs2);
        $document_id = (int) $row2[0];
        $document_filename = $row2[1];
        ...
        if ($document_id != 0) {
            $curpdf = 'u';
          /*  $pdftext .= 'Current Image: [<a href="../productDocuments.php?id=' . $updID . '" target="_blank">' . $document_filename . '</a>] (' . $DisplaySize . ')
			<br /><a href="#" onclick="doPDFSample(' . $updID . '); return false;">Manage Preview</a>'; */
        }

        $sql2 = "SELECT SQL_NO_CACHE document_id,document_filename,document_size_byte,bucket_name FROM cscan_document_orig WHERE productID=$updID AND document_id=1";
        $rs2 = $DRW->query($sql2, $DRW_read);
        $row2 = $DRW->fetch_row($rs2);
    ...
    if ($updID != '') {
        $displayArray[$dak][$dai]['value'] .= ' [<a href="#" onclick="doPDFText(' . $updID . ','.$_REQUEST['add'].'); return false;">View Current Content</a>]';
    }
...
if ($fromtemp) {
    $displayArray[$dak][$dai]['value'] .= '<input type="hidden" name="img_companyID" value="' . $img_companyID_new . '" />';
}
$dai++;
if (!$fromtemp) {
    $displayArray[$dak][$dai]['title'] = '*Company Image';
...
//$displayArray[$dak][$dai]['value'] = '<select class="combo_box" name="mChannelID" onchange="getDeps(\'offerOrigin\');markInsuranceexchange();checkChannel();doDelMeth();checkDeps_mc();"'.$disabled.'><option value="0">-- Select One --</option>';
$displayArray[$dak][$dai]['value'] = '<select class="combo_box" name="mChannelID" onchange="getDeps(\'offerOrigin\');markInsuranceexchange();checkChannel();doDelMeth(); checkDeps_mc();"' . $disabled . '>';
...
if( $selected2=='' AND $selected3=='' AND $selected4=='' AND $selected5=='' AND $selected==''){    
    $selected='checked="checked"';
}
//$displayArray[$dak][$dai]['value'] = $showhide;
$displayArray[$dak][$dai]['title'] = 'Digital Source :';
$displayArray[$dak][$dai]['value'] = '<input type="radio" value="1" name="ismobile" id="ismobile1"'.$selected.'><label for="ismobile1">Desktop </label>&nbsp;&nbsp;<input type="radio" value="2" name="ismobile" id="ismobile2"'.$selected2.'><label for="ismobile2">Mobile</label><input type="radio" value="3" name="ismobile" id="ismobile3"'.$selected3.'><label for="ismobile3">In App Android</label><input type="radio" value="4" name="ismobile" id="ismobile4"'.$selected4.'><label for="ismobile4">In App IOS</label><input type="radio" value="5" name="ismobile" id="ismobile5"'.$selected5.'><label for="ismobile5">Social</label>';$dai++;
...
<div style="clear:left;"><input type="text" name="productName" size="40" onblur="update_digital_headlines()"; maxlength="255" class="input_box" value="' . htmlspecialchars($productName, ENT_QUOTES) . '"' . $disabled . ' /></div>';
...
$displayArray[$dak][$dai]['value'] = '<textarea name="productHeadline" rows="5" cols="60" readonly="readonly" class="input_box" onkeyup="startTimer(\'showHeads(\\\'';
if ($fromtemp) {
    $displayArray[$dak][$dai]['value'] .= 'admin/';
}
$displayArray[$dak][$dai]['value'] .= 'checkheads.php\\\')\');" onblur="hideHeads();"' . $disabled . '>' . htmlentities($productHeadline) . '</textarea><input type="hidden" name="old_productHeadline" value="' . htmlspecialchars($productHeadline, ENT_QUOTES) . '" />';
...
$displayArray[$dak][$dai]['value'] .= ' /><input type="hidden" id="dmtmsource" name="dmtmsource" value="'.htmlspecialchars($DMSource,ENT_QUOTES).'" /><input type="hidden" name="isdevice_mobile" value="'.$isdevice_mobile.'" /><input type="hidden" name="simple_domain" value="'.$simple_domain.'" /><input type="hidden" name="consumer_insights" value="'.$consumer_insights.'" /><input type="hidden" name="select_dig_tbl" value="'.$selecttable.'" /><input type="hidden" name="is_subp" value="' . $is_subp . '" />';
//if (!empty($impressions)) {
//    $displayArray[$dak][$dai]['value'] .= '<input type="hidden" name="impressions" value="'.$impressions.'" />';           
//}
$dai++;
...
$displayArray[$dak][$dai]['value'] = '<select class="combo_box" name="mTypeID"' . $disabled . ' onchange="checkDeps_mtvariant();">';
...
$dai++;
$saveDIArray['mc_primary_country'] = array("checkMediaChannel('2,5,6,7,10')");
...
if ($fromtemp) {
    ...
    $displayArray[$dak][$dai]['value'] = '<textarea name="productComment" rows="5" cols="40" class="input_box" style="background:#DDF9EE;">' . htmlspecialchars($productComment, ENT_QUOTES) . '</textarea><input type="hidden" name="temp" value="' . $is_digital_temp . '" />';
    $dai++;
    if ($updID != '') {
    ...
    $displayArray[$dak][$dai]['value'] = '<div id="show_buttons" style="display:none;"><input class="button" type="submit" '.$disabledButton.' name="saver" value="';
    if (checkGroup(27)) {
        $displayArray[$dak][$dai]['value'] .= 'Approved';
    } else {
    ...
    if (checkGroup(28) && $updID != '') {
        $displayArray[$dak][$dai]['value'] .= '<input class="button" type="submit" name="rep" '.$disabledButton.' value="Reprocessed" onclick="return doSaved(3);" /> &nbsp; ';
    }
    if (checkGroup(27)) {
        $displayArray[$dak][$dai]['value'] .= '<input class="button" type="submit" name="una" '.$disabledButton.' value="Unapproved" onclick="return doSaved(2);" /> &nbsp; ';
    }
    $displayArray[$dak][$dai]['value'] .= '<input class="button" type="submit" name="prob" '.$disabledButton.' value="Problem" onclick="return doSaved(4);" /> &nbsp; <input class="button" type="button" value="Cancel" onclick="location.href=\'manageproduct-digital.php\'; return false;" />';
    if ($is_citi == '1' || in_array(266, $sectorID) || in_array(315, $sectorID)) {
        $displayArray[$dak][$dai]['value'] .= ' &nbsp; <input class="button" type="submit" '.$disabledButton.' name="prob" value="Unused" onclick="return doSaved(-1);" />';
    }
    if ($updID != '') {
        $displayArray[$dak][$dai]['value'] .= ' &nbsp; <input class="button" type="button" '.$disabledButton.' value="Delete" onclick="doProdDelete_digital(' . $updID . '); return false;" />';
    }
