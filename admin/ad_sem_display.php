...
$limit = 50;
if (isset($_REQUEST['p']))
    $p = $_SESSION['manageCategory_pr'] = $_REQUEST['p'];
elseif (isset($_SESSION['manageCategory_pr']))
    $p = $_SESSION['manageCategory_pr'];
else
    $p = 0;
$sort = '';
...
$sql = "SELECT creative_id,creative_path,ad_md5,digital_channel,file_type, lastUpdated FROM " . $selecttable . " WHERE digital_channel='Search Engine Marketing' AND file_type!='None' AND capture_status=0 AND created_date<= DATE_SUB(NOW(), INTERVAL 61 minute) $where_pan ORDER BY creative_id DESC  LIMIT $p, $limit";
$rs = $DRW->query($sql, $DRW_digital);
$resultCount = $DRW->num_rows($rs);


$numquery = "Select COUNT(creative_id) as numrows FROM " . $selecttable . " WHERE digital_channel='Search Engine Marketing' AND file_type!='None' AND capture_status=0 AND created_date<= DATE_SUB(NOW(), INTERVAL 61 minute) $where_pan";
...
<form method='post' name='frm1'>
    <table width='100%' border='0' cellspacing='0' cellpadding='5' class='text' rules='none' bordercolor='#B9B9B9' align='center' style='border-collapse:collapse'>
        <tr><td class="adminhead" align='center' colspan='6'>SEARCH ENGINE MARKETING PRODUCT</td></tr>
                        ...
                        <td align='right' colspan='2'>
                            <table width="100%" border="0" cellspacing="0" cellpadding="0" class="text">
                                ...
                                <tr>
                                    ...
                                    <td align='right' width="15%"><a href="ad_sem_capture.php" style="text-decoration:none;"><input class='button' style='width:180px' type='button' name='submit1' ID='delBt' value='Go to Capture Page'></a><br /></td>
                                </tr>
                            </table>
                        </td>
                    </tr>
