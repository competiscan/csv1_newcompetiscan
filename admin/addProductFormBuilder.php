...
if(ENV == 'localhost'){
    ...
    $displays3URL='https://vv1a3ga23f.execute-api.us-east-1.amazonaws.com/Stage/';
    $displays3URL2='https://files1.competiscan.com/';
    ...
    $displays3URL2='https://files.competiscan.com/';
}
...
if(!$fromtemp && $updID!=''){
	...
	if($DRW->num_rows($checkV)>0){
		...
		while($dataV = $DRW->fetch_row($checkV)){
			$displayArray[$dak][$dai]['value'] .= "<a class=\"hlinks\" href=\"addproduct.php?id=$dataV[1]\">".$dataV[0].'</a><br />';
		}
		$dai++;
	}
}
...
/* ######## Start for prcess PDF ######## */
if(!empty($updID)){
    $query_email = "select muid from cscan_product_email where productID=$updID";   
    $query_result_email = $DRW->query($query_email,$DRW_read);
    $numrows_email=$DRW->num_rows($query_result_email);
    $muid_email='';
    $process_pdf_html='';
    if($numrows_email>0){
        $data_email = $DRW->fetch_row($query_result_email);
        $muid_email=$data_email[0];
    }
}

if(empty($muid_email)){
    if(isset($_GET['muid']) && $_GET['muid']!=''){
        $muid_email=$_GET['muid'];
    }    
}

if(!empty($muid_email)){
    //$process_pdf_html='&nbsp;&nbsp;OR &nbsp;&nbsp;<label><input class="input_box" type="checkbox" id="process_pdf_chkbox" name="process_pdf" value="'.$muid_email.'"> Use Process PDF </label>&nbsp;&nbsp; <a class="bluelink" href="javascript:void(0);" onclick="htmlWin('.$muid_email.'); return false;" title="Processed PDF">View Process PDF</a>';
	//$process_pdf_html='&nbsp;&nbsp;<span id="process_pdf_chkbox_field" style="display:none;"> OR &nbsp;&nbsp;<label><input class="input_box"  type="checkbox" id="process_pdf_chkbox" name="process_pdf" value="'.$muid_email.'"> Use Process PDF </label>&nbsp;&nbsp;</span> <a class="bluelink" href="javascript:void(0);" onclick="htmlWin('.$muid_email.'); return false;" title="Processed PDF">View Process PDF</a>';
	$process_pdf_html='&nbsp;&nbsp;<span id="process_pdf_chkbox_field"> OR &nbsp;&nbsp;<label><input class="input_box"  type="checkbox" id="process_pdf_chkbox" name="process_pdf" value="'.$muid_email.'" onclick="checkProcessPdf('.$muid_email.')"> Use Process PDF </label>&nbsp;&nbsp;</span> <a class="bluelink" href="javascript:void(0);" onclick="htmlWin('.$muid_email.'); return false;" title="Processed PDF">View Process PDF</a><br><span align="right" style="float:right; margin-right:280px;display:none;" id="checking_process_pdf"><img src="../images/searching.gif">&nbsp;&nbsp;Checking process pdf...</span><span id="processpdf_status" align="right" style="float:right; margin-right:280px;display:none;"></span>';
}
...
if(!$fromtemp){
	...
	$displayArray[$dak][$dai]['value'] = '<input type="file" class="input_box" name="PDFFILE" size="40" id="PDFFILE" />'.$process_pdf_html;
	$dai++;
	$document_id = 0;
	if($updID!='') {
		$pdftext = '';
		$sql2 = "SELECT SQL_NO_CACHE document_id,document_filename,document_size_byte,document_path FROM cscan_document WHERE productID=$updID AND document_id=1";
		$rs2 = $DRW->query($sql2,$DRW_read);
		$row2 = $DRW->fetch_row($rs2);
		...
		$document_size_byte = (int)$row2[2];
                ...
                if(strpos($documentPath,'/')=='0'){
                    $documentPath  = substr($documentPath,1);
                }
                if(ENV == 'localhost' || ENV == 'demo.competiscan.com'){                
                    
                    $s3pdfurl=$displays3URL2.$documentPath.$document_filename;
                }else{
                     $s3pdfurl='';
                }
		...
		if($document_id!=0) {
                    ...
                    if((ENV == 'localhost' || ENV == 'demo.competiscan.com') && $s3pdfurl!='' && $document_filename!=''){
                        $pdftext .= '&nbsp;&nbsp;<a target="_blank" href="'.$s3pdfurl.'" download="'.$document_filename.'">View PDF</a>';
                    }
                    $pdftext .= '<br /><a href="#" onclick="doPDFSample('.$updID.'); return false;">Manage Preview</a>';
		}
		
		$sql2 = "SELECT SQL_NO_CACHE document_id,document_filename,document_size_byte,document_path FROM cscan_document_orig WHERE productID=$updID AND document_id=1";
		$rs2 = $DRW->query($sql2,$DRW_read);
		$row2 = $DRW->fetch_row($rs2);
		...
		$sizeofPDFinMB=$sizeofPDFinKB/1024;
                $documentPath_o= $row2[3];
                if(strpos($documentPath_o,'/')=='0'){
                    $documentPath_o  = substr($documentPath_o,1);
                }
                if(ENV == 'localhost' || ENV == 'demo.competiscan.com'){
                    $s3pdfurl_o=$displays3URL2.$documentPath_o.$document_filename_o;
                }else{
                    $s3pdfurl_o='';
                }
		if($sizeofPDFinMB<1) {
		...
		if($document_id_o!=0) {
                    if (checkGroup(95)){
                        ...
                        if((ENV == 'localhost' || ENV == 'demo.competiscan.com') && $s3pdfurl_o!='' && $document_filename_o!=''){
                            $pdftext .='&nbsp;&nbsp;<a target="_blank" href="'.$s3pdfurl_o.'" download="'.$document_filename_o.'">View PDF</a>';
                        }
                    }
		}
		if(!empty($pdftext)){
...
$displayArray[$dak][$dai]['value'] = '<select class="combo_box" onchange="doEnvelopePostageData();checkDeps_EnvelopePostageData();" name="delmethid"';
...
$displayArray[$dak] = array();
$displayKeys[$dak] = '';
$saveDIArray['mc_advertiser_address'] = array("checkMediaChannel('1') && checkMediaPanel('1,2') && (checkSector('266') || (checkSector('4') && checkCategory('454') && checkSubCategory('506')))");
$saveDIArray['mc_advertiser_address'][] = $dak.'_'.$dai;
$saveDIArray['mp_advertiser_address'] = array("checkMediaChannel('1') && checkMediaPanel('1,2') && (checkSector('266') || (checkSector('4') && checkCategory('454') && checkSubCategory('506')))");
$saveDIArray['mp_advertiser_address'][] = $dak.'_'.$dai;
$saveDIArray['s_advertiser_address'] = array("checkMediaChannel('1') && checkMediaPanel('1,2') && (checkSector('266') || (checkSector('4') && checkCategory('454') && checkSubCategory('506')))");
$saveDIArray['s_advertiser_address'][] = $dak.'_'.$dai;
$saveDIArray['c_advertiser_address'] = array("checkMediaChannel('1') && checkMediaPanel('1,2') && (checkSector('266') || (checkSector('4') && checkCategory('454') && checkSubCategory('506')))");
$saveDIArray['c_advertiser_address'][] = $dak.'_'.$dai;
$saveDIArray['sc_advertiser_address'] = array("checkMediaChannel('1') && checkMediaPanel('1,2') && (checkSector('266') || (checkSector('4') && checkCategory('454') && checkSubCategory('506')))");
$saveDIArray['sc_advertiser_address'][] = $dak.'_'.$dai;
$displayArray[$dak][$dai]['title'] = 'Advertiser Address';
$displayArray[$dak][$dai]['value'] = '<div style="clear:left;"><input type="text" name="advertiser_address" size="40" maxlength="255" class="input_box" value="'.htmlspecialchars($advertiser_address,ENT_QUOTES).'"'.$disabled.' /></div>';
$disfields[] = 'advertiser_address';
$dai++;

$saveDIArray['mc_advertiser_city'] = array("checkMediaChannel('1') && checkMediaPanel('1,2') && (checkSector('266') || (checkSector('4') && checkCategory('454') && checkSubCategory('506')))");
$saveDIArray['mc_advertiser_city'][] = $dak.'_'.$dai;
$saveDIArray['mp_advertiser_city'] = array("checkMediaChannel('1') && checkMediaPanel('1,2') && (checkSector('266') || (checkSector('4') && checkCategory('454') && checkSubCategory('506')))");
$saveDIArray['mp_advertiser_city'][] = $dak.'_'.$dai;
$saveDIArray['s_advertiser_city'] = array("checkMediaChannel('1') && checkMediaPanel('1,2') && (checkSector('266') || (checkSector('4') && checkCategory('454') && checkSubCategory('506')))");
$saveDIArray['s_advertiser_city'][] = $dak.'_'.$dai;
$saveDIArray['s_advertiser_city'] = array("checkMediaChannel('1') && checkMediaPanel('1,2') && (checkSector('266') || (checkSector('4') && checkCategory('454') && checkSubCategory('506')))");
$saveDIArray['s_advertiser_city'][] = $dak.'_'.$dai;
$saveDIArray['s_advertiser_city'] = array("checkMediaChannel('1') && checkMediaPanel('1,2') && (checkSector('266') || (checkSector('4') && checkCategory('454') && checkSubCategory('506')))");
$saveDIArray['s_advertiser_city'][] = $dak.'_'.$dai;
$displayArray[$dak][$dai]['title'] = 'Advertiser City';
$displayArray[$dak][$dai]['value'] = '<div style="clear:left;"><input type="text" name="advertiser_city" size="40" maxlength="50" class="input_box" value="'.htmlspecialchars($advertiser_city,ENT_QUOTES).'"'.$disabled.' /></div>';
$disfields[] = 'advertiser_city';
$dai++;

$saveDIArray['mc_advertiser_state'] = array("checkMediaChannel('1') && checkMediaPanel('1,2') && (checkSector('266') || (checkSector('4') && checkCategory('454') && checkSubCategory('506')))");
$saveDIArray['mc_advertiser_state'][] = $dak.'_'.$dai;
$saveDIArray['mp_advertiser_state'] = array("checkMediaChannel('1') && checkMediaPanel('1,2') && (checkSector('266') || (checkSector('4') && checkCategory('454') && checkSubCategory('506')))");
$saveDIArray['mp_advertiser_state'][] = $dak.'_'.$dai;
$saveDIArray['s_advertiser_state'] = array("checkMediaChannel('1') && checkMediaPanel('1,2') && (checkSector('266') || (checkSector('4') && checkCategory('454') && checkSubCategory('506')))");
$saveDIArray['s_advertiser_state'][] = $dak.'_'.$dai;
$saveDIArray['c_advertiser_state'] = array("checkMediaChannel('1') && checkMediaPanel('1,2') && (checkSector('266') || (checkSector('4') && checkCategory('454') && checkSubCategory('506')))");
$saveDIArray['c_advertiser_state'][] = $dak.'_'.$dai;
$saveDIArray['sc_advertiser_state'] = array("checkMediaChannel('1') && checkMediaPanel('1,2') && (checkSector('266') || (checkSector('4') && checkCategory('454') && checkSubCategory('506')))");
$saveDIArray['sc_advertiser_state'][] = $dak.'_'.$dai;

$displayArray[$dak][$dai]['title'] = 'Advertiser State/Province';
$displayArray[$dak][$dai]['value'] = '<select name="advertiser_state" class="combo_box">';
$disfields[] = 'advertiser_state';
$displayArray[$dak][$dai]['value'] .= '<option value="0"';
if($state=='0') {
	$displayArray[$dak][$dai]['value'] .= " selected=\"selected\"";
}
$displayArray[$dak][$dai]['value'] .= '>--Select--</option>';
ob_start();
getStates($advertiser_state);
$displayArray[$dak][$dai]['value'] .= ob_get_clean();
$displayArray[$dak][$dai]['value'] .= '</select>';
$dai++;

$saveDIArray['mc_advertiser_zipcode'] = array("checkMediaChannel('1') && checkMediaPanel('1,2') && (checkSector('266') || (checkSector('4') && checkCategory('454') && checkSubCategory('506')))");
$saveDIArray['mc_advertiser_zipcode'][] = $dak.'_'.$dai;
$saveDIArray['mp_advertiser_zipcode'] = array("checkMediaChannel('1') && checkMediaPanel('1,2') && (checkSector('266') || (checkSector('4') && checkCategory('454') && checkSubCategory('506')))");
$saveDIArray['mp_advertiser_zipcode'][] = $dak.'_'.$dai;
$saveDIArray['s_advertiser_zipcode'] = array("checkMediaChannel('1') && checkMediaPanel('1,2') && (checkSector('266') || (checkSector('4') && checkCategory('454') && checkSubCategory('506')))");
$saveDIArray['s_advertiser_zipcode'][] = $dak.'_'.$dai;
$saveDIArray['c_advertiser_zipcode'] = array("checkMediaChannel('1') && checkMediaPanel('1,2') && (checkSector('266') || (checkSector('4') && checkCategory('454') && checkSubCategory('506')))");
$saveDIArray['c_advertiser_zipcode'][] = $dak.'_'.$dai;
$saveDIArray['sc_advertiser_zipcode'] = array("checkMediaChannel('1') && checkMediaPanel('1,2') && (checkSector('266') || (checkSector('4') && checkCategory('454') && checkSubCategory('506')))");
$saveDIArray['sc_advertiser_zipcode'][] = $dak.'_'.$dai;
$displayArray[$dak][$dai]['title'] = 'Advertiser Zip/Postal Code';
$displayArray[$dak][$dai]['value'] = '<div style="clear:left;"><input type="text" name="advertiser_zipcode" size="40" maxlength="8" class="input_box" value="'.$advertiser_zipcode.'"'.$disabled.' /></div>';
$disfields[] = 'advertiser_zipcode';
$dai++;

$saveDIArray['mc_advertiser_phone_number'] = array("checkMediaChannel('1') && checkMediaPanel('1,2') && (checkSector('266') || (checkSector('4') && checkCategory('454') && checkSubCategory('506')))");
$saveDIArray['mc_advertiser_phone_number'][] = $dak.'_'.$dai;
$saveDIArray['mp_advertiser_phone_number'] = array("checkMediaChannel('1') && checkMediaPanel('1,2') && (checkSector('266') || (checkSector('4') && checkCategory('454') && checkSubCategory('506')))");
$saveDIArray['mp_advertiser_phone_number'][] = $dak.'_'.$dai;
$saveDIArray['s_advertiser_phone_number'] = array("checkMediaChannel('1') && checkMediaPanel('1,2') && (checkSector('266') || (checkSector('4') && checkCategory('454') && checkSubCategory('506')))");
$saveDIArray['s_advertiser_phone_number'][] = $dak.'_'.$dai;
$saveDIArray['c_advertiser_phone_number'] = array("checkMediaChannel('1') && checkMediaPanel('1,2') && (checkSector('266') || (checkSector('4') && checkCategory('454') && checkSubCategory('506')))");
$saveDIArray['c_advertiser_phone_number'][] = $dak.'_'.$dai;
$saveDIArray['sc_advertiser_phone_number'] = array("checkMediaChannel('1') && checkMediaPanel('1,2') && (checkSector('266') || (checkSector('4') && checkCategory('454') && checkSubCategory('506')))");
$saveDIArray['sc_advertiser_phone_number'][] = $dak.'_'.$dai;
$displayArray[$dak][$dai]['title'] = 'Advertiser Phone Number';
$displayArray[$dak][$dai]['value'] = '<div style="clear:left;"><input type="text" name="advertiser_phone_number" size="40" maxlength="15" class="input_box" value="'.$advertiser_phone_number.'"'.$disabled.' /></div>';
$disfields[] = 'advertiser_phone_number';
$dai++;

$saveDIArray['mc_advertiser_url'] = array("checkMediaChannel('1') && checkMediaPanel('1,2') && (checkSector('266') || (checkSector('4') && checkCategory('454') && checkSubCategory('506')))");
$saveDIArray['mc_advertiser_url'][] = $dak.'_'.$dai;
$saveDIArray['mp_advertiser_url'] = array("checkMediaChannel('1') && checkMediaPanel('1,2') && (checkSector('266') || (checkSector('4') && checkCategory('454') && checkSubCategory('506')))");
$saveDIArray['mp_advertiser_url'][] = $dak.'_'.$dai;
$saveDIArray['s_advertiser_url'] = array("checkMediaChannel('1') && checkMediaPanel('1,2') && (checkSector('266') || (checkSector('4') && checkCategory('454') && checkSubCategory('506')))");
$saveDIArray['s_advertiser_url'][] = $dak.'_'.$dai;
$saveDIArray['c_advertiser_url'] = array("checkMediaChannel('1') && checkMediaPanel('1,2') && (checkSector('266') || (checkSector('4') && checkCategory('454') && checkSubCategory('506')))");
$saveDIArray['c_advertiser_url'][] = $dak.'_'.$dai;
$saveDIArray['sc_advertiser_url'] = array("checkMediaChannel('1') && checkMediaPanel('1,2') && (checkSector('266') || (checkSector('4') && checkCategory('454') && checkSubCategory('506')))");
$saveDIArray['sc_advertiser_url'][] = $dak.'_'.$dai;
$displayArray[$dak][$dai]['title'] = 'Advertiser URL';
$displayArray[$dak][$dai]['value'] = '<div style="clear:left;"><input type="text" name="advertiser_url" size="40" maxlength="255" class="input_box" value="'.$advertiser_url.'"'.$disabled.' /></div>';
$disfields[] = 'advertiser_url';
$dai++;
}
...
$displayArray[$dak][$dai]['title'] .= '';
$displayArray[$dak][$dai]['value'] = '<div style="clear:left;"><input type="hidden" name="remove_promotion_link" id="remove_promotion_link" class="input_box" value="" /></div>';
$dai++;
/*####### END NEW PROMOTION FIELD #########*/
...
if($fromtemp){
	...
	$dai++;
	$displayArray[$dak][$dai]['title'] = '&nbsp;';
	$displayArray[$dak][$dai]['value'] = '<div id="show_buttons" style="display:none;"><input class="button" type="submit" id="btnSave" '.$disabledButton.' name="save" value="'.$button.'" /> &nbsp; <input class="button" type="submit" name="cancel" value="Cancel" onclick="self.close(); return false;" />
	...
	if($updID!='') {
		...
		$dai++;
         ...
         // Restrict media channel email case($disabledButton)
	$displayArray[$dak][$dai]['title'] = 'Mark As';
	$displayArray[$dak][$dai]['value'] = '<div id="show_buttons" style="display:none;"><input class="button" id="btnSaver" type="submit" '.$disabledButton.' name="saver" value="';
	if(checkGroup(27)) {
		$displayArray[$dak][$dai]['value'] .= 'Approved';
	}
	else {
	...
	if(checkGroup(28) && $updID!=''){
		$displayArray[$dak][$dai]['value'] .= '<input class="button" id="btnRep" '.$disabledButton.' type="submit" name="rep" value="Reprocessed" onclick="return doSaved(3);" /> &nbsp; ';
	}
	if(checkGroup(27)) {
		$displayArray[$dak][$dai]['value'] .= '<input class="button" id="btnUna" '.$disabledButton.' type="submit" name="una" value="Unapproved" onclick="return doSaved(2);" /> &nbsp; ';
	}
	$displayArray[$dak][$dai]['value'] .= '<input class="button" id="btnProb" '.$disabledButton.' type="submit" name="prob" value="Problem" onclick="return doSaved(4);" /> &nbsp; <input class="button" type="button" value="Cancel" onclick="location.href=\'manageproduct.php\'; return false;" />';
	if($is_citi=='1' || in_array(266,$sectorID) || in_array(315,$sectorID)) {
		$displayArray[$dak][$dai]['value'] .= ' &nbsp; <input class="button"  id="btnUnused" '.$disabledButton.' type="submit" name="prob" value="Unused" onclick="return doSaved(-1);" />';
	}
	if($updID!='' && checkGroup(23)) {
		$displayArray[$dak][$dai]['value'] .= ' &nbsp; <input class="button" id="btnProdDelete" '.$disabledButton.' type="button" value="Delete" onclick="doProdDelete('.$updID.'); return false;" />';
	}
	if($updID=='' && !isset($_GET['muid'])) {
		$displayArray[$dak][$dai]['value'] .= '<br /><input class="button" id="btnProdSaveAndAdd" '.$disabledButton.' type="submit" name="saveAndAdd" value="';
		if(checkGroup(27)) {
			$displayArray[$dak][$dai]['value'] .= 'Approved';
		}
		else {
