...
if (!empty($_POST['bonus_point'])) {
    ...
    foreach ($_POST['bonus_point'] as $key => $val) {
        $sqlhistory = "INSERT INTO cscan_consumer_scoring_report_history (id,panelist_id,competi_id,parent_panelist_id,direct_mail_piece,direct_mail_point,
                                    email_piece,email_piece_point,digital_point,bonus_point,total_point,bag_remaining,entry_date,bagupdate_by,bagupdate_date,reset_by,reset_date,add_bonus_point_date,add_bonus_point_by
                                    )
        ...
        $updatesql = "update cscan_consumer_scoring_report_total set
                             bonus_point='" . $val . "',
                             total_point=( direct_mail_point+email_piece_point+ digital_point+" . $val . ")   
                             $addsql     
                             where id = '" . $key . "'
                           ";
        $ressql     =   $DRW->query($updatesql, $DRW_main);
        $sqlselpanelist    =   "SELECT panelist_id FROM cscan_consumer_scoring_report_total
                                WHERE  id ='".$key."'
                            ";
        $rspanelist  =   $DRW->query($sqlselpanelist,$DRW_main);        
        $rowpanelist =   $DRW->fetch_assoc($rspanelist);
        $panelist_id =   $rowpanelist['panelist_id'];
        $updaterep   =   "update cscan_consumer_scoring_report set
                             bonus_point='" . $val . "'                              
    ...
    foreach ($_POST['remainingbags'] as $key => $val) {
        ...
        if ($DRW->num_rows($rsselect) > 0) {
            ...
            if ($val != $rowsel['bag_remaining']) {
                $sqlhistory = "INSERT INTO cscan_consumer_scoring_report_history (id,panelist_id,competi_id,parent_panelist_id,direct_mail_piece,direct_mail_point,
                                    email_piece,email_piece_point,digital_point,total_point,bonus_point,bag_remaining,entry_date,bagupdate_by,bagupdate_date,reset_by,reset_date,add_bonus_point_date,add_bonus_point_by 
                                    )
                                    SELECT id,panelist_id,competi_id,parent_panelist_id,direct_mail_piece,direct_mail_point,
                                    email_piece,email_piece_point,digital_point,total_point,bonus_point,bag_remaining,entry_date,bagupdate_by,bagupdate_date,reset_by,reset_date,add_bonus_point_date,add_bonus_point_by
                                    FROM   cscan_consumer_scoring_report_total
                ...
                $updatesl = "update cscan_consumer_scoring_report_total set
                                      bag_remaining='" . $val . "'
                                      $addsql   
                                      where id = '" . $key . "'
                                    ";
        ...
        $msg=1;
        //header("location:consumer_scoring_report.php?msg=2");
    }
}
...
function resetdata($id) {
    ...
    $msg = '';
    $userid = $GLOBALS['AUTH_DATA']['userID'];
    $sqlrest = " SELECT id,panelist_id, competi_id, direct_mail_piece, 
                    direct_mail_point, email_piece, email_piece_point, 
                    digital_point,bonus_point, total_point, bag_remaining, entry_date
    ...
    if ($DRW->num_rows($rsreset) > 0) {
        ...
        if ($rowreset['total_point'] >= MAX_RESET_POINT) {
            ...
            $newdirect_mail_piece   =   $rowreset['direct_mail_piece'];
            ...
            $newdigital_point       =   $rowreset['digital_point'];
            $date                   =   date('Y-m-d H:i:s');
            ...
            if ($remaining_total_point > 0) {
                if (($rowreset['email_piece_point'] >= $remaining_total_point)) {
                    ...
                    $newemail_piece = round(($newemail_piece_point / EMAIL_PIECE_MULTIPLIER), 2);
                    $remaining_total_point = 0;
                } else {
            ...
            if ($remaining_total_point > 0) {
                if (($rowreset['direct_mail_point'] >= $remaining_total_point)) {
                    ...
                    $newdirect_mail_piece = round(($newdirect_mail_point / DIRECT_EMAIL_PIECE_MULTIPLIER), 2);
                    $remaining_total_point = 0;
                } else {
                    $newdirect_mail_point = 0;
                    $newdirect_mail_piece = 0;
                    $remaining_total_point = ($remaining_total_point - $rowreset['direct_mail_point']);
                }
            }
            
            if ($remaining_total_point > 0) {
                if (($rowreset['digital_point'] >= $remaining_total_point)) {
                    $newdigital_point=  ($newdigital_point-$remaining_total_point);
                    $remaining_total_point=0;
                } else {
                    $remaining_total_point  =   ($remaining_total_point-$newdigital_point);
                    $newdigital_point       =   0;
                }
            }
            
            /* if ($remaining_total_point > 0 && $rowreset['digital_point']>0) {
                if (($remaining_total_point <= DIGITAL_MAIL_POINT )) {
                    $newdigital_point = ($rowreset['digital_point']-DIGITAL_MAIL_POINT);
                    $remaining_total_point = 0;
                }else if (($remaining_total_point > DIGITAL_MAIL_POINT && $remaining_total_point>= $rowreset['digital_point'])) {
                    $newdigital_point = 0;
                    $remaining_total_point = ($remaining_total_point-$rowreset['digital_point']);
                }
                else if (($remaining_total_point > DIGITAL_MAIL_POINT && $remaining_total_point< $rowreset['digital_point'])) {
                    $remaining_total_point = 0;
                    $newval =   ($rowreset['digital_point']-$remaining_total_point);
                    if($newval<=DIGITAL_MAIL_POINT){
                        $newdigital_point = DIGITAL_MAIL_POINT;
                    }else{
                        $addedvalue=($newval%DIGITAL_MAIL_POINT);
                        if($addedvalue>0){
                            $assignedval =  ($newdigital_point-$addedvalue );
                            $newdigital_point=($assignedval+DIGITAL_MAIL_POINT);
                        }else{
                            $newdigital_point=$addedvalue;
                        }
                    }
...
//                }
            ...
            */
            ...
            $sqlhistory = "INSERT INTO cscan_consumer_scoring_report_history (id,panelist_id,competi_id,parent_panelist_id,direct_mail_piece,direct_mail_point,
                                    email_piece,email_piece_point,digital_point,bonus_point,total_point,bag_remaining,entry_date,reset_status,bagupdate_by,bagupdate_date,reset_by,reset_date,add_bonus_point_date,add_bonus_point_by
                                    )
                                    SELECT id,panelist_id,competi_id,parent_panelist_id,direct_mail_piece,direct_mail_point,
                                    email_piece,email_piece_point,digital_point,bonus_point,total_point,bag_remaining,entry_date,1,bagupdate_by,bagupdate_date,reset_by,reset_date,add_bonus_point_date,add_bonus_point_by
                                    FROM   cscan_consumer_scoring_report_total
                                WHERE  id ='" . $id . "' 
                                ";
            $DRW->query($sqlhistory, $DRW_main);
            ...
            $updatebonus = "update cscan_consumer_scoring_report set
                             bonus_point='" . $newbonus_point . "',
                             direct_mail_piece='" . $newdirect_mail_piece . "',
                             direct_mail_point='" . $newdirect_mail_point . "',
                             email_piece='" . $newemail_piece . "',
            ...
            $updatepointreport = "update cscan_consumer_scoring_report set
                             bonus_point='0',
                             direct_mail_piece='0',
                             direct_mail_point='0',
                             email_piece='0',
...
if (!empty($_REQUEST['clear']) && trim($_REQUEST['clear']) == 'Clear') {
    ...
    $tdt = date('Y-m-d');
    header("location:consumer_scoring_report.php");
}
...
if (!empty($_REQUEST['export']) && trim($_REQUEST['export']) == 'Export') {//pr($_POST);die;
    ...
    if (!empty($exp_rs)) {
        //total email sent        
        $arrExport['data'][] = array("Panelist ID", "Direct Mail Piece",
        ...
        while ($exp_row = $DRW->fetch_assoc($exp_rs)) {
            ...
            if (empty($bag_remaining))
                $bag_remaining = 0;
            $arrExport['data'][] = array($exp_row['competi_id'], $direct_mail_piece,
                $direct_mail_point, $email_piece, $email_piece_point,
                $digital_point, $bonus_point, $total_point, $bag_remaining,
            );
        }
    }
    download_send_headers("consumer_scoring_report" . date("Y-m-d") . ".csv");
...
<table width='100%' border='0' cellspacing='0' cellpadding='8' class='text' rules='none' bordercolor='#B9B9B9' align='center' style='border-collapse:collapse'>
    <tr><td class="adminhead" align='center'>Consumer Scoring Report</td></tr>
...
?> 
   <tr>  <td align='right'>
           <a href="consumer_scoring_monthly_report.php" class="button" style="padding:5px;text-decoration: none;float:right;margin-right: 10px; color: #fff;">Monthly Report</a>
...
<?php if (!empty($numrows)) { ?>
        ...
        <!--                    <td>
                        ...
                        <?php /* ?>
                          <tr>
                          <td  align="left"  class="adminhead" height='15px'><input name="setUnset" onclick="setAll()" value="on" type="checkbox"></td>
                          <td align="left"  class="adminhead" height='15px' ><b>Panelist ID</b><?php doSort($sort,$order,1,$p,$competi_id, $panelist_type); ?></td>
                          <td align="left" class="adminhead" height='15px' ><b>Direct Mail Pieces</b><?php doSort($sort,$order,2,$p,$competi_id, $panelist_type); ?></td>
                          <td align="left" class="adminhead" height='15px' ><b>Direct Mail Point</b><?php doSort($sort,$order,3,$p,$competi_id, $panelist_type); ?></td>
                          <td align="left" class="adminhead" height='15px' ><b>Email Pieces</b><?php doSort($sort,$order,4,$p,$competi_id, $panelist_type); ?></td>
                          <td align="left" class="adminhead" height='15px' ><b>Email Point</b><?php doSort($sort,$order,5,$p,$competi_id, $panelist_type); ?></td>
                          <td align="left" class="adminhead" height='15px' ><b>Digital Point</b><?php doSort($sort,$order,6,$p,$competi_id, $panelist_type); ?></td>
                          <td align="left" class="adminhead" height='15px' ><b>Total Point</b><?php doSort($sort,$order,7,$p,$competi_id, $panelist_type); ?></td>
                          <td align="left" class="adminhead" height='15px' ><b>Bags Remaining</b><?php doSort($sort,$order,8,$p,$competi_id, $panelist_type); ?></td>
                          <td align="left" class="adminhead" height='15px' ><b>Action</b></td>
                          </tr>
                          <?php */ ?>
                        <tr>
    ...
    ?>   
                                    <a style="background-color: #14734F;color: #FFFFFF;font-weight: bold;font-family: Verdana, Arial, Helvetica, sans-serif;font-size: 11px;" href="<?php echo $param; ?>">
                                        Bonus Point</a></b><?php doSort($sort, $order, 9, $p, $competi_id, $panelist_type); ?></td>
    ...
    ?>   
                                ...
                                </b><?php doSort($sort, $order, 8, $p, $competi_id, $panelist_type); ?></td>
                        ...
                        while ($row = $DRW->fetch_assoc($rs)) {
                            ...
                            ?>
                                ...
                                <td align="left">
                                    ...
                                    <a href="javascript:void(0)" onclick="openpopupViewHistory(<?php echo $row['panelist_id']; ?>);">
                                    View
                                    </a>
                                    &nbsp;&nbsp;&nbsp;
                                    <?php //} ?>
...
<script type="text/javascript">
    ...
    function openpopup(id) {
        var wind = window.open('consumer_score_log.php?id=' + id, "winpop", "left=0, top=0, scrollbars=yes, resizable=yes, width=400, height=300");
        wind.focus();
    }
    function openpopupViewHistory(id){
        var wind = window.open('consumer_scoring_history.php?pid=' + id, "winpop", "left=0, top=0, scrollbars=yes, resizable=yes, width=650, height=650");
        wind.focus();
    }
