...
if (!empty($_REQUEST['clear']) && trim($_REQUEST['clear']) == 'Clear') {
    header("location:mortgage_broker_scoring_monthly_reports.php");
}
$filter = array();
if (!empty($_REQUEST)) {
...
if (!empty($_REQUEST['export']) && trim($_REQUEST['export']) == 'Export') {//pr($_POST);die;
    $arrExport = array();
    $exp_sql="SELECT SQL_CALC_FOUND_ROWS panelist_id, competi_id,first_name,last_name,address,city,state,postalcode,email FROM cscan_panelists Where active=1 And parent_panelist_id=0 AND competi_id REGEXP '-55-' AND contact_type='prod_panelist' $conditions1 $order_bytxt $order_by";
    ...
    if (!empty($exp_rs)) {
        ...
        while ($exp_row = $DRW->fetch_assoc($exp_rs)) {
            ...
            $chkSql_month = "SELECT email_piece,email_piece_point,entry_date,insert_date FROM  cscan_mortgage_broker_scoring_monthly_reports WHERE  competi_id='".$competi_id."' $conditions";
    ...
    download_send_headers("mortgage_broker_scoring_monthly_reports_" . date("Y-m-d") . ".csv");
...
$sql = "SELECT SQL_CALC_FOUND_ROWS panelist_id,competi_id,first_name,last_name,address,city,state,postalcode,email FROM cscan_panelists Where active=1 AND parent_panelist_id=0 AND competi_id REGEXP '-55-' AND contact_type='prod_panelist' $conditions1 $order_bytxt $order_by LIMIT $p, $limit";
//echo $sql;
...
<table width='100%' border='0' cellspacing='0' cellpadding='8' class='text' rules='none' bordercolor='#B9B9B9' align='center' style='border-collapse:collapse'>
    <tr><td class="adminhead" align='center'>Mortgage Broker Scoring Monthly Report</td></tr>
     <?php if ($msg == '1') {
        ?>
                ...
                <form name="frmreport" id="frmreport" method="post" action="">
                    <table width='100%' border='0' cellspacing='0' cellpadding='8' class='text' rules='none' bordercolor='#B9B9B9' align='center' style='border-collapse:collapse'>
                        ...
                        while ($row = $DRW->fetch_assoc($rs)) {
                            ...
                            $chkSql_month = "SELECT email_piece,email_piece_point,entry_date,insert_date FROM  cscan_mortgage_broker_scoring_monthly_reports WHERE competi_id='".$competi_id1."' $conditions";
                            $chkQuery_month = $DRW->query($chkSql_month, $DRW_read);
                            $email_piece_point='0';
                            $email_piece='0';
                            if($DRW->num_rows($chkQuery_month) > 0){
