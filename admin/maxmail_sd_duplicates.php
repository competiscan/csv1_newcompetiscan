...
$sql_last = "SELECT DATE_FORMAT(datetime, '%Y-%m-%d') AS last_date FROM cscan_damaxmail_sameday_duplicate ORDER BY datetime DESC LIMIT 1";
...
/* $sql = "SELECT SQL_CALC_FOUND_ROWS pp.competi_id,pdd.subject,pdd.id,pdd.muid,pdd.panelist_id,pp.parent_panelist_id,pdd.percentage_match,pdd.duplicate_file,pdd.duplicate_with,pdd.datetime
        FROM cscan_damaxmail_sameday_duplicate pdd
...
$sql = "SELECT SQL_CALC_FOUND_ROWS pp.competi_id,pdd.subject,pdd.muid,pdd.percentage_match,pdd.datetime,SUBSTRING_INDEX(duplicate_file, '/', -3) AS duplicate_file, SUBSTRING_INDEX(duplicate_with, '/', -3) AS duplicate_with,SPLIT_STR(SUBSTRING_INDEX(duplicate_with, '/', -1), '_', 2) as mailbox_uid,SPLIT_STR(SPLIT_STR(SUBSTRING_INDEX(duplicate_with, '/', -1), '_', 3),'.',1) as origional_pid
        FROM cscan_damaxmail_sameday_duplicate pdd
        LEFT JOIN cscan_panelists pp ON(pdd.panelist_id=pp.panelist_id)
        WHERE DATE_FORMAT(datetime, '%Y-%m-%d') = '".$cdt."'
        ORDER BY $order_by LIMIT $p,$limit";
...
<table width='100%' border='0' cellspacing='0' cellpadding='8' class='text' rules='none' bordercolor='#B9B9B9' align='center' style='border-collapse:collapse'>
    ...
    <!-- <tr><td class="adminhead" align='center' colspan="7"><a href="maxmail_sd_duplicates.php" style="color: #ffffff;">DA's Maxmaxil Sameday Duplicate Records</a></td></tr> -->
    ...
    <tr>
        <td colspan="7">
            <table width='100%' border='0' cellspacing='0' cellpadding='8' class='text' rules='none' bordercolor='#B9B9B9' align='center' style='border-collapse:collapse'>
                ...
                <tr>
                    <td align="left" class="adminhead" height='15px' ><b>Email Subject</b></br><?= doSort($sort,$order,1,$p,$cdt);?></td>
                    ...
                    <td align="left" class="adminhead" height='15px' ><b>Indexed File</b></br>&nbsp;</td>
                ...
                $className='';               
                 while($row = $DRW->fetch_assoc($rs)){//pr($row);die;
                    ...
                    //if(empty($row['parent_panelist_id'])){//echo $row['filename'];die;
                        //$path = str_replace("admin","",dirname(__FILE__));
                        /* $f1 = str_replace($path,"",$row['duplicate_with']);
                        $f2 = str_replace($path,"",$row['duplicate_file']);
                        $approved_file = basename($row['duplicate_with']);
                        $duplicate_file = basename($f2);
                        $approved_file2 = explode(".",$approved_file);
                        $arr_approved = explode("_",$approved_file2[0]); */
                        //$muid_pid = current($arr_approved);
                        /* $mailbox_uid = $arr_approved[1]; */

                        $f1 = $row['duplicate_with'];
                        $f2 = $row['duplicate_file'];
                        $approved_file = basename($f1);
                        $duplicate_file = basename($f2);
                        $mailbox_uid = $row['mailbox_uid'];

                        $orig_muid_pid = '';
                        if($mailbox_uid){
                            $sql = "SELECT muid FROM cscan_email WHERE mailbox_uid='".$mailbox_uid."' LIMIT 1";
                            $query = $DRW->query($sql,$DRW_read);
                            if($DRW->num_rows($query)>0){
                                $reslt = $DRW->fetch_assoc($query);
                                $orig_muid_pid = $reslt['muid'];
                            }
                        }
                        if(empty($orig_muid_pid)){
                            continue;
                        }
                        /* $origional_pid = $arr_approved[2]; */
                        $origional_pid = $row['origional_pid'];
                        //$origional_pid = str_replace("_","&p_id=",$approved_file2);
                        /* $approved_file3 = explode("_",$approved_file2);
                        $origional_pid = (int)end($approved_file3); */
                    ?>
                        <tr valign=top class="<?php echo $className; ?>">
                            <td><?php echo '<a href="../temp_product.php?muid='.$orig_muid_pid.'&p_id='.$origional_pid.'&dmuid='.$row['muid'].'" target="_blank">'.$row['subject'].'</a>';?></td>
                            ...
                            <td><?= $row['datetime'];?></td>
                            <td><?php
                                echo '<a href="pdfcompare.php?type=maxmail_tmp&u1='.urlencode($f1).'&u2='.urlencode($f2).'" target="_blank">Compare</a>';
                            ?></td>
                        </tr>
                    <?php //}?>
                <?php }?>
