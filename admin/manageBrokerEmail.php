...
$sort='';
?>
<table width='100%' border='0' cellspacing='0' cellpadding='12' class='text' rules='none' bordercolor='#B9B9B9' align='center' style='border-collapse:collapse'>
<tr><td class="adminhead" align='center' colspan='12'>Mortgage Broker Mail</td></tr>
...
if(isset($_REQUEST['delID']) && ($_REQUEST['delID'] > 0)) {
	...
	for($i=0;$i<count($delID);$i++){
		$delThis = $delID[$i];
		$sql_sel = "SELECT id FROM cscan_sbkc_broker WHERE id = '$delThis'";
		$res = $DRW->query($sql_sel,$DRW_read);
                
		if ($DRW->num_rows($res) > 0) {
			$sql="DELETE FROM cscan_sbkc_broker WHERE id = '$delThis'";			
                    /* Added for track on delete operation */
                        ...
                        $track_delete_data = [
                                ...
                                'delete_type' => 'SBKC Mortgage Broker Mail',
                                'is_mobile' => isMobile(),
        ...
        /* Added for track on delete */
            if(count($emailData)>0){
                ...
                $html .= '</table>';
                sendDevAlert('Caution! Data Deleted From Mortgage Broker Mail',$html);
            }
...
$sql = "SELECT id,first_name,last_name,email,company,address,city,state,zip,phone,refered_by,inserted_date FROM cscan_sbkc_broker order by id DESC  LIMIT $p, $limit";
$rs = $DRW->query($sql,$DRW_read);
$resultCount = $DRW->num_rows($rs);


$numquery = "Select COUNT(id) as numrows FROM cscan_sbkc_broker";
...
?>
<tr>
<td width='1%' class="adminhead">
  <?php if(checkGroup(101)){?>
    <input type='checkbox' name='setUnset' onclick='setAll()'>
  <?php }?>
</td>       
<td align="left" width='7%' class="adminhead"><b>First Name</td>
<td align="left" width='7%' class="adminhead"><b>Last Name</td>
<td align="left" width='9%' class="adminhead"><b>Email</td>
<td align="left" width='8%' class="adminhead"><b>Company</td>
<td align="left" width='14%' class="adminhead"><b>Address</td>
<td align="left" width='5%' class="adminhead"><b>City</td>
<td align="left" width='4%' class="adminhead"><b>State</td>
<td align="left" width='4%' class="adminhead"><b>Zip</td>
<td align="left" width='5%' class="adminhead"><b>Phone</td>
<td align="left" width='12%' class="adminhead"><b>Referred By </td>
<td align="left" width='20%' class="adminhead"><b>Send Date</td>
</tr>
<tr><td colspan='12' align='center' class='error'><?php echo $message; ?></td></tr>
<?php
if($resultCount > 0) {
	$className='';
	while($row = $DRW->fetch_array($rs)){
		...
		$email 	= $row['email'];
                ...
                $zip = $row['zip'];
		$phone 	= $row['phone'];
		$refered_by = trim(stripslashes($row['refered_by']));                
		...
		<tr valign=top class="<?php echo $className; ?>">
			 ...
			 <td><?php echo nl2br($refered_by); ?></td>
...
<?php }
	...
	echo "<tr><td colspan=12 class='error' align=center>There are no record exist.</td></tr>";
