...
$sql_last = "SELECT DATE_FORMAT(datetime, '%Y-%m-%d') AS last_date FROM cscan_product_detail_duplicate ORDER BY datetime DESC LIMIT 1";
...
$sql = "SELECT SQL_CALC_FOUND_ROWS pp.competi_id,pd.productName,pdd.id,pdd.productID,pdd.panelist_id,pdd.percentage_match,pdd.duplicate_filename,pdd.filename,pdd.datetime,pdd.is_telecom
        ...
        <td colspan="7">
            <table width='100%' border='0' cellspacing='0' cellpadding='8' class='text' rules='none' bordercolor='#B9B9B9' align='center' style='border-collapse:collapse'>
                ...
                <tr>
                    ...
                    <td align="left" class="adminhead" height='15px' ><b>Panelist</b></br><?= doSort($sort,$order,2,$p,$cdt);?></td>
                    <td align="left" class="adminhead" height='15px' ><b>Match(%)</b></br><?= doSort($sort,$order,3,$p,$cdt);?></td>
                    <td align="left" class="adminhead" height='15px' ><b>Approved File</b></br><?= doSort($sort,$order,4,$p,$cdt);?></td>
                    <td align="left" class="adminhead" height='15px' ><b>Deleted File</b></br><?= doSort($sort,$order,5,$p,$cdt);?></td>
                ...
                $className='';                
                 while($row = $DRW->fetch_assoc($rs)){//pr($row);die;
                    ...
                    if(empty($row['parent_panelist_id'])){//echo $row['filename'];die;
                        ...
                        $approved_file = str_replace($root, "",$row['duplicate_filename']);
                        $duplicate_file = str_replace("\\","/",str_replace($root, "", $row['filename']));
                    ...
                    ?>
                        <tr valign=top class="<?php echo $className; ?>">
                            ...
                            <td><?= basename($approved_file);?></td>
                            <td><?= $duplicate_file;?></td>
                            <td><?= $row['datetime'];?></td>
                            <td><?php
                                echo '<a href="pdfcompare.php?u1='.urlencode($approved_file).'&u2='.urlencode($duplicate_file).'" target="_blank">Compare</a>';
                            ?></td>
                        </tr>
                    <?php }?>
                <?php }?>
