...
//    
    ...
    if(isset($_POST['productStatus']) && $_POST['productStatus']==1){
        ...
        if(empty($_POST['mChannelID']) || empty($_POST['mPanelID']) || empty($_POST['scsc_comboIDs']) || empty($_POST['co_comboIDs'])){
            $redirect_url1 = "addproduct.php?new=1&error_req=1";
            if($updID!=''){
             $redirect_url1 = "addproduct.php?id=$updID&error_req=1";
            }
            ob_end_clean();
...
//                }
        ...
        if ($updID != '') {
            ...
            $DRW->query($sql, $DRW_main);
            // for sinking purpose
            InsertAndUpdateProductID($updID);
        } else {
       ...
       ####################### end for display the product image under temp ###############       
        ...
        if ($productStatus == 1 && $variantID != 0) {
            ...
            $DRW->query($sqlu, $DRW_main);
            // for sinking purpose
            InsertAndUpdateProductID($variantID);
        }
...
function does_url_exists($url) {
    ...
    if(trim($url_contents)=='' OR trim($url_contents)=='this pdf not exist'){
        $status = false;
    }
