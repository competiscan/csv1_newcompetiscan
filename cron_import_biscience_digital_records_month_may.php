...
$sql = "SELECT id,creation_date,location,channel,advertiser_name,compaign_title,creative_wrapper,publisher,impressions,spend,monitored_page,file_id,advertiser_domain,campaign_landing_page FROM cscan_digital_records202101 where status=0 AND LEFT(creation_date,10) >='2021-06-01' ORDER BY id limit 4500 ";
//$sql = "SELECT id,creation_date,location,channel,advertiser_name,compaign_title,creative_wrapper,publisher,impressions,spend,monitored_page,file_id,advertiser_domain,campaign_landing_page FROM cscan_digital_records202101_test where status=0 ORDER BY id limit 6000 ";
...
if($DRW->num_rows($result)>0){
    while( $row = $DRW->fetch_row($result)){
        ...
        if($DRW->num_rows($resultDup)>0){
            ...
            $sqlR = "Update cscan_digital_records202101 set status=2 where id='".$id."'";
            ...
            $sqlR = "Update cscan_digital_records202101 set status=1 where id='".$id."'";
            $DRW->query($sqlR, $DRW_main);
        }        
    }    
    echo 'completed: ';
