...
if(isset($_REQUEST['mchannel']) AND !empty($_REQUEST['mchannel'])) {
    if($_REQUEST['mchannel'][0]!=''){
      ...
      $postdigita_data['mchannel']=$mchannel_id;
       ...
       unset($mchannel[0]);
       if(!empty($mchannel)){
       $mchannel_id = array_map('intval', $_SESSION['mchannel']);
       $postdigita_data['mchannel']=$mchannel_id;
       }
    }
 }
...
// Priority: REQUEST → SESSION → default empty
if (!empty($_REQUEST['sector_list'])) {
    $sector_list_array = $_REQUEST['sector_list'];
    $_SESSION['sector_list'] = $sector_list_array;

} elseif (!empty($_SESSION['sector_list']) && (isset($_REQUEST['search_button'])!='Search')) {
...
if (!empty($sector_list_array)) {
    ...
    // Remove '0' if present as first element
    if (isset($sector_list_array[0]) && $sector_list_array[0] == '0') {
        unset($sector_list_array[0]);
    }

    // Re-check after unset
    if (!empty($sector_list_array)) {
        $postdigita_data['sector_id'] = array_values(array_map('intval', $sector_list_array));
    }
}
/*############ Category LIST################*/
$category_list_array = [];

// Priority: REQUEST → SESSION → default
if (!empty($_REQUEST['category_list'])) {
    $category_list_array = $_REQUEST['category_list'];
    $_SESSION['category_list'] = $category_list_array;

} elseif (!empty($_SESSION['category_list']) && (isset($_REQUEST['search_button'])!='Search')) {
...
if (!empty($category_list_array)) {
    ...
    // Remove '0' if it's the first element
    if (isset($category_list_array[0]) && $category_list_array[0] == '0') {
        unset($category_list_array[0]);
    }

    // Re-check after cleanup
    if (!empty($category_list_array)) {
        $postdigita_data['category_id'] = array_values(array_map('intval', $category_list_array));
    }
}

/*############ SUBCategory LIST################*/
$subcategory_list_array = [];

// Priority: REQUEST → SESSION → default
if (!empty($_REQUEST['subcategory_list'])) {
    $subcategory_list_array = $_REQUEST['subcategory_list'];
    $_SESSION['subcategory_list'] = $subcategory_list_array;

} elseif (!empty($_SESSION['subcategory_list']) && (isset($_REQUEST['search_button'])!='Search')) {
...
// Process subcategory list
if (!empty($subcategory_list_array)) {

    // Remove '0' if first element
    if (isset($subcategory_list_array[0]) && $subcategory_list_array[0] == '0') {
        unset($subcategory_list_array[0]);
    }

    // Re-check after cleanup
    if (!empty($subcategory_list_array)) {
        $postdigita_data['subcategory_id'] = array_values(array_map('intval', $subcategory_list_array));
    }
}
...
if (!empty($_REQUEST['state'])) {
    ...
    if (!empty($state)) {
        //print_r($state);
        $postdigita_data['state'] = array_values($state);
    }
...
if(isset($_REQUEST['city']) AND !empty($_REQUEST['state'])){
    if($_REQUEST['city'][0]!=''){
      ...
      $postdigita_data['city'] = $city;
       ...
       unset($city[0]);
       if(!empty($city)){
       $postdigita_data['city'] = array_values($city);
       }
    }
...
// echo "</pre>";
$posted_digital_data=json_encode($postdigita_data,JSON_UNESCAPED_SLASHES);
...
<div id="page">
            ...
            <div class="row">
                ...
                <div class="col-md-4">
                    ...
                    <div>
                       ...
                       $o_count_country = count(explode(',', $country));
                         if(is_array($_SESSION['state'])) { $o_count_country_sess = count($_SESSION['state']); }
                         $selectany='';
                ...
                <div class="col-md-4">
                    ...
                    <div>
                        <select name="city[]" id="city_list" multiple="multiple" size="3"
                            ...
                            <?php
                            if (!empty($state)) {
                                $post = array('country' => $country, 'state' => array_values($state));
                                $posted_data = json_encode($post);
                ...
                <div class="col-md-4">
                    ...
                    <select name="category_list[]" id="category_list_data_htm" onchange="getSubCategory();"  multiple="multiple" size="3" class="combo_box" tabindex="6">
                        <?php 
                         ...
                         $o_count_sec_sess = count($category_list_array);
                         $selectany='';
                         if ($o_count_sec == 0 || $o_count_sec_sess==0 || $category_list_array=='') {
                             $selectany='selected=selected';
                         }
                        ?>    
                        <option value="0" <?php echo $selectany;?>>Any</option>
                            <?php 
                               if(!empty($sector_list_array)){
                                // echo "<pre>";
                                // print_r($sector_list_array);
                                // echo "</pre>";
                                $arrayValue = array_values(array_map('intval', $sector_list_array));
                                ...
                                if(!empty($response_category)){
                                    foreach($response_category as $getCateName) {
                                        if(!in_array($getCateName['sectorID'],$_SESSION['sess_category'])){
                                         ...
                                         ?>
                                    <option  <?php if(in_array($getCateName['sectorID'],$category_list_array)) { echo "selected"; } ?> value="<?php echo $getCateName['sectorID'];?>" ><?php echo htmlspecialchars($getCateName['sectorName'], ENT_QUOTES); ?></option> 
                               <?php 
                                }
                                }
                               }
                ...
                <div class="col-md-4">
                    ...
                    <select name="subcategory_list[]" id="subcategory_list" onchange="getSubSubCategory();" multiple="multiple" size="3" class="combo_box" tabindex="7">
                        <?php 
                         ...
                         $o_count_cat_sess = count($subcategory_list_array);
                         $selectany='';
                         if ($o_count_cat == 0 || $o_count_cat_sess==0 || $subcategory_list_array=='') {
                             $selectany='selected=selected';
                         }
                        ?>    
                        <option value="0" <?php echo $selectany;?>>Any</option>
                            <?php 
                               if(!empty($category_list_array)){
                                ...
                                $arrayValue = array_values(array_map('intval', $category_list_array));
                                $postSector['sectors']=$arrayValue;
                                $posted_data=json_encode($postSector);
                                //echo $posted_data;
                                $APISECTORURL=RPVAPIURL.'data/sectors';
                                ...
                                if(!empty($response_subcategory)){
                                    foreach($response_subcategory as $getSubCateName) {
                                         ...
                                         ?>
                                    <option  <?php if(in_array($getSubCateName['sectorID'],$subcategory_list_array)) { echo "selected"; } ?> value="<?php echo $getSubCateName['sectorID'];?>" ><?php echo htmlspecialchars($getSubCateName['sectorName'], ENT_QUOTES); ?></option> 
                               <?php 
                                }
                                }
                               }
    ...
    <div>
        ...
        <form name="outputForm" id="outputForm" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
            <?php if($num_of_rows <=100000 && $num_of_rows >0) {  ?>
