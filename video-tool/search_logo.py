				...
				if frame_records is not None:		
					...
					for frameval in frame_records:
						...
						try:							
							...
							print('Good=>'+str(len(good)),'Matched Points=>'+str(matched_points),'Matched Points2=>'+str(matched_points2),'Vid=>'+str(vid),'Lid=>'+str(lid),'Frame=>'+str(frame_name))
								
							#if((matched_points>=compare_points and len(good_matches)>=45 and len(good)>=48) or (len(good)>50 and len(good_matches)>=45)):
							if((len(good)>=76) or (matched_points>=25 and len(good)>=36) or (matched_points2>=111 and matched_points>=16)):		
...
cmd = ['pgrep -f .*python.*search_keyword_audio.py']
process = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, 
stderr=subprocess.PIPE)
my_pid, err = process.communicate()
if len(my_pid.splitlines()) >0:
   #print("Running")  
   os.system("pkill -f search_keyword_audio.py")

#subprocess.call(['python','/var/www/html/competiscan.com/video-tool/detect-text.py'])
subprocess.call(['python','/srv/httpd/competiscan.com/html/video-tool/detect-text.py'])	
