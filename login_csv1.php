...
if(isset($_POST['login'])){
	...
	if($count > 0) {
		...
		while($data2 = $DRW->fetch_row($result)){
		 ...
		 $_SESSION['sess_ai_analysis_link'] = array();
		 $result_q = $DRW->query("SELECT name FROM cscan_user_ai_analysis_allowed WHERE userID=$userID",$DRW_read);
		while($data2_ai = $DRW->fetch_row($result_q)){
					$_SESSION['sess_ai_analysis_link'][] = $data2_ai[0];
		}
...
?>
			...
			<form name="loginForm" action="login_csv1.php" method="post" onsubmit="return validate();">
