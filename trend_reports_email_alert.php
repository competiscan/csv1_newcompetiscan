...
<div class="col-md-3 pull-right">
        <span style="text-align:right;"> <a class="submitbutton" href="trend_reports_test.php">Trend Report Page</a> </spn>
</div>
...
<form name="trendForm" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
     <div style="text-align:left; font-family: Verdana, Arial, Helvetica, sans-serif;font-size: 13px; font-weight: bold;text-decoration: none;margin:0px 0px 16px 0px;padding:10px;border:solid 1px #cccccc;">
        ...
        <label><input type="radio" <?php //if(!empty($country_id) && $country_id==''){ echo "checked";} ?> name="country" checked value="">All</label>
    </div>
    <div style="text-align:left; font-family: Verdana, Arial, Helvetica, sans-serif;font-size: 13px; font-weight: bold;text-decoration: none;margin:0px 0px 16px 0px;padding:10px;border:solid 1px #cccccc;">
      <label valign="top"> Audience :</label> <br/>
 <?php  $mailing_panel = getMailingPanel();
                       // print_r($audience_id);die;
         foreach($mailing_panel as $mid=>$name){  ?>
        <input type="checkbox" name="audience[]" value="<?php echo $mid; ?>"/><?php echo $name; ?>
       <?php } ?>
    </div>
    <?php
...
function displayCategory($ID,$level=0) {
	...
	while($rowa = $DRW->fetch_array($rsa)) {
		...
		if(!in_array($catid,$_SESSION['sess_sector']) && !in_array($catid,$_SESSION['sess_category']) && !in_array($catid,$_SESSION['sess_subcategory'])){
		 ...
		 $sqltwo = "SELECT ctr.trend_id,trend_name,trend_link,file_path,file_name,ctc.sector_id,ctc.category_id,ctc.subcategory_id,ctc.subtosubcategory_id FROM cscan_trend_report ctr JOIN cscan_trends_category ctc ON 
                               ctc.trend_id = ctr.trend_id WHERE ctc.sector_id=$catid or ctc.category_id=$catid or ctc.subcategory_id = $catid or ctc.subtosubcategory_id = $catid  GROUP BY ctr.trend_id ORDER BY trend_date DESC";
		...
		if($innertext!='') {
			$print .= "<div";
			if($level==0) {
				$print .= " style=\"text-align:left; font-family: Verdana, Arial, Helvetica, sans-serif;font-size: 13px; font-weight: bold;text-decoration: none;margin:0px 0px 16px 0px;padding:10px;border:solid 1px #cccccc;\"";
			}
			else {
				$print .= " style=\"";
				if($level<2) {
...
?>
<div align="left"><input type="submit" name="submit" value="Receive Alert" class="submitbutton" /><input type="hidden" name="send" value="1" /></div>
</form>
...
if(count($savedArray) > 0) {
	foreach($savedArray as $sectorID=>$sectorName) {
		...
		echo " <tr class=\"$className\"><td class=\"bodytext\" valign=\"top\">$sectorName</td><td class=\"bodytext\" valign=\"top\">";
		if($trendArray[$sectorID]!=0) {
			...
			echo "<a href=\"trend_report.php?trend_id={$trendArray[$sectorID]}\">$row2[0]</a>";
		}
		else {
			echo '&nbsp;';
		}
