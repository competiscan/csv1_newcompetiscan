...
if($pid!=0){
	...
	while($dataC = $DRW->fetch_row($resultC)){
        ...
        if(!empty($cos)){
	echo "<div><strong>Affinities:</strong> ".implode(', ',$cos)."</div><hr />";
        }
	$cos = array();
	...
	while($dataC = $DRW->fetch_row($resultC)){
		$cos[] = $dataC[0];
          ...
          if(!empty($cos)){
	echo "<div><strong>Loyalty/Retention, Statement Companies:</strong> ".implode(', ',$cos)."</div><hr />";
          }
	if($_SESSION['sess_plevel']==1 || $_SESSION['sess_plevel']==2){
		...
		if(count($cos)>0){
			echo "<div><strong>Address Change History:</strong> ".implode(', ',$cos)."</div>";
		}
	}
}
?> 