...
if($_SERVER['argc']<3) {
	print "exportExcel_back.php sessionid serialized_array\n";
	exit;
}
...
if((in_array(1,$mPanelIDArray) || in_array(2,$mPanelIDArray)) && (in_array(1,$mChannelIDArray) || in_array(3,$mChannelIDArray))) $consumer = true;
else $consumer = false;

if($consumer && ((count($mPanelIDArray)==1 && (in_array(1,$mPanelIDArray) || in_array(2,$mPanelIDArray))) || (count($mPanelIDArray)==2 && in_array(1,$mPanelIDArray) && in_array(2,$mPanelIDArray)))) $consumer_only = true;
else $consumer_only = false;
...
if($consumer){
    ...
    if (!is21FilterOn($sess_userID))  $showheading['age'] = false;
	//$showheading['ficos'] = false;
	$showheading['gender'] = false;
	if (!is21FilterOn($sess_userID)) $showheading['income'] = false;
...
if(is_array($post_field) && count($post_field)>0){
	$fromExportAll = false;
...
if($ssid>0 || $bid>=0) {
	...
	if($bid>=0) {
		list($sql) = doQuery(0, false, '', false, $bid, false,false,false,false,-1,array(),$sess_userID);
	}
	else{
		...
		list($sql) = doQuery($ssid,false,'',false,-1,$dorelev,$doexpans, false,false,-1,array(),$sess_userID);
	}
	
	$sql .= $orderby;
	
	if($bid>=0) {
		list($countQuery) = doQuery(0, true, '', false, $bid, false,false,false,false,-1,array(),$sess_userID);
	}
	else{
		list($countQuery) = doQuery($ssid,true, '', false,-1, false,false,false,false,-1,array(),$sess_userID);
	}
	...
	while($row = $DRW->fetch_assoc($rs)) {
		...
		if(is_array($mult) && count($mult)==0 && (!empty($showheading['mailvolume']) || $is_mv)){
			$mult[] = array('','','',0,'','','','','','','','','','');
		}
				...
				if(!empty($showheading[$k])) {
					switch ($k) {
						...
						case 'AffinitySubCategoryID':
							if($row['affinityAssociation']==1) $name = 'Yes';
							else $name = 'No';
						...
						case 'creditUnion': 
							...
							$resultC = $DRW->query("SELECT count(*) FROM cscan_company pa,cscan_company_product pp 
								WHERE pa.companyID=pp.companyID AND pp.productID={$row['theproductID']} AND isCreditUnion=1",$DRW_read);
							$dataC = $DRW->fetch_row($resultC);
							if(!empty($dataC[0])){
								$yesno = 'Yes';
							}
...
function makeUTF($value){
	return iconv('CP1252', 'UTF-8', $value);
}
