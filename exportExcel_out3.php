...
if(!empty($cancel)){
	$proc_pid = running_php_cmd_pid('exportExcel_back3.php '.$_SESSION['sess_userID']);
	if(!empty($proc_pid)){
...
elseif(!empty($check)){
	...
	if($out){
		...
		$data = $DRW->fetch_row($rs);
		$pct = (int)$data[0];
		if($pct<300 && !running_php_cmd('exportExcel_back3.php '.$_SESSION['sess_userID'])){
			if($pct==250){
				$pct = -1;
			}
			else{
	...
	if(!running_php_cmd('exportExcel_back3.php '.$_SESSION['sess_userID'])){
		...
		exec("/usr/bin/php exportExcel_back3.php ".escapeshellarg($_SESSION['sess_userID'])." ".escapeshellarg(serialize($argsArray))."".$exec);
	}
