...
if($productID!=0){
    ...
    if($is_digital=='1' && $document_id!=''){       
        $query_mob = "SELECT max(document_id),document_filename,document_content_type,document_size_byte,UNIX_TIMESTAMP(document_createddate),document_path,bucket_name FROM $table WHERE productID=$productID ";
        $query_result_mob = $DRW->query($query_mob,$DRW_read);
        $data_prod_mob = $DRW->fetch_row($query_result_mob);
    }     
    if($document_id > 1 ){
        $query2 = "SELECT document_id,document_filename,document_content_type,document_size_byte,UNIX_TIMESTAMP(document_createddate),document_path,bucket_name FROM $table WHERE productID=$productID AND document_id=$document_id";
        $query_result2 = $DRW->query($query2,$DRW_read);
    ...
    if($document_content_type=='html' && $document_path!=''){
    ...
    }else if(($document_content_type=='video/mp4' || $document_content_type=='video/mpeg-4' || $document_content_type=='image/mpeg-4' || $document_content_type=='image/mp4') && $document_filename!=''){
        ...
        //$content='<video width="500" height="300" align="left" controls :src="' .$s3VideoURL. '" ></video>';
        ...
        if(!empty($document_filename) && !empty($document_path)){ 
            ...
            if($check_bucket_name!='NULL' AND $check_bucket_name!=''){
               @readfile($displays3CSV2URL.substr($document_path,0).$document_filename);
             }else{
