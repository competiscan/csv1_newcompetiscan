...
$dd=date("d");
$ymd=$yy."-".$mm."-".$dd;
$s_date=date("Y-m-01", strtotime($ymd));
$e_date=date("Y-m-t", strtotime($ymd));*/
$sql_log = "SELECT min_val,max_val FROM cscan_biscience_log where id='".trim($month)."'";
$result_log = $DRW->query($sql_log,$DRW_read2);
if($DRW->num_rows($result_log)>0){
    $dataLog = $DRW->fetch_row($result_log);
    $min_id=$dataLog[0];
    $max_id=$dataLog[1];
//$sql = "SELECT id,creation_date,location,channel,advertiser_name,compaign_title,creative_wrapper,publisher,impressions,spend,monitored_page,file_id,advertiser_domain,campaign_landing_page,compaign_title FROM cscan_digital_records where productID=0 AND creation_date>='".$s_date."' AND creation_date<='".$e_date."' Limit 2000"; die;
$sql = "SELECT id,creation_date,location,channel,advertiser_name,compaign_title,creative_wrapper,publisher,impressions,spend,monitored_page,file_id,advertiser_domain,campaign_landing_page,compaign_title FROM cscan_digital_records where productID=0 AND id >='".$min_id."' AND id <='".$max_id."' Limit 2000";
