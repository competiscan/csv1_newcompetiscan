		...
		if($date_choice==3 && $end>0){//month
			...
			for($i=$start;$i<=$end;$i++){
				...
				for($j=$start_m;$j<=$end_m;$j++){
					$imageDataLinks[] = 'graph_img.php?new='.date('YmdHis').'&amp;ym='.$i.'-'.str_pad($j,2,'0',STR_PAD_LEFT);
				}
			}
		}
			...
			for($i=$start;$i<=$end;$i++){
				if($date_choice==4){
					...
					foreach($quarters as $q=>$max){
						$imageDataLinks[] = 'graph_img.php?new='.date('YmdHis').'&amp;ym='.$i.'-'.str_pad($max1,2,'0',STR_PAD_LEFT).'&amp;ym2='.$i.'-'.str_pad($max,2,'0',STR_PAD_LEFT);
						$max1 = $max + 1;
					}
				}
				else{
					$imageDataLinks[] = 'graph_img.php?new='.date('YmdHis').'&amp;ym='.$i;
				}
			}
		}
	}
	else{
		$imageDataLinks[] = 'graph_img.php?new='.date('YmdHis');
	}
	if($pp){
		$SKIP_FUNCTION = true;
		require_once('graph_img.php');
