...
if(isset($_POST['sendmass']) && $_POST['muids']!=''){
	...
	if(!empty($_POST['copies_id']) && !empty($_POST['marked'])){
		...
		if(!empty($cmuid)){
			$sqlc = "SELECT deleted,panelist_core,panelist_score,email_read FROM cscan_email$hy WHERE muid='".$DRW->real_escape_string($cmuid)."'";
...
if(checkGroup(20)){
	...
	if(is_array($panelist_core_search) && count($panelist_core_search)>0){
...
$count_result = $DRW->query("SELECT COUNT(DISTINCT ce.`muid`) $q",$DRW_read);// SQL_NO_CACHE
	...
	while($data = $DRW->fetch_row($query_result)){
		$muid = $data[0];
           ...
           *  */ 
             ...
             $query2 = $DRW->query("SELECT COUNT(*) from cscan_email_text$hy WHERE muid='".$DRW->real_escape_string($muid)."'",$DRW_read);
