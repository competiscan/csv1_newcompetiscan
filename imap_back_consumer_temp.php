...
//delete records of day before today
$yesterday = date("Y-m-d", strtotime(" -1 day"));
$sql = "SELECT filename FROM `cscan_da_maxmail_html` WHERE date_created < '".date("Y-m-d")."'";
...
//    }
    ...
    $sql ="CREATE TABLE IF NOT EXISTS cscan_email_dup LIKE cscan_email_tmp";
    if($DRW->query($sql,$DRW_main)){
        $sql = "INSERT INTO cscan_email_dup(email_date,email_to,email_from,email_subject,contact_type_m_c,deleted,email_cc,email_bcc,panelist_score,mailbox_uid,email_read,email_from_one,panelist_id,e_assigned_admin_userID,panelist_core,email_stateID,isdelete,is_text_file) (SELECT email_date,email_to,email_from,email_subject,contact_type_m_c,deleted,email_cc,email_bcc,panelist_score,mailbox_uid,email_read,email_from_one,panelist_id,e_assigned_admin_userID,panelist_core,email_stateID,isdelete,is_text_file,from_sent_name,from_sent_email_address,from_sent_date,from_sent_date_format,is_fetch,isnamereplace FROM cscan_email_tmp)";
        if($DRW->query($sql,$DRW_main)){
            $sql ="DROP TABLE IF EXISTS cscan_email_tmp";  
            if($DRW->query($sql, $DRW_main)){
                $sql ="CREATE TABLE IF NOT EXISTS cscan_email_tmp LIKE cscan_email";
                $DRW->query($sql, $DRW_main);
            }
        }
    }
    
    $sql ="CREATE TABLE IF NOT EXISTS cscan_email_text_dup LIKE cscan_email_text_tmp";
    if($DRW->query($sql,$DRW_main)){
        $sql = "INSERT INTO cscan_email_text_dup(cetpart,cettext,cettype,muid,cetidentification,isdelete) (SELECT cetpart,cettext,cettype,muid,cetidentification,isdelete FROM cscan_email_text_tmp)";
        if($DRW->query($sql,$DRW_main)){
            $sql ="DROP TABLE IF EXISTS cscan_email_text_tmp"; 
            if($DRW->query($sql, $DRW_main)){
                $sql ="CREATE TABLE IF NOT EXISTS cscan_email_text_tmp LIKE cscan_email_text";
                $DRW->query($sql, $DRW_main);
            }
        }
    }
    
    $sql ="CREATE TABLE IF NOT EXISTS cscan_email_file_dup LIKE cscan_email_file_tmp";
    if($DRW->query($sql,$DRW_main)){
        $sql = "INSERT INTO cscan_email_file_dup(cefpart,cefdata,cefname,ceftype,muid,cefidentification,cefdisposition,cefencoding,cefsplit,cefpath,isdelete) (SELECT cefpart,cefdata,cefname,ceftype,muid,cefidentification,cefdisposition,cefencoding,cefsplit,cefpath,isdelete FROM cscan_email_file_tmp)";
        if($DRW->query($sql,$DRW_main)){
            $sql ="DROP TABLE IF EXISTS cscan_email_file_tmp";
            if($DRW->query($sql, $DRW_main)){
                $sql ="CREATE TABLE IF NOT EXISTS cscan_email_file_tmp LIKE cscan_email_file";
                $DRW->query($sql, $DRW_main);
            }
        }
    }
        
    $sql = "DROP TABLE IF EXISTS cscan_da_maxmail_html";
    if($DRW->query($sql, $DRW_main)){
        $sql = "CREATE TABLE IF NOT EXISTS cscan_da_maxmail_html (
...
$username1 = 'consumers@sbkcenter.com';
//$password1 = 'f5iAK3syA';
$password1 = '69cF205R';
...
foreach ($accountArray as $account) {
    ...
    if ($imap_stream === false) {
        ...
        //for ($i = 1; $i <=501; $i++) {
            ...
            //print_r($structure);
            //$query = "SELECT SQL_NO_CACHE COUNT(*) FROM `cscan_email_tmp` WHERE `mailbox_uid`='".$DRW->real_escape_string($mailbox_uid)."' AND `email_date`='".$DRW->real_escape_string($email_date)."'";
            /* if($subject!=''){
              $query = "SELECT SQL_NO_CACHE COUNT(*) FROM `cscan_email_tmp` WHERE email_subject='".$DRW->real_escape_string(strtolower($subject))."'";
              }else{ */
            $query = "SELECT SQL_NO_CACHE COUNT(*) FROM `cscan_email_tmp` WHERE `mailbox_uid`='" . $DRW->real_escape_string($mailbox_uid) . "' AND `email_date`='" . $DRW->real_escape_string($email_date) . "'";
            //  }
            if ($defaultcontact != '')
            ...
            if ($count[0] == 0) {
                ...
                if ($keep) {
                    ...
                    $result_c_p = $DRW->query("SELECT panelist_id,stateID FROM cscan_panelists WHERE active=1 AND (email='" . $DRW->real_escape_string($checkemail) . "' OR alt_email='" . $DRW->real_escape_string($checkemail) . "' OR more_email LIKE '%" . mysqlLike($checkemail) . "%') LIMIT 1", $DRW_read2);
                    $data_c_p = $DRW->fetch_row($result_c_p);
                    $panelist_id = (int) $data_c_p[0];
                    $email_stateID = (int) $data_c_p[1];
                    $DRW->free_result($result_c_p);
                    $query = "INSERT INTO `cscan_email_tmp` (e_assigned_admin_userID,`mailbox_uid`,`email_date`,`email_to`,`email_from`,`email_from_one`,`email_subject`,`contact_type_m_c`,`email_cc`,`email_bcc`,`deleted`,`panelist_id`,`email_stateID`) 
                        ...
                        if ($parti != '') {
                            ...
                            if ($bodydata != '') {
                                if ($type_no == 0 && ($subtype == 'plain' || $subtype == 'html')) {
                                    ...
                                    $query = "INSERT INTO `cscan_email_text_tmp` (`cetpart`,`cettext`,`cettype`,`muid`,`cetidentification`) VALUES ('" . $DRW->real_escape_string($parti) . "','" . $DRW->real_escape_string(utf8_encode($bodydata)) . "','" . $DRW->real_escape_string($type . '/' . $subtype) . "','" . $DRW->real_escape_string($muid) . "','" . $DRW->real_escape_string($id) . "')";
                                    //  $query = "INSERT INTO `cscan_email_text_tmp` (`cetpart`,`cettext`,`cettype`,`muid`,`cetidentification`) VALUES ('" . $DRW->real_escape_string($parti) . "','" . $DRW->real_escape_string($bodydata) . "','" . $DRW->real_escape_string($type . '/' . $subtype) . "','" . $DRW->real_escape_string($muid) . "','" . $DRW->real_escape_string($id) . "')";
                                   ######### end latest encode message data #################
                                    
                                    if($DRW->query($query, $DRW_main)){
                                        echo '$subtype => '.$subtype.'</br>';
                                        if($subtype == 'html'){echo '$subtype => '.$subtype.'</br>';
                                            copydaMaxmailHtml($muid,$bodydata,$panelist_id);
                                        }
                                            
                                        $query_em = "UPDATE `cscan_email_tmp` SET is_text_file='1' WHERE muid=$muid";
                                    ...
                                    foreach ($efilesArray as $efile) {
                                        ...
                                        $query = "INSERT INTO `cscan_email_file_tmp` (`cefpart`,`cefname`,`ceftype`,`muid`,`cefidentification`,`cefdisposition`,`cefencoding`) 
                                        ...
                                        if ($fp) {
                                            ...
                                            $query = "UPDATE `cscan_email_file_tmp` SET cefpath='" . $DRW->real_escape_string('/contentFiles/' . $yearpath . $monthpath . $daypath . $cefid) . "' WHERE cefid=$cefid";
                                            $DRW->query($query, $DRW_main);
                                        }
                                    }
                                }
...
//                    error_log(date("Y-m-d H:i:s") . ' .NUH ' . $header->message_id);
                ...
                /* if(true !== imap_mail_move($imap_stream, $mailbox_uid, $tempmailbox, CP_UID)){
                  $ehL->write("imap_mail_move(imap_stream (mailbox $mailbox, username $username), $mailbox_uid, $tempmailbox, CP_UID) failed\n".print_r(imap_errors(),true));
                  ++$nimadp_errors;
                  } */
//                imap_delete($imap_stream, $mailbox_uid, FT_UID);
//                imap_expunge($imap_stream);
...
################### Log ###########################
$csv = dirname(__FILE__).'/damaxmailcsv/'.date('Y-m-d').'/'.date('Y-m-d').'_search_input.csv';
if(file_exists($csv) && $num_msg >0){
    $arrLog[0] = 'ReadConsumerEmail(c)'; 
...
if(!empty($arrLog) && $exec == 0){
    ksort($arrLog);
    $localLog = dirname(__FILE__)."/damaxmailcsv/".date('Y-m-d')."/log.csv";
    updateLog($arrLog,$localLog);
    $globalLog = dirname(__FILE__)."/damaxmailcsv/".date('Y-m')."_daLog.csv";
...
function getEAssignment($assign_queue = '') {
    ...
    $sql2 = "SELECT SQL_NO_CACHE userID,COUNT(e_assigned_admin_userID) AS emails FROM 
		...
		(SELECT e_assigned_admin_userID FROM cscan_email_tmp pd WHERE e_assigned_admin_userID<>0$ct) AS cpd 
    ...
    /* $sql2 = "SELECT userID,COUNT(e_assigned_admin_userID) AS emails FROM 
      cscan_admin_users LEFT JOIN
      (SELECT e_assigned_admin_userID FROM cscan_email_tmp pd WHERE email_date>=CONCAT(CURDATE(),' 00:00:00') AND email_date<=CONCAT(CURDATE(),' 23:59:59') AND e_assigned_admin_userID<>0) AS cpd
      ON(userID=e_assigned_admin_userID)
      WHERE is_email_assign_queue$assign_queue=1 AND user_status=1 GROUP BY userID order by emails,RAND() LIMIT 1"; */
