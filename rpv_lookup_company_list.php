...
$parent_field;
ob_clean();
//echo $statusCode = $response['data']['status'];
//echo $data = $response['data']['Status_code']; 
///echo "PA".$parent_field;
 //echo "Company_text".$companytext."Shoalla".$showall."Serahindex".$searchindex;die;
if($parent_field=='retail_company'){
$i = 0;
$usedArray = array();
if($companytext!='' || $showall) {
	$RPVAPILINK=RPVAPIURL.'company/primary';
	if($companytext!=""){
		$postdata['searchcompany']=$companytext;
		$posteddata = json_encode($postdata);
		$get_data = callAPI('POST', $RPVAPILINK, $posteddata);
	}else{
		$get_data = callAPI('GET', $RPVAPILINK, false);
	}
