...
function getNotificationListData(){
    ...
    $query = "SELECT SQL_NO_CACHE DISTINCT ID, searchName, s.userID, notify,
        lastSentDate, queryDate, sendTo, mail_format, addedToDatabase, weekday, is_public,u.emailAddress, u.is_public_user,u.ordering
        FROM cscan_search as s
        JOIN cscan_users as u on (u.userID=s.userID)
        WHERE emailAlert='1' AND u.userID=27156 AND
              active='y' AND
...
function sendNotificationToNate(){
    global $DRW, $DRW_read2, $DRW_main, $DRW_crm;
    
    $to = "nate@competiscan.com,pradeep.chaurasia@nmgtechnologies.com";    
    $subject="Automated E-mail Alert to notify";
    $message=" E-mail Alert sent to Jay.Thudium@fglife.com";
    $html = '<html>
