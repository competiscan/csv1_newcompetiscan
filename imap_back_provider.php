...
$username2 = 'providers@sbkcenter.com';
$password2 = '50!88Comp';

$movemailbox2 = 'INBOX.crm';
$forwardnonmatch2 = 'jennifer@competiscan.com';
$producerArray = array($sever2, $mailbox2, $username2, $password2, $movemailbox2, 'prov_panelist', 1);
   ...
   // $defaultcontact = $account[5];
    ...
    if ($imap_stream === false) {
        ...
        for ($i = 1; $i <=101; $i++) {
            ...
            //pr($count);pr($query);die;
            if ($count[0] == 0) {

                $keep = false;
                //pr($keepSubjects);pr($subject);
                foreach ($keepSubjects as $subj) {
                    if (trim($subj) == $subject) {echo '$keep = 1</br>';
                        $keep = true;
                        break;
                    }
                }
                ...
                if ($keep) {
                    ...
                    if ($contact_type_m_c == 'prov_panelist') {
                        $e_assigned_admin_userID = getEAssignment(3);
                    } else {
                        $e_assigned_admin_userID = 0; //getEAssignment('2');
                    }
                    $result_c_p = $DRW->query("SELECT panelist_id,stateID FROM cscan_panelists WHERE active=1 AND (email='" . $DRW->real_escape_string($checkemail) . "' OR alt_email='" . $DRW->real_escape_string($checkemail) . "' OR more_email LIKE '%" . mysqlLike($checkemail) . "%') AND competi_id REGEXP '-44-' LIMIT 1", $DRW_read2);
                        ...
                        if ($parti != '') {
                            ...
                            if ($bodydata != '') {
                                if ($type_no == 0 && ($subtype == 'plain' || $subtype == 'html')) {
                                    ...
                                    $query = "INSERT INTO `cscan_email_text` (`cetpart`,`cettext`,`cettype`,`muid`,`cetidentification`) VALUES ('" . $DRW->real_escape_string($parti) . "','" . $DRW->real_escape_string(utf8_encode($bodydata)) . "','" . $DRW->real_escape_string($type . '/' . $subtype) . "','" . $DRW->real_escape_string($muid) . "','" . $DRW->real_escape_string($id) . "')";
                                    //  $query = "INSERT INTO `cscan_email_text` (`cetpart`,`cettext`,`cettype`,`muid`,`cetidentification`) VALUES ('" . $DRW->real_escape_string($parti) . "','" . $DRW->real_escape_string($bodydata) . "','" . $DRW->real_escape_string($type . '/' . $subtype) . "','" . $DRW->real_escape_string($muid) . "','" . $DRW->real_escape_string($id) . "')";
                                   ######### end latest encode message data #################
                                    
                                    if($DRW->query($query, $DRW_main)){
                                        $query_em = "UPDATE `cscan_email` SET is_text_file='1' WHERE muid=$muid";
...
function getEAssignment($assign_queue = '') {
    ...
    if ($assign_queue == 2) {
        ...
        $ct = " AND contact_type_m_c='brok_panelist' AND email_read=0";
    }elseif($assign_queue == 4){
        $ct = " AND contact_type_m_c='prov_panelist' AND email_read=0";
    } else {
