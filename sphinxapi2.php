...
class SphinxClient
{
	...
	function __destruct()
	{
		if (isset($this->_socket) && is_resource($this->_socket)) {
			@fclose($this->_socket);
		}
	}
	...
	function AddQuery ( $query, $index="*", $comment="" )
	{
		...
		$req .= pack ( "N", count((array)$this->_weights) ); // weights
		...
		// filters
		$req .= pack ( "N", count((array)$this->_filters) );
		...
		// per-index weights
		$req .= pack ( "N", count((array)$this->_indexweights) );
		...
		// per-field weights
		$req .= pack ( "N", count((array)$this->_fieldweights) );
		...
		// attribute overrides
		$req .= pack ( "N", count((array)$this->_overrides) );
		...
		$this->_reqs[] = $req;
		return count((array)$this->_reqs)-1;
	}
