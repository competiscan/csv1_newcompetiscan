...
if($bid<0){
	$savedQ = "SELECT searchKey,searchType,DMA_ID_mult FROM cscan_search WHERE ID='".$ssid."'";
 ...
 ##################### for the multiple dma case use the original doquery function ##################    
        
     $doqueryuse='';   
        if($data[2]!=''){
            $dmaalldata=explode(",",$data[2]);
            if(count($dmaalldata)>2){
                $doqueryuse=1;
            }
        }
        if($doqueryuse==''){
        //echo $ssid;exit;
            ...
            list($selectQuery,$saved,$sortby) = doQuery($ssid,false,'',false,-1,$dorelev,$doexpans,$unapproved);
	
            list($countQuery) = doQuery($ssid,true,'',false,-1,false,false,$unapproved);
