...
$HEAD ='<script src="includes/myExcel_latest.js?v=20121220" type="text/JavaScript"></script>';
...
<table width="100%" border="0" cellpadding="5" cellspacing="0" class="bodytext">
	...
	<form name="form1" id ="myform1" action="" method="post">
	<!--<form name="form1" id ="myform1" action="" method="post" target="waiter" onsubmit="doBasketSearchChange('form1'); return validate();">-->
	<input type="hidden" name="ssid" value="<?php if($_SESSION['sess_api_searchID']!=""){echo $_SESSION['sess_api_searchID'];}else{echo $_SESSION['sess_api_searchID']='0';} ?>" />
	<input type="hidden" name="bid" value="<?php echo $basket_id; ?>" />
	<input type="hidden" name="page" value="<?php echo $page; ?>" />
	<input type="hidden" name="sort" value="<?php echo $sort; ?>" />
	...
	<table border="0" class="bodytext" cellpadding="4" cellspacing="0" width="100%">
		...
		if ($basket_id < 0) {
			$savedQ = "SELECT mPanelID,sectorID,mChannelID,categoryID,subCategoryID,delmethid_mult,deliveryTypeId,postageId,presortedId,packageTypeId FROM cscan_search WHERE ID=".$ssid;
    ...
    //                    $showArray[] = array("usda","USDA");
		...
		$showArray[] = array("creditUnion","Credit Union");                
				...
				if(in_array(1,$mChannelIDArray)) {
					$showArray[] = array("tip_on_card","Tip-on Card");   
					$showArray[] = array("handwriting","Handwriting");                                   
				} 	
		...
		//}
		// if(in_array(6,$sectorIDArray)|| in_array(9,$sectorIDArray) || in_array(87,$sectorIDArray) ||in_array(90,$sectorIDArray) || in_array(219,$sectorIDArray) || in_array(266,$sectorIDArray) || in_array(315,$sectorIDArray)) {
		// 	$showArray[] = array('incentive_signon','Sign-On Incentive');
			...
			for($j=1;$j<=$cols;$j++){
				for($i=0;$i<$rows;$i++){
					...
					if($arrayi==0){
						...
						$chexblock++;
						$printArray[$i] .= '<label><input type="checkbox" name="allField" value="allField" onclick="chexStart('.$chexblock.','.$chex.',this.value);" />All</label>';
					}
		...
		if (in_array(315,$sectorIDArray) || in_array(90,$sectorIDArray) || in_array(87,$sectorIDArray) || in_array(6,$sectorIDArray) ||
			...
			echo '<tr><td style="padding:5px 10px 5px 0px;" colspan="'.$cols.'"><strong>Additional Details</strong> <a href="#" title="adddition_detail" class="bluelink" onclick="chexStart(1000,'.$chex.',this.title); return false;">Select All</a></td></tr>';

			$showArray2[] = array('','Incentives');
			//if(in_array(5,$sectorIDArray) || in_array(6,$sectorIDArray) || in_array(9,$sectorIDArray) || in_array(87,$sectorIDArray) ||in_array(90,$sectorIDArray) || in_array(219,$sectorIDArray) || in_array(266,$sectorIDArray) || in_array(315,$sectorIDArray)) {
			$showArray2[] = array('incentive_signon','Sign-On Incentive');
		...
		if ($consumer && ($_SESSION['sess_plevel']==1 || $_SESSION['sess_plevel']==2 || !in_array('me_conp',$_SESSION['sess_search_exclude']) || !in_array('me_prizm',$_SESSION['sess_search_exclude']) || !in_array('DMA_CODE',$_SESSION['sess_search_exclude']) || (isset($_SESSION['sess_search_additional_field']) AND $_SESSION['sess_search_additional_field']!=''))) {
			$show_add_var = true;
			echo '<input type="hidden" value=""  name="field[]" id="additional_ppdate" />';
			echo '<input type="hidden" value=""  name="field[]" id="competi_id" />';
		...
		<label><input type="radio" name="file_choice" value="3" checked='checked' <?php //if($templateFileType==3) echo ' checked="checked"'; ?> />Excel(.xlsx)</label><br />
		...
		if(running_php_cmd('exportExcel_back3.php '.$_SESSION['sess_userID'])){
			$savedQ = "SELECT file_choice FROM cscan_progress WHERE userID={$_SESSION['sess_userID']}";
			$rs = $DRW->query($savedQ,$DRW_read);
			$data = $DRW->fetch_row($rs);
			$file_choice = (int)$data[0];
			echo 'exportExcel_out3.php?file_choice='.$file_choice;
		}
		else{
			echo 'blank.html';
		}
		...
		<!--################## RESTRICTION EXCEL DOWNLOAD################-->
		<?php //if($_SESSION['total_searchedcount']<=100){ ?>
		<div id="submits">
		<input type="hidden" value=""  name="btn_value" id="btn_value"/>
		<!--<input type="hidden" value="<?php echo utf8_decode($displayKeywords); ?>"  name="search_keyword" id="search_keyword"/>-->

		<input type="button" name="submit2" value="Generate File for Current Page" style="width:280px;margin-bottom:4px;" class="submitbutton myExcel_download"  />
		<br /><input type="button" name="submit1" value="Generate File for all Records" style="width:280px;margin-bottom:4px;" class="submitbutton myExcel_download" />
		<?php 
		if(in_array(87,$sectorIDArray)){
			echo '<br /><input type="button" name="submit87" value="Comprehensive Banking Report" style="width:280px;margin-bottom:4px;" class="submitbutton myExcel_download" onclick="document.form1.allField.checked=true;set_field();submitPre(0,87); return true;" />';
		}
		if(in_array(90,$sectorIDArray)){
			echo '<br /><input type="button" name="submit90" value="Comprehensive Credit Card Report" style="width:280px;margin-bottom:4px;" class="submitbutton myExcel_download" onclick="document.form1.allField.checked=true;set_field();submitPre(0,90); return true;" />';
		}
		if ($_SESSION['sess_plevel']==1 || $_SESSION['sess_plevel']==2 || !in_array('me_conp',$_SESSION['sess_search_exclude'])) {
			echo '<br /><input type="button" name="submitCons" value="Panelist Report (Panelist ID)" style="width:280px;margin-bottom:4px;" class="submitbutton myExcel_download" onclick="submitPre(0,1); return true;" />';
			if($_SESSION['sess_plevel']==1 || $_SESSION['sess_plevel']==2){
				echo '<br /><input type="button" name="submitCons" value="Panelist Report (Entry ID)" style="width:280px;margin-bottom:4px;" class="submitbutton myExcel_download" onclick="submitPre(0,2); return true;" />';
			}
		}
		?>
		</div>
		...
		<hr />
		<form name="form3" id="formData_chart" action="graph_html.php" method="post" target="_blank" onsubmit="doBasketSearchChange('form3'); return true;">
		<input type="hidden" name="eb_date1" value="" />
		<input type="hidden" name="eb_date2" value="" />
		<input type="hidden" name="eb_date3" value="" />
		<input type="hidden" name="eb_gender" value="" />
		...
		<select name="chart_choice" class="input_box" id="chart_choice">
...
?>
		...
		?>
			...
			<?php } ?>
		<?php } ?>
		<?php ####### Excel Chart Spend Impression ####### ?>
		<?php /*if(in_array(5,$mChannelIDArray) || in_array(9,$mChannelIDArray) || in_array(10,$mChannelIDArray)){ ?>
			<option value="13">Percent Estimated Digital Spend</option>
			<option value="11">Total Estimated Digital Spend</option>
			<option value="14">Percent Estimated Digital Impressions</option>
			<option value="12">Total Estimated Digital Impressions</option>
		<?php } */ ?>
		<?php ####### Excel Chart Spend Impression ####### ?>
		</select>
		</div>
		...
		<!--<input type="button" name="submit3" value="Generate Chart for all Records" style="width:280px;margin-bottom:4px;" class="submitbutton generate_chart" />-->
		</div>
		</td>
		</tr>
	</table>
...
<div id="overlay" style="display:none;">
	...
	<br/>
	<!-- <progress id="progressCounter123" value="30" max="100"></progress> -->
    <div style="color:#00a4e4;"><span id="progressCounter" style="z-index: 5001;">0 %</span></div><br/>
...
$(document).ready(function() {
    $('.myExcel_download').click(function() {
        var btn_value1 = $(this).val();
        $("#btn_value").val(btn_value1);
        var formData = $('#myform1').serializeArray();
        var fileName = "<?php echo $filename_csv; ?>";
		var csv1_search_id="<?php echo $ssid; ?>";
		function generateUniqueId() {
            return  Date.now() + Math.floor(Math.random() * 10000);
        }

        let status_unique_id = generateUniqueId();
        console.log(status_unique_id);
        // let status_unique_id = '<?php //echo (rand()); ?>';
        // console.log(status_unique_id);
        
        function checkProgress() {
            var xhr = new XMLHttpRequest();
            var progressUrl = '<?php echo PROGRESS_MYEXCEL_PROD.$_SESSION['sess_userID']."/".$_SESSION['sess_api_searchID']; ?>/'+status_unique_id;
            xhr.onreadystatechange = function() {
                if (xhr.readyState === XMLHttpRequest.DONE) {
                    if (xhr.status === 200) {
                        let response = xhr.responseText;
                        console.log(response);
						if (response != "100" && response != "500") {
                            $('#progressCounter').text(response + '%');
                            ...
                            $('#progressCounter').text('Request failed');
        ...
        $.ajax({
            url: 'load_ajax_myexceldata.php',
            data: { formData: formData, csv1_search_id:csv1_search_id,status_unique_id: status_unique_id, action: "download_myexcel" },
            type: 'POST',
            beforeSend: function() {
...
$('.generate_chart').click(function() {
	var form_chart_data = $('#formData_chart').serializeArray();
	let screenWidth = window.screen.width;
	let screenHeight = window.screen.height;
	var graph_type=document.getElementById("chart_choice").value;
	// console.log(`Screen Width: ${screenWidth}`);
	// console.log(`Screen Height: ${screenHeight}`);

	//console.warn(graph_type);
	$.ajax({
		url: 'generatechart_data.php',
		data: {form_chart_data: form_chart_data,screenWidth:screenWidth,screenHeight:screenHeight, action: "display_chart" },
		type: 'POST',
		beforeSend: function() {
            $('#progressCounter').hide();
			$('#overlay').css("display", "block");
		},
		success: function(chart_data_response) {
			if(graph_type>2){
				//console.log(chart_data_response);
				let jsonObject = JSON.parse(chart_data_response);
				// Access the 'filelink' property
				const filelink = jsonObject.filelink;
				//console.log(filelink);
				var link_d = document.createElement("a");
                link_d.href = filelink;
				...
				newTab.document.write(chart_data_response);
				newTab.document.close();
				/* Open a new popup window
				const newWindow = window.open('', '_blank', 'width=650,height=500,scrollbars=yes');
				// Check if the popup was blocked or opened successfully
				if (newWindow) {
					// Open the document for writing
					newWindow.document.open();
					// Write the content into the new window
					newWindow.document.write(chart_data_response);
